...
//    Result result = new Result( typeId, columns );
    ...
    focusPic = isEmpty( inner.get( "focusPic" )) ? new String[0] : inner.get("focusPic").split("\\,");//左，右，宽，高，位移，图片名
...
<head>
    ...
    <style>
        #focus{
            ...
            overflow: hidden;
            background-image: url("images/J<%=focusPic[5]%>Focus.png");
            visibility: visible;
        }
    </style>
...
<script language="javascript" type="text/javascript">
    ...
    cursor.initialize({
        ...
        show : function(){
            ...
            var focus = cursor.focusable[blocked].focus;
            if (direct == 1){
                $("focus").style.top = String(<%= focusPic[1]%>+focus*<%= focusPic[4]%>)+"px";
                $("focus").style.backgroundPosition = "0px "+String(focus*(-<%= focusPic[4]%>))+"px";
            } else {
                $("focus").style.left = String(<%= focusPic[0]%>+focus*<%= focusPic[4]%>)+"px";
                $("focus").style.backgroundPosition = String(focus*(-<%= focusPic[4]%>))+"px "+"0px";
            }
        }
    });
    -->
