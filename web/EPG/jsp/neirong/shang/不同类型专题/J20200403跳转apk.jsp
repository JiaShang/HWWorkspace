...
<%
    ...
    if( isEmpty(typeId ) ) typeId = "10000100000000090000000000114540";
    ...
    cursor.initialize({
        ...
        init: function () {
            ...
            for(var j = 0; j < cursor.focusable[0].items.length; j++){
                if(typeof cursor.focusable[0].items[j].posters != "undefined" && typeof cursor.focusable[0].items[j].posters['2'] != "undefined"){
                    posters[j] = cursor.focusable[0].items[j].posters['2'];
                }else {
                    posters[j] = "images/defaultImg.png";
                }
            }
            cursor.focusable[0].items[0] = {
                linkandr : "com.ktcp.tvvideo,com.ktcp.video.activity.MainActivity,cmd_ex###show_video_detail###video_id###dxd1v76tmu0wjuj###video_type###0###video_ui_style###0",
                linkP60Apk :"-a com.tencent.qqlivetv.open -d tenvideo2://?action=1&cover_id=dxd1v76tmu0wjuj"
            };
