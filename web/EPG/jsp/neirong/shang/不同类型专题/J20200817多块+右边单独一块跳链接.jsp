...
<%
    ...
    if( isEmpty(typeId ) ) typeId = "10000100000000090000000000116929";
    ...
    String picture = column == null ? "" : inner.pictureUrl("images/J20200817Bg.png",column.getPosters(),"7");
    String[] sc = {};
    String[] titlePic = {};
    String[] focusPic = {};
...
//    blocked = !isNumber( inner.get("blocked") ) ? 2 : Integer.valueOf(inner.get("blocked"));
    ...
    maxTitleLen = !isNumber( inner.get("maxTitleLen") ) ? 21 : Integer.valueOf(inner.get("maxTitleLen"));
...
<script language="javascript" type="text/javascript">
    ...
    cursor.initialize({
        ...
        init : function(){
            ...
            cursor.focusable[2] = {};
            cursor.focusable[2].typeId ="10000100000000090000000000116932";
            cursor.focusable[2].focus = 0;
            cursor.focusable[2].items = [];
            cursor.focusable[2].items[0] = {
                'name':'安全指南',
                'typeId':'10000100000000090000000000116932',
                'linkto':'/EPG/jsp/neirong/shang/Jpicture.jsp?typeId=10000100000000090000000000116932&direct=1'
            }
            cursor.palyBlocked = cursor.blocked == totalBlocked ? totalBlocked-1 : cursor.blocked;
            cursor.palyIndex = cursor.focusable[cursor.palyBlocked].focus;
            setTimeout(function(){
                initList();
            ...
            if( typeof cursor.moviePos != 'undefined' ) {
                cursor.playIndex = cursor.focusable[cursor.palyBlocked].focus;
                setTimeout(function(){cursor.call('prepareVideo');},150);
            }
        },
        nextVideo   :   function () {
            var playIndex = cursor.playIndex;
            var blocked = cursor.palyBlocked;
        ...
        playMovie : function(item){
            ...
            player.play({
                ...
                callback:function(){
                    cursor.focusable[cursor.palyBlocked].focus = cursor.playIndex;
                    //cursor.call('show');
        ...
        move : function(index){
            ...
            if( index == 11 && listBox.position > 0 && blocked < totalBlocked ){
                cursor.call('loseFocus');
                listBox.changeList(-1);
                cursor.focusable[blocked].focus = listBox.position;
                cursor.call('show');
            }else if( index == -11 && listBox.position < listBox.dataSize-1 && blocked < totalBlocked){
                cursor.call('loseFocus');
                listBox.changeList(1);
                cursor.focusable[blocked].focus = listBox.position;
                cursor.call('show');
                ...
                if (blocked < totalBlocked-1) {
                    cursor.call('loseFocus');
                    blocked++;
                    cursor.blocked = blocked;
                    initList();
                    cursor.call('show');
                }else if (blocked == totalBlocked-1) {
                    cursor.call('loseFocus');
                    cursor.blocked = totalBlocked;
                    cursor.call('show');
                }
            ...
            cursor.moveTimer = setTimeout(function(){
                ...
                if( cursor.blocked !=totalBlocked && (cursor.palyBlocked != cursor.blocked || cursor.playIndex != focus) ) {
        ...
        show:function(){
            ...
            if (blocked < totalBlocked) {
                ...
                $("focus1").style.backgroundImage = "url(images/J20200817Focus10.png)";
            }else {
                var titleFlag = <%=titlePic[5]  %>;
                if (titleFlag == 1) {
                    $("title").style.backgroundImage = "url(images/J<%=titlePic[4]%>Title" + (totalBlocked-1) + ".png)";
                }
                $("focus1").style.backgroundImage = "url(images/J20200817Focus11.png)";
            }
    ...
    function initList() {
        var blocked = cursor.blocked;
        if (cursor.blocked == totalBlocked){
            blocked = totalBlocked-1;
        }
        var focus = cursor.focusable[blocked].focus;
...
<div id="focus1" style="background:transparent no-repeat;" ></div>
