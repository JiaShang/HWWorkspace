...
<%
    ...
    if( isEmpty(typeId ) ) typeId = "10000100000000090000000000114724";
...
<div id="title" style="width:555px;height:58px;left:<%=titleLeft %>px;top:<%=titleTop %>px;position:absolute;overflow:hidden; background:transparent <%= isEmpty(titlePic) ? "none" : (" url(images/J" + titlePic + "Title0.png)")%> no-repeat;visibility: visible;" ></div>
<div id="focus0" style="width:497px;height:72px;left:<%=focusLeft %>px;top:<%=focusTop %>px;position:absolute;overflow:hidden; background:transparent <%= isEmpty(focusPic) ? "none" : (" url(images/J" + focusPic + "Focus.png)")%> no-repeat no-repeat;" ></div>
            ...
            for( var i = 0; i < this.data.length; i ++){
                ...
                if (i == 0){
                    ...
                    cursor.focusable[i].items[0] = {
                        'name': '底部入口',
                        'linkto': '/EPG/jsp/neirong/shang/Jpicture.jsp?typeId=' + this.data[i]["id"]
                    }
                }
            }
