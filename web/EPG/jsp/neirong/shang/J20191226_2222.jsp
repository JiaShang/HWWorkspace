...
<%
    ...
    if( isEmpty(typeId ) ) typeId = "10000100000000090000000000113440";
    ...
    picture = "images/J20191226_2Bg.png";
    String titlePic = "";
    String[] sc = {};
...
<div style="width:1280px;height:720px;left:0px;top:0px;position:absolute;overflow:hidden; background:transparent <%= isEmpty(picture) ? "url(images/J20191226_2Bg.png)" : (" url('" + picture + "')")%> no-repeat;"></div>
<%if( sc.length >= 5 ){%>
    <div id='scrollBarContainer' class='scrollBarContainer'><div id='scrollBar'></div></div>
<%}%>
    <div id="title" style="width:440px;height:72px;left:685px;top:75px;position:absolute;overflow:hidden; background:transparent no-repeat;" ></div>
    <div id="focus" style="width:497px;height:72px;left:705px;top:145px;position:absolute;overflow:hidden; background:transparent url(images/J20191226_2Focus.png) no-repeat;" ></div>
...
<script language="javascript" type="text/javascript">
    ...
    cursor.initialize({
        ...
        move : function(index){
            ...
            if( blocked <= 0 && index === -1 || blocked === 1 && index === 1 || index === 11 && focus <= 0 || index === -11 && focus + 1 >= items.length ) return;
        ...
        show:function(){
            ...
            var flowCursorIndex = Math.floor(focus / pageCount) * pageCount;
               // $("title1").className = "title title1 title1Focus";
            $("title").style.backgroundImage = "url(images/J20191226_2Title"+String(blocked)+".png)";
            <% if( sc.length >= 5 ) { %>
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                ...
                html +='</div></div>';
                if(i == focus){
                    cursor.focusPos = i%pageCount;
                    var lastLength = items.length%pageCount;
                    if(Math.ceil( ( focus + 1.0 ) / pageCount) == + Math.ceil( ( items.length * 1.0 ) / pageCount)){
                        cursor.focusPos = cursor.focusPos + (pageCount-lastLength);
                    }
                }
            }
            $("flowed").innerHTML = html;
            setTimeout(function () {
                $("focus").style.top =String(cursor.focusPos*60+148)+"px";
            },100);
