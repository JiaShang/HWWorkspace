...
<%
    ...
    if( isEmpty(typeId ) ) typeId = "10000100000000090000000000113209";
...
<div id='button' style="position: absolute;width: 321px;height: 62px;top: 555px;left: 725px;background: url('images/J20191114button0.png') no-repeat;"></div>
</body>
<script language="javascript" type="text/javascript">
    ...
    cursor.initialize({
        ...
        init : function(){
            ...
            cursor.focusable[1] = {};
            cursor.focusable[1].typeId = "";
            cursor.focusable[1].focus = 0;
                cursor.focusable[1].items = [{
                'name':'上传流程',
                'linkto':'/EPG/jsp/neirong/shang/J20191114Detail.jsp'
            }];
            cursor.call('show');
        ...
        move : function(index){
            ...
            if( blocked === 0 && ( index === -1 || index === 11 && focus <= 0 || index === -11 && focus + 1 >= items.length)) return;
        ...
        show:function(){
            ...
            $("button").style.backgroundImage = "url(images/J20191114button"+blocked+".png)";
            <% if( sc.length >= 5 ) { %>
            if( items.length <= pageCount ) {
                ...
                $("scrollBar").style.top = Math.ceil( focus * 1.0 && blocked === 0/ (items.length - 1) * ( scrollBarHeight - h ) ) + "px";
