        ...
        var flowedShow = function(index){
            ...
            for(var i = 0; i < items.length; i += 1) {
                ...
                if( focus % pageSize == i ) {
                    ...
                    $("mask").className = "absolute maskColumn2Item";
                    $("mask").style.top = (131 + focus %  pageSize * 51 ) +  "px";
                    $("mask").innerHTML = "<div class='icon'></div><div class='txt'>" + ( item.length >= <%=popItemMaxCharLength%> ? ( "<marquee class='column2ItemMarquee' scrollamount='5'>" + item.text + "</marquee>" ) : item.text) + "</div>";
                } else {
        ...
        function init(){
            ...
            $.buildUserInterface = function(option){
                ...
                if(typeof option.flowed != 'undefined' ) {
                    html += "<div id='flowed' class='absolute column2Flowed'></div>";
                    focusable.flowed = [];
