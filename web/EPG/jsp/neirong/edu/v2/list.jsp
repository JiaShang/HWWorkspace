...
<html>
<%
    ///EPG/jsp/neirong/edu/v2/list.jsp?parentId=10000100000000090000000000109312&index=0&typeId=10000100000000090000000000109314&useChild=1&EPGflag=EPGflag
    //首先获取参数中的栏目ID
    String typeId = inner.get("typeId");
    boolean redirect = isEmpty(inner.get("redirect"));
    if( isEmpty(typeId) ) typeId = "10000100000000090000000000107189";
    int start = 0;
    if( isNumber(inner.get("start"))) start = Integer.parseInt(inner.get("start"));
    ...
    try {
        ...
        for (Column column : columnList ) {
            result = inner.getVodList(column.id,start, 199);
        ...
        inner.setWithId( with );
        result = inner.getVodList(typeId, start, 199);
    ...
    if( column.getName().equalsIgnoreCase("培训课堂") ){
        majorWidth = 222;
    } else if( column.getName().equalsIgnoreCase("初心使命") ){
        majorWidth = 270;
    } else if( column.getName().equalsIgnoreCase("党性教育") ){
        majorWidth = 300;
    } else if( column.getName().equalsIgnoreCase("党章党规") ) {
        majorWidth = 400;
    } else if( column.getName().equalsIgnoreCase("先进典型") || column.getName().equalsIgnoreCase("精品党课") ) {
        majorWidth = 340;
    }
%>
<head>
    ...
    <style>
        ...
        .majorContent {width:<%=majorWidth + 20%>px;height:260px;left:<%=420-majorWidth%>px;top:400px;overflow:hidden;position:absolute;}
        .majorContainer{width:<%=majorWidth + 20%>px;left:0px;top:0px;position:absolute;overflow: visible;}
        .majorItem,.majorItemFocus {width:<%= majorWidth + 20%>px;height:54px;float:right;background-color: transparent;overflow: visible;}
        .majorItem div,.majorItemFocus div {color:white;font-size:24px;padding:8px 15px 8px 15px;text-align: right; overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;background-color: transparent;}
        ...
        .subItem .item,.subItemFocus .item {width:680px;height: 52px;color:white;font-size:22px;line-height: 45px; padding:2px 20px 3px 20px; overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;background-color: transparent;}
        .subItem .text,.subItemFocus .text { float:left;width:520px;height: 52px;color:white;font-size:20px;line-height: 45px; padding:2px 20px 3px 20px; overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;background-color: transparent;}
        .subItem .time,.subItemFocus .time { float:left;width:140px;height: 52px;color:white;font-size:20px;line-height: 45px; overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;background-color: transparent;overflow: hidden;}
        .subItemFocus .item,.subItemFocus .text ,.subItemFocus .time {color:black;background-color: yellow;}

        .subItemFocus .item .marqueed {width:640px;}
        .subItemFocus .text .marqueed {width:490px;}
...
<script language="javascript" type="text/javascript" defer="defer">
    ...
    var initialize = {
        ...
        isShowTime : function(item){
            //共产党员不能显示日期
            return typeof item != 'undefined' && ( item.name == "焦点访谈" || item.name == "新闻1＋1" || item.name == "党建要闻" );
        },
        data : [<%
                String html = "";
        ...
        init : function(){
            ...
            for(var i = 0; i < this.data[0].data.length; i ++ ){
                var item = this.data[0].data[i];
                var showTime = cursor.call("isShowTime", item);
                if( showTime ) {
                    for( var j = 0; j < this.data[i + 1].data.length; j ++ ){
                        var child = this.data[ i + 1].data[j];
            ...
            for( var i = 0; i < this.data.length; i ++){
                var o = this.data[i];
                if( <%=redirect%> && o.redirected ) continue;
                cursor.focusable[i] = {};
            ...
            if(this.focused.length == 0 ){
                ...
                if( index.isEmpty() || isNaN(Number(index)) ) {
                    var name = cursor.href.query("name");
                    if( name.isEmpty() ){
                        cursor.blocked = 0;
                    } else {
                        name = decodeURIComponent(name);
                        var search = function(){
                            var ret = 0, found = false;
                        ...
                        cursor.focusable[0].focus = cursor.blocked = search();
                    }
        ...
        move : function(index){
            ...
            if( blocked === 0) {
                ...
                var item = cursor.focusable[0].items[blocked];
                var showTime = cursor.call("isShowTime",item);
                ...
                if( showTime ) {
                    ...
                    $("item" + id ).innerHTML = '<div class="item">' + item.name + "</div>";
                }
            ...
            (function(blocked,focus){
                cursor.moveTimer = setTimeout(function(){
                    ...
                    var width = showTime ? 490 : 596;
                    var id = (showTime ? "text" : "item") + ( focus + 1);
                    clearTimeout(cursor.moveTimer);
                    cursor.moveTimer = undefined;
                    cursor.calcStringPixels(item.name, 22, function(pixelsWidth){
                        if( pixelsWidth >= width && cursor.blocked === blocked && cursor.focusable[cursor.blocked].focus === focus ){
                            $(id).innerHTML = '<div class="item"><marquee class="marqueed" scrollamount="10">' + item.name + "</marquee></div>";
                        }
                    });
                },1500);
            }(blocked, focus));
            ...
            if( items.length > 0 ) {
                for( var i = 0; i < items.length; i ++ ){
                    ...
                    if( showTime ) {
                        ...
                        html += '<div id="item' + id + '" class="subItem"><div class="item">' + item.name + "</div></div>";
                    }
        ...
        select : function() {
            ...
            if( blocked === 0 ){
                ...
                var column = cursor.focusable[0].items[blocked] || {};
                if( column.name === "党性教育" && item.isSitcom && typeof item.redirect === 'string' ){
                    item.linkto = "/EPG/jsp/neirong/edu/v2/list.jsp?parentId=" + column.id + "&redirect=1&typeId=10000100000000090000000000109430&name=" + encodeURIComponent(item.name);
                }
