...
<%
    ...
    */
    ...
    infos.add(new ColumnInfo("在线测试","", 0, 0));                                            //在线测试
    ...
    for ( int i = 0; i < infos.size(); i++) {
        ...
        if( info.name.equalsIgnoreCase("在线测试")) {
            totalHeight += visibleHeight;
            margins.add( totalHeight );
            content.append("<div id='blocked").append( i + 1).append("' style='height:").append(visibleHeight).append("px;' class='onlineTest'></div>");
        ...
        List<Vod> vodList = (List<Vod>)result.getData();
...
<head>
    ...
    <style>
        ...
        .crumbItem .mask {background: #EC9F00 none no-repeat;}
        ...
        .onlineTest {background: transparent url('images/onlineTestBg.png') no-repeat no-repeat 5px 20px;}
        .questions {width:900px;height:300px;position:relative;overflow: hidden;left:40px;top: 140px;}
        .context {width:900px;height: auto;}
        .questions .question {width:900px;height:100px;overflow: hidden;}
        .questions .question .title {width:900px;height:38px;line-height: 35px;color:white;font-size:22px;overflow: hidden;text-align: left;overflow: hidden;}
        .questions .question .choices {width:900px;height:42px;overflow: hidden;}
        .questions .question .item,.questions .question .choice,.questions .question .selected,.questions .question .choiceSelected {width:160px;height:42px;overflow: hidden;float:left;}

        .icon {width:40px;height:40px;float:left;background:transparent url("images/onlineTestIcon.png") no-repeat 0px 0px; background-position: -40px -7px;}
        .option {width:120px;height:42px;float:left; color:white;font-size:20px;text-align: left;line-height: 40px;color:white;}

        .questions .question .choice .option,.questions .question .choiceSelected .option{color:#ead700;}
        .questions .question .choice .icon {background-position: -164px -6px;}
        .questions .question .selected .icon {background-position: -100px -7px;}
        .questions .question .choiceSelected .icon {background-position: -224px -6px;}
        .onlineTestCommit {width:122px;height:34px;position:absolute;left:855px;top:485px;visibility: hidden;background:transparent url("images/onlineTestIcon.png") no-repeat 0px -65px;}
        ...
        .onlineTestResult {position: absolute;width:1280px;height:720px;left:0px;top:0px;background: transparent url("images/onlineTestResultBg.png") no-repeat left top;visibility: hidden;}
        .resultRight,.resultError {width:30px;height:30px;font-size:20px;line-height:30px; text-align: center;color: white;position:absolute;left:705px;top:307px;}
        .resultError {top:343px;}
    </style>
...
<script language="javascript" type="text/javascript">
    ...
    var initialize = {
        ...
        init        :   function(){
            ...
            var questions = [
                {question:'1、从现在到二〇二〇年，是全面建成小康社会决胜期。',choices:["A、对","B、错"],answer:0},
                {question:'2、预备党员的预备期为一年。',choices:["A、对","B、错"],answer:0},
                {question:'3、党的全国代表大会每三年举行一次。',choices:["A、对","B、错"],answer:1}];
            // 此为搜索按钮
            cursor.focusable[cursor.focusable.length - 2].items = questions;
        ...
        move        :   function(index){
            ...
            if( columnName === "在线测试") rowSize = 1;
            if( blocked === 0 && ( index == -1 || index === -11 && focus + 1 >= items.length ) ||
                (blocked >= 1 && blocked < cursor.focusable.length - 1 && blocked != 6 /*在线考试*/ ) && index === 1 && ( focus % rowSize === (rowSize - 1) || focus + 1 >= items.length ) ||
            ...
            if(  blocked === 0 ) {
                if( index === 11 || index === -11 )
                {
                    ...
                    if( columnName === "在线测试") {
                        ...
                        cursor.focusable[blocked].items[focus].current = 0;
                    } else {
                ...
                if( index == 1 && selected + 1 >= choices.length || cursor.focusable[blocked].showCommit  && ( index == 1 || index == -1)) return;

                if( cursor.focusable[blocked].showCommit ) {
                    $("onlineTestCommit").style.visibility = "hidden";
                } else {
                ...
                if( index == -11 || index == 11 ) {
                    if( cursor.focusable[blocked].showCommit && index == 11 ) {
                        focus = items.length - 1;
                        cursor.focusable[blocked].showCommit = false;
                    } else {
                        focus += index > 0 ? -1 : 1;
                        if( focus >= items.length || focus < 0 ) {
                            if( focus < 0 ) {
                                cursor.focusable[0].focus = blocked -= 1; focus = Math.floor((cursor.focusable[blocked].items.length - 1.0) / 3) * 3; //此处的3原本应该用变量代替
                            } else {
                                //移动到了提交答案按钮上
                                if( ! cursor.focusable[blocked].showCommit ) {
                                    cursor.focusable[blocked].showCommit = true; focus -= 1; //因为前面 +1 ，此处-1 是为了下次移动焦点不会出错
                                } else {
                                    cursor.focusable[blocked].showCommit = false;
                                    cursor.focusable[0].focus = blocked += 1; focus = 0;
                                }
                            }
                            moveOut = true;
                        }
                    ...
                    selected += index;
                    if( selected < 0 ) {
                        blocked = 0; items[focus].current = selected = 0;
                        focus = cursor.focusable[blocked].focus;
                        moveOut = true;
                    }
                }
                    ...
                    if( nextFocus < 0 || nextFocus >= items.length) {
                        if( nextFocus < 0 ){
                            ...
                            if( nextItemCount === 0 ) {
                                ...
                                blocked = current; nextFocus = nextItemCount - 1;
                            } else {
                                blocked = current;
        ...
        marginShow  : function(){
            ...
            if( columnName === "在线测试") {
                ...
                if( invisible > <%= visibleHeight%> ) {
                    var marginTop = Math.floor( invisible * 1.0 / <%= visibleHeight%>) * <%= visibleHeight%>;
                    var marginOffset = 0;
                    if( columnName !== '经典影视' && columnName !== "在线测试" || Math.ceil((focus + 1.0) / rowSize) % 2 === 1 && Math.ceil((focus + 1.0) / rowSize) < Math.ceil(itemsCount * 1.0 / rowSize )) {
        ...
        focusQuestions : function(index){
            var blocked = 6;//6 是在线考试
            var focus = cursor.focusable[blocked].focus;
            var items = cursor.focusable[blocked].items;
            var showCommit = cursor.focusable[blocked].showCommit;
            if( !showCommit ) {
                var item = items[focus];
                var selected  = typeof item.selected == "undefined" ? 0 : item.selected;
                var current  = typeof item.current == "undefined" ? 0 : item.current;

                if($("onlineTestCommit"))$("onlineTestCommit").style.visibility = "hidden";

                $("choice" + (focus + 1) + (current + 1)).className = item.selected === current ? "choiceSelected" : "choice";
                var margin = Math.floor((focus * 1.0) / 3) * -270;//270是三道题的高度
                $("context").style.marginTop = margin + "px";
            } else {
                $("onlineTestCommit").style.visibility = "visible";
            }
        },
        showQuestions : function( item ) {
            var html = "";
            var items = item.items;
            html += "<div id='questions' class='questions'><div id='context' class='context'>";
            for( var i = 0; i < items.length; i ++){
                ...
                for(var j = 0; j < choices.length; j ++ ) {
                    ...
                    html += "<div id='choice" + id + (j + 1) + "' class='item'><div class='icon'></div><div class='option'>" + choice + "</div></div>";
                }
                html += "</div></div>";
            }
            html += "</div></div><div id='onlineTestCommit' class='onlineTestCommit'></div>";
            $("blocked" + (cursor.focusable[0].focus)).innerHTML = html;
            cursor.focusable[cursor.focusable[0].focus].loaded = true;
        },
        /**
         * 延时加载
        ...
        lazyLoad : function(blocked){
            ...
            var columnName = cursor.focusable[0].items[cursor.focusable[0].focus].name;
            if( columnName === "在线测试") {
                cursor.call("showQuestions", cursor.focusable[blocked]);
                return;
            }
            ...
            if(blocked === 6 ) {    //在线测试
                ...
                if( showCommit ) {
                    var answered = function(){
                        ...
                        for( var i = 0; i < items.length; i ++ ) {
                            item = items[i];
                            if( item.selected == item.answer ) right ++;
                            else error ++;
                        }
                        $("onlineTestResult").innerHTML = "<div class='resultRight'>" + right + "</div><div class='resultError'>" + error + "</div>";
                        $("onlineTestResult").style.visibility = "visible";
                ...
                $("choice" + (focus + 1) + (current + 1)).className = "choiceSelected";
            ...
            if( blocked === 0 ){
                ...
                if( focus === 0 ) {
                    if( typeof iPanel.eventFrame.systemId !== 'undefined' )  {
                        ...
                        top.window.location.href = iPanel.eventFrame.portal_url;
                    }
                    return;
                ...
                if( item.isSitcom === 0 ){
                    ...
                    if( typeof item.isRequiredCourse !== 'undefined'){
                        //此ID用来设置在列表页读取标题
        ...
        show        :   function(){
            ...
            if( blocked === cursor.focusable.length - 1 ) {
                ...
                if( blocked !== 0 && blocked !== 6  ) { //blocked == 6 是在线考试
