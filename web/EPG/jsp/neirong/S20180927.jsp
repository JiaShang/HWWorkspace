...
<head>
    ...
    <style>
        .mask {width:59px;height:437px;top:214px;position:absolute;background:transparent url('images/mask-2018-09-27.png') no-repeat;background-position: 0px 0px;}
        ...
        .image {width:260px;height:142px;position:absolute;left:4px;top:4px;overflow: hidden;}
        .image img {width:260px;height:142px;}

        .text{width:260px;height:30px;left:4px;top:144px;background-position:0px -210px;}
        .text .name{width:180px;height:30px;color:white;font-size:18px;line-height:30px;text-align: left;left:0px;top:0px;overflow: hidden;position:absolute}
        .text .vote{width:80px;height:30px;color:white;font-size:18px;line-height:30px;text-align: right;left:178px;top:0px;overflow: hidden;position:absolute}

        .focusVote{width:264px;height:60px;background-position:0px -70px;left:0px;top:178px;}
        .voteBtn{width:264px;height:60px;background-position:0px -140px;left:0px;top:178px;overflow:hidden;}

        .mask1{width:148px;height: 63px;left:65px;top:106px;background-position: 0px 0px;}
        .tooltip{width: 1280px;height:720px;position:absolute;left:0px;top:0px;}
    </style>
    <script language="javascript" type="text/javascript" src="player/common.js"></script>
</head>
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2018-09-27.jpg') no-repeat;" onUnload="exit();">
<div id="mask"></div>
...
<script language="javascript" type="text/javascript">
    ...
    var initialize = {
        ...
        init        :   function(){
            ...
            cursor.voteId = 430;
            cursor.enabled = (new Date()).Format("yyyy-MM-dd hh:mm:ss") < "2018-10-11 00:00:00";
            ...
            cursor.focusable[1] = {focus : 0 ,items:[ {name:'活动详情',linkto:'/EPG/jsp/neirong/S20180823Role.jsp'}]};
        ...
        move        :   function(index){
            ...
            if( blocked == 0 ) {
                ...
                if( index == -1 ) {
                    ...
                    if( focus + column >= items.length && Math.ceil( (focus + 1.0) / column) == Math.ceil( items.length * 1.0 / column)  ) return;
                    else focus += column;
                    if( focus >= items.length ) focus = items.length - 1;
                }
            }  else {
        ...
        show        :   function(){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                ...
                html += '<div class="mask text" style="background-repeat: no-repeat"><div class="name" id="txt' + String(i + 1) + '">' + String( i + 1) + '号 ' +  item.name + '</div><div class="vote" id="vote' + String(i + 1) + '">' + String(item.voteCount || 0) +  '</div></div>';
