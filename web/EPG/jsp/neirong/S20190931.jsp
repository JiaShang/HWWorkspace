...
<%
    ...
    if( isEmpty(typeId ) ) typeId = "10000100000000090000000000112822";
    infos.add(new ColumnInfo(typeId, 0, 99));

    //获取当前栏目的详细信息
    Column column = new Column();
    column = inner.getDetail(typeId,column);
    String picture = column == null ? "" : inner.pictureUrl("images/bg-2019-09-31.png",column.getPosters(),"7");
    String maskPic = column == null ? "" : inner.pictureUrl("images/mask-2019-09-31.png",column.getPosters(),"4");
    String[] sc = {};
...
<head>
    ...
    <style>
        ...
        .mask{width:222px;height:54px;top:564px;background: transparent url("<%=maskPic%>") no-repeat; background-position: 0px 0px; position: absolute;}
        .mask1 { left:137px;background-position: 0px 0px;}
        .mask2 { left:364px;background-position: -250px 0px;}
    </style>
    ...
    cursor.initialize({
        ...
        init : function(){
            ...
            cursor.focusable[1] = {
                ...
                items:[{ name : "十佳" ,linkto:"/EPG/jsp/neirong/S20190931Vote1.jsp"},
                       { name : "最美" ,linkto:"/EPG/jsp/neirong/S20190931Vote2.jsp"}]
            };
            cursor.call('show');
        ...
        move : function(index){
            ...
            if( blocked == 0 && ( index === -1 || index === 1 || index === 11 && focus <= 0  ) || blocked == 1 && ( index == -1 && focus <= 0 || index == 1 && focus >= 1 || index == -11 ) ) return;
            if( blocked == 0) {
        ...
        show:function(){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                var item = items[i];
                html += '<div class="' + ( i == focus && cursor.blocked == 0 ? 'focusItem' : 'item' ) + '"><div class="container" id="txt' + ( i + 1) + '">';
