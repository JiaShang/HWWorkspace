...
<head>
    ...
    <script language="javascript" type="text/javascript">
        ...
        function init(){
            ...
            var links = [
                {text:'优生活的测试链接',length:<%= StringUtil.length("优生活的测试链接")%>,shortText:'<%= StringUtil.limitStringLength("优生活的测试链接", popItemMaxCharLength)%>',playType:-3,picture:'images/item-2018-01-05-2.jpg',link:'http://192.168.17.12:8080/web/iptv/BookActionCQ_GD.do?method=login&id=11&from=bxj'},
                {text:'电竞游戏的测试链接',length:<%= StringUtil.length("电竞游戏的测试链接")%>,shortText:'<%= StringUtil.limitStringLength("电竞游戏的测试链接", popItemMaxCharLength)%>',playType:-3,picture:'images/item-2018-01-05-3.jpg',link:'http://192.168.34.131:8300/video_hall/special/jumanji/'},
                {text:'巴渝文化云测试链接',length:<%= StringUtil.length("巴渝文化云测试链接")%>,shortText:'<%= StringUtil.limitStringLength("巴渝文化云测试链接", popItemMaxCharLength)%>',playType:-3,picture:'images/item-2018-01-05-4.jpg',link:'http://192.168.18.166:82/test.html'}
            ];

            focusable[0].items.insertAt(0,links[0]);
            focusable[0].items.insertAt(1,links[1]);
            focusable[0].items.insertAt(2,links[2]);
