...
<%
    ...
    bodyTop = inner.pictureUrl("images/bg-2018-09-22-top.jpg", column.getPosters(), "7",0 );
    bodyLeft = inner.pictureUrl("images/bg-2018-09-22-left.jpg", column.getPosters(), "7",1 );
    bodyRight = inner.pictureUrl("images/bg-2018-09-22-right.jpg", column.getPosters(), "7",2 );
    bodyBottom = inner.pictureUrl("images/bg-2018-09-22-bottom.jpg", column.getPosters(), "7",3 );
...
<head>
    ...
    <style>
        ...
        .bodyTop {position: absolute;z-index:0;overflow: hidden; left:0px;top:0px;width:1280px;height: <%=videoTop%>px;<%= !isEmpty(bodyTop) ? "background:transparent url('" + bodyTop + "') no-repeat left top;" : ""%>}
        .bodyLeft {position: absolute;z-index:0;overflow: hidden; left:0px;top:<%=videoTop - 1 %>px;width:<%= videoLeft %>px;height: <%=videoHeight + 2 %>px;<%= !isEmpty(bodyLeft) ? "background:transparent url('" + bodyLeft + "') no-repeat left top;" : ""%>}
        .bodyRight {position: absolute;z-index:0;overflow: hidden; left:<%=videoWidth + videoLeft%>px;top:<%=videoTop - 1 %>px;width:<%= 1280-videoLeft-videoWidth %>px;height: <%=videoHeight + 2%>px;<%= !isEmpty(bodyRight) ? "background:transparent url('" + bodyRight + "') no-repeat left top;" : ""%>}
        .bodyBottom {position: absolute;z-index:0;overflow: hidden; left:0px;top:<%=videoTop + videoHeight%>px;width:1280px;height: <%=720-videoTop-videoHeight%>px;<%= !isEmpty(bodyBottom) ? "background:transparent url('" + bodyBottom + "') no-repeat left top;" : ""%>}
        ...
        .marquee {line-height: <%=itemHeight - focusBorderWidth * 5 - 1%>px; height:<%=itemHeight - focusBorderWidth * 5 - 1%>px;width:<%= itemWidth - 50 - 32 %>px;}
        ...
        init : function(){
            ...
            if( cursor.split ) {
                for( var i = 0 ; i < cursor.focusable[0].items.length; i ++){
                    var txt = cursor.focusable[0].items[i].name;
                    var match = /(.*)（(.*)）/gi.exec( txt );
                    if (typeof match === 'undefined' || match === null ) {
                        cursor.focusable[0].items[i].mark = ''; continue;
                    }
                    cursor.focusable[0].items[i].name = match[1];
                    cursor.focusable[0].items[i].mark = match[2];
                }
            }
            cursor.call('show');
        ...
        show : function(){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                ...
                if( i === played && i === focus ) {
                    ...
                    html += "<div class='item'>";
                    html += "<span class='itemPlayFocusedText' id='txt" + id + "'></span>";//<span class='itemPlayed'></span>
                } else if(i === played ) {
                    ...
                    html += "<div class='itemIconFocus'></div>";
                    html += "<div class='item'>";
                    html += "<div class='itemFocusText' id='txt" + id + "'></div>";
                } else {
                    html += "<div class='itemIcon'></div>";
                    html += "<div class='item'>";
            ...
            (function(id,value){
                cursor.calcStringPixels(value, <%= fontSize %>, function(pixelsWidth){
                    var innerHTML = pixelsWidth > <%= itemWidth - 50 - 16 %> ? ('<marquee class="marquee" scrollamount="10">' + value + "</marquee>") : '<span class="inner">' + value + '</span>' ;
