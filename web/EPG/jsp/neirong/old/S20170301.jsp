...
<%
    ...
    final String[] types = {"10000100000000090000000000107322"};
...
<head>
    <title>是时候和金刚狼说再见</title>
    <style>
        .mask {width:195px;height:41px;position:absolute;background:transparent url('images/mask-2017-03-01.png') no-repeat;background-position: 0px 0px;}
        .mask1{left:84px;top:527px;background-position:0px 0px}
        .mask2{left:84px;top:567px;background-position:0px -45px}
        .mask3{left:290px;top:527px;background-position:0px -90px}
        .mask4{left:290px;top:567px;background-position:0px -135px}
        .mask5{left:499px;top:527px;background-position:0px -180px}
        .mask6{left:499px;top:567px;background-position:0px -225px}
        .mask7{left:499px;top:607px;background-position:0px -270px}
                ...
                for(int i = 0; i < list.size() ; i++){
                    ...
                    for( int j = 0; j < vodList.size() ; j++ ){
                        ...
                        String str = "{ mid:'" + vod.getId() + "'," +
                                   "style:'mask mask" + (j + 1) + "'" + "," +"playType:" + vod.getIsSitcom() ;
                            str += "}" ;
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && ( index == 11 && focus % 2 == 0 && focus != 6 || index == -11 && focus % 2 == 1 && focus != 5 || index == -1 && focus <= 1 || index == 1 && focus >= 4 ) ) return;
                    if( index == -11 || index == 11 ) {
                        focus += index > 0 ? -1 : 1;
                    } else {
                        if( index == -1 && focus >= 5 ) focus = 3;
                        else focus += index * 2;
                    }
                }
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-03-01.jpg') no-repeat;" onUnload="exit();"></body>
</html>