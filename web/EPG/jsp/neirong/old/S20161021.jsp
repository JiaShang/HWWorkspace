...
<%
    ...
    final String[] types = {"10000100000000090000000000106748","10000100000000090000000000106748"};
    final int[] position = {5,0,3,5};
    //TODO:每次需要修改
...
<head>
    <title>制作人篇</title>
    <style>
        .mask {width:83px;height:30px;top:376px;position:absolute;background:transparent url('images/mask-2016-10-21.png') no-repeat;background-position: 0px 0px;}
        ...
        .blocked2,.blocked2Focus { width:210px;height:275px;top:415px;}
        .blocked2 {background-position: -250px -50px;}
        .blocked2Focus {background-position: 0px -50px;}
        ...
        .text {width:162px;height:22px;top:635px;font-size: 18px; line-height: 22px; color:#ffffff;position:absolute; text-align: center;}
        .text1 {left:81px;}
        .text2 {left:271px;}
        .text3 {left:463px;}
        .textFocus{color:#ff3a3a;}

        .flowed {}
        .after {width:423px;height:22px;left:136px;top:380px;background-position: -100px 0px;}
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 ) {
                        ...
                        if( index == 1 || index == -1 ) {
                            if(index == 1 && focus == focusable[blocked].items.length - 1 ) {
                            ...
                            blocked = 0;
                            if( focus == 0 )  focus = 0;
                            else if( focus == focusable[1].items.length  -1 ) {
                                focus = focusable[blocked].items.length  -1;
                            } else {
                                focus = 2;
                            }
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                ...
                if( i == focus && blocked == index ){
                    $("item" + ( i + 1)).className = "mask blocked2Focus item" + (i + 1);
                    $("text" + ( i + 1 )).className = "text text" + ( i + 1) + " textFocus";
                    $("text" + ( i + 1 )).innerHTML = item.text != item.shortText ? ( "<marquee>" + item.text +  "</marquee>" ) : item.shortText;
                } else {
                    $("item" + ( i + 1)).className = "mask blocked2 item" + (i + 1);
                    $("text" + ( i + 1 )).className = "text text" + ( i + 1);
                    $("text" + ( i + 1 )).innerText = item.shortText;
                }
            }
        }
        ...
        function init(){
            ...
            $.buildUserInterface = function(option){
                ...
                if(typeof option.flowed != 'undefined' ) {
                    html += "<div id='item1'></div><div id='item2'></div><div id='item3'></div><div id='text1'></div><div id='text2'></div><div id='text3'></div>";
                    focusable.flowed = [];
            ...
            $.buildUserInterface({flowed:[1],popup:undefined,vote:undefined,video:undefined,position:undefined,external:{after:'<div class="mask after"></div>',before:undefined}});
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-10-21.jpg') no-repeat;" onUnload="exit();"></body>
</html>