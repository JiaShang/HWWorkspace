...
<%
    ...
    final String[] types = {"10000100000000090000000000107922"};
...
<head>
    <title>英美剧暑期1元购</title>
    <style>
        .mask {width:235px;height:40px;position:absolute;background:transparent url('images/mask-2017-06-29.png') no-repeat;background-position: 0px 0px;}
        .mask0 {width:190px;height:50px;left:573px;top:368px; background-position: 0px 0px;}
        .mask1 {left:394px;top:503px; background-position: 0px -50px;}
        .mask2 {left:684px;top:503px; background-position: 0px -100px;}
        .mask3 {left:394px;top:551px; background-position: 0px -150px;}
        .mask4 {left:684px;top:551px; background-position: 0px -200px;}
        .mask5 {left:394px;top:598px; background-position: 0px -250px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,items : [{name:'活动细则',style:'mask mask0',playType:-3,link:'http://tvservicehall.cqccn.com/ccn-tv/dg/combo/1720?comboCateParentId=1003'}]},
            <%
            value = "";
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && !$.vote.tooltiped && blocked == focusable.length - 1 && focus == 1 ) focus = focus + 1;
                    ...
                    if( blocked == 0 && index != -11 ||
                        blocked == 1 && ( index == -1 && focus % 2 == 0 || index == 1 && focus % 2 == 1 || index == -11 && focus >= 3 )
                    ) return;
                    if( blocked == 0 ){
            ...
            switch(item.playType){
                ...
                case -3:
                    ...
                    if( ! item.link.startWith('http') ){
                        ...
                        url = item.link;
                        url += '&cardNo=' + CA.card.serialNumber;
                        url += '&terminalType=' + (iPanel.eventFrame.systemId == 1 ? 1 : 2);
                        url += url.indexOf("?") > 0 ? '&' : '?';
                        url += 'backURL=';
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-06-29.jpg') no-repeat;" onUnload="exit();"></body>
</html>