...
<%
    String typeId = "10000100000000090000000000106800";

    String[] types = null,backgrounds = null;
    int[] position = {};
    //TODO:每次需要修改
    ...
    try {
        ...
        List<Column> columns = getTypeList(metaHelper, typeId, 3,0);
        types = new String[columns.size()];
        position = new int[columns.size() * 2];
        backgrounds = new String[columns.size()];
        for( int i = 0; i< columns.size(); i++){
            Column column = columns.get(i);
            types[i] = column.getId();
            position[i * 2] = 90;
            position[i * 2 + 1 ] = 0;
            String picture = Utils.pictureUrl( "", column.getPosters(), "7", 0, request);
            backgrounds[i] = StringUtils.isEmpty(picture) ? "" : picture;
        }
...
<head>
    <title>超强大片轮番轰炸</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2016-10-25.png') no-repeat;background-position: 0px 0px;}
        .arrow{width:41px;height:41px;top:532px;}
        .arrowLeft{left:23px;background-position:-400px -100px;}
        .arrowRight{left:926px;background-position:-450px -100px;}

        .monthBg {width:405px;height:53px;left:81px;top:328px;background-position: 0px 0px;}
        .monthText {width:383px;height:58px;left:84px;top:315px;background-position: 0px -100px;}

        .month {width:124px;height:53px;top:328px;background-position:-450px 0px}
        .month1 {left:81px;}
        .month2 {left:222px;}
        .month3 {left:362px;}

        .flowed {width:850px;height:300px;left:72px;top:395px;overflow:hidden;position:absolute;}
        .flowedItemContainer {position:relative;float:left;width:208px;height:300px;overflow: hidden;}
        .border,.focus {width:187px;height:258px;top:0px;left:11px;}
        .border{background-position:0px -160px;}
        .focus{background-position:-300px -160px;}
        .text {width:181px;height:23px;position:absolute;left:14px;top:263px;font-size:22px; color: white;text-align: center;}
        .image {width:181px;height:252px;left:14px;top:3px;position:absolute;}
        .image img {width:181px;height:252px;border:none;}
        .after {}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,items : [
                {name:'10月',style:'mask month1'},
                {name:'11月',style:'mask month2'},
                {name:'12月',style:'mask month3'}
            ]},
            <%
            value = "";
            if( list != null && list.size() > 0){
                ...
                for(int i = 0; i < list.size() ; i++){
                    value += "{focus:0,bg:'" + backgrounds[i] + "',typeId:'" + types[i] + "',items:[";
                    List<Vod> vodList = list.get(i);
                    for( int j = 0; vodList != null && j < vodList.size(); j++ ){
                        Vod vod = vodList.get(j);
                        Matcher matcher = pattern.matcher( vod.getName() );
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && ( index == 11 || focus == 0 && index == -1 || focus + 1 >= focusable[blocked].items.length && index == 1 ) ||
                        blocked >= 1 && blocked <= 3 && ( index == -11 || focus == 0 && index == -1 ) ) return;
                    if( index == 1 || index == -1 ) {
                        if( blocked >= 1 && blocked <= 3 && index == 1 && focus + 1 >= focusable[blocked].items.length ){
                            focusable[focusable.length - 1].from = blocked;
                            blocked = focusable.length - 1;
                            focus = 0;
                        } else if ( blocked == focusable.length - 1 && index == -1){
                            blocked = focusable[focusable.length - 1].from;
                            focus = focusable[blocked].focus;
                        } else focus += index;
                    } else if( index == -11 && blocked == 0 ){
                        blocked = focusable[0].focus + 1;
                        focus = focusable[blocked].focus;
                    } else if( index == 11 && blocked >= 1 && blocked <= 3 ){
            ...
            document.body.style.backgroundImage = "url('" + focusable[focusable[0].focus + 1].bg + "')";
            $("monthMask").className = "mask month month" + ( focusable[0].focus + 1 );
            flowedShow( focusable[0].focus + 1 );

            if( blocked == focusable.length - 1 ) {
                $("mask").className = focusable[blocked].items[focus].style;
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                var item = items[i];
                html += "<div class='flowedItemContainer'>";
                html += "<div class='image'>"
                html += "<img src='" + item.picture + "'/>";
                html += "</div>";
                html += "<div class='mask " + (  i == focus && blocked == index ? "focus" : "border" ) + "'></div>";
            ...
            flowCursorIndex += pageCount;
            if( items.length > flowCursorIndex ) {
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:undefined,popup:undefined,vote:undefined,video:undefined,position:undefined,external:{after:'<div id="monthMask"></div><div id="arrowLeft"></div><div id="arrowRight"></div><div class="mask monthText"></div>',before:'<div class="backNormal"></div><div class="mask monthBg"></div><div id="flowed" class="flowed"></div>'}});
            if( blocked == 0 ) blocked = new Date().getMonth() - 10 + 2;
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black none no-repeat;" onUnload="exit();"></body>
</html>