...
<%
    ...
    final String[] types = {
            "10000100000000090000000000105268",
            "10000100000000090000000000105269",
            "10000100000000090000000000105270",
            "10000100000000090000000000105271",
            "10000100000000090000000000105272",
            "10000100000000090000000000105273",
            "10000100000000090000000000105274"
    };
    final int[] position = {5,0,5,0,5,0,5,0,5,0,5,0,5,0};
    //TODO:每次需要修改
    ...
    try {
        ...
        list = new ArrayList<List<Column>>();
        int i = 0;
        for( String type : types ){
            List<Column> ls = getTypeList(metaHelper, type, position[i], position[i +1]);
            list.add( ls );
            i+=2;
        }
    } catch (Exception e){}
...
<head>
    ...
    <style>
        ...
        .flowedContainer{width:1280px;height: 401px;left:0px;top:196px;position: relative;}
        ...
        .flowedItem,.flowedItemFocused {height: 40px;line-height: 32px;width: 152px;overflow: hidden;white-space: nowrap; font-size: 20px;color: #ffffff;text-align: center;}
        .flowedItemFocused {background:transparent url('images/mask-2015-06-11.png') no-repeat -20px -2px; }
        ...
        .popuped .item{width:542px;font-size: 24px; height: 51px;padding-left: 56px; color: #313131; position:static;line-height:45px; background:transparent url('images/popup.png') no-repeat 0px -426px; overflow: hidden;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            ...
            if( list != null && list.size() > 0){
                for(int i = 0; i < list.size() ; i++){
                    ...
                    List<Column> vodList = list.get(i);
                    for( int j = 0; j < vodList.size() ; j++ ){
                        Column column = vodList.get(j);
                        String str = "{ mid:'" + column.getId() + "'," +
                              ...
                              "link:\"/EPG/jsp/defaultHD/en/iCatch_yulq_list.jsp?typeId=" + column.getId() + "&pageLength=8\"" + "," +
                     ( j == 0 ? "picture:'" + Utils.pictureUrl("images/default_preview_lb.png", column.getPosters(), "1", request)+ "'" + "," : "") +
                               "playType:-3";
                            str += "}" ;
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == focusable.length - 1 ) {
                        ...
                        if( index == 11 || index == -11 ) {
                            focus += index > 0 ? -1 : 1;
                            if( focus < 0 ) return;
                            else if( focus >= focusable[blocked].items.length ){
                                if( blocked < 6 ) return;
                            ...
                            previous = blocked;
                            blocked = previous + index;
                            if( blocked < 0 || blocked > 6 ) {blocked = previous; return;}
                            else {
                               flowedShow(previous);
                               focus = focusable[previous].focus;
        ...
        var flowedShow = function(index){
            ...
            for(var i = 0; i < length; i += 1) {
                ...
                if( current == focus && blocked == index ){
                    html += '<div class="flowedItemFocused">';
                    html += items[current].length > <%=popItemMaxCharLength%> ? ( '<marquee class="flowedMarquee">' + items[current].text + '</marquee>') : items[current].text;
                    html +=  '</div>';
                } else
                    html += '<div class="flowedItem">' + items[current].shortText + '</div>';
            }
            $("childrenContainer" + index ).innerHTML = html;
        }
        function init(){
            ...
            $.buildUserInterface = function(option){
                var html = "";
                html += "<div id='flowedContainer' class='flowedContainer'></div>";
            ...
            $.buildUserInterface({flowed:[0,1,2,3,4,5,6],popup:undefined,vote:undefined,video:undefined,position:undefined,external:{after:undefined,before:undefined}});
            if( focusable.length == 2 ) {blocked = focusable.length - 1;}
            html = '';
            for( var i = 0; i< 7; i++){
                html += '<div class="flowed" id="flowed' + i + '">';
                var floweds = focusable[i].items;
                var pic =  ( typeof floweds == 'object' && typeof floweds[0] == 'object' ) ? floweds[0].picture : '';
                html += '<img src="' + pic + '" style="width:152px;height:196px;" />';
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-06-11.jpg') no-repeat;" onUnload="exit();"></body>
</html>