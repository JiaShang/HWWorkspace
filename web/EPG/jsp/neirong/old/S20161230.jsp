...
<%
    ...
    final String[] types = {"10000100000000090000000000107102","10000100000000090000000000107103","10000100000000090000000000107104"};
    final int[] position = {99,0,99,0,99,0};
    //TODO:每次需要修改
...
<head>
    <title>1617跨年演唱会</title>
    <style>
        .mask {width:106px;height:45px;top:294px;position:absolute;background:transparent url('images/mask-2016-12-30.png') no-repeat;background-position: 0px 0px;}
        .mask1{left:94px;background-position: 0px 0px;}
        .mask2{left:209px;background-position: -110px 0px;}
        .mask3{width:155px;left:322px;background-position: -220px 0px;}
        .mask4{left:485px;background-position: 0px -50px;}
        .mask5{left:598px;background-position: -110px -50px;}

        .item {width:297px;height:49px;overflow: hidden;float: left;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;}
        .normal,.focus {background:transparent url('images/mask-2016-12-30.png') no-repeat;width:297px;height:29px;line-height: 27px;font-size:22px;color:#ffffff;}
        .normal {background-position: -20px -97px;}
        .focus {background-position: -20px -145px;}
        .text {width:220px;height:29px;line-height: 24px;font-size:22px;color:#ffffff;margin:0px 0px 0px 42px;}

        .flowed {width:600px;height:250px;position:absolute;left:125px;top:410px;}

        .arrow {width:41px;height:69px;top:471px;}
        .arrowLeft{left:70px;background-position:0px -190px;}
        .arrowRight{left:695px;background-position:-100px -190px;}
        .pageCount {width:180px;height:24px;line-height: 24px;font-size:20px;color:#6EE5FF;position:absolute;text-align: center;left:550px;top:657px;}
        .after {}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0, items : [
                {name:'嗨唱金曲',style:'mask mask1'},
                {name:'全场回放',style:'mask mask2'},
                {name:'更多巨星',style:'mask mask3'},
                {name:'HIFI音乐',style:'mask mask4',playType:-3,link:'http://192.168.34.81/HiFi_cq/wailiu/wl/HiFi_songList.html?albumId=wl_005&type=0'},
                {name:'彩虹音乐',style:'mask mask5',playType:-3,link:'http://192.168.48.217:8085/iptv/entrance.jsp?gpid=266'}
            ]},
            <%
            value = "";
        ...
        var columns = 2; rows = 4;              //定义行列，　几行几列
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    previous = focusable[0].focus >= 2 ? 3 : ( focusable[0].focus + 1 );

                    var currentPage = Math.ceil( (focus + 1.0) / pageCount );
                    var computeTotal = Math.ceil( (focusable[previous].items.length  * 1.0 ) / pageCount );

                    if( blocked == 0 && ( index == 11 || index == -1 && focus <= 0 || index == 1 && focus + 1 >= focusable[blocked].items.length ) ||
                        blocked >= 1 && blocked <= 3 && ( index == -1 && focus % 2 == 0 && currentPage <= 1 || index == 1 && currentPage >= computeTotal && (focus % 2 == 1 || focus % 2 == 0 && focus + 1 >= focusable[blocked].items.length ) || index == -11 && ( focus % 8 >= 6 || focus + columns >= focusable[blocked].items.length) )
                    ) return;

                    if( blocked == 0 ) {
                        if( index == -11 ) {
                            blocked = focusable[0].focus >= 2 ? 3 : ( focusable[0].focus + 1 );
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                var item = items[i];
                html += "<div class='item'>";
                html += "<div class='" + ( i == focus && blocked == index ? "focus" : "normal" ) + "'>";
                html += "<div class='text'>";
                html  += ( i == focus && blocked == index && item.text != item.shortText  ) ? ( "<marquee style='width:220px;line-height:24px;'>" + item.text + "</marquee>") : item.shortText;
                html += "</div></div>";
                html += "</div>";
            }
            ...
            var currentPage = Math.ceil( (focus + 1.0) / pageCount );
            var computeTotal = Math.ceil( (focusable[index].items.length  * 1.0 ) / pageCount );
            if( currentPage > 1 ) {
                $("arrowLeft").style.visibility = 'visible';
                $("arrowLeft").className = 'mask arrow arrowLeft';
            } else {
                $("arrowLeft").style.visibility = 'hidden';
            }
            if( currentPage < computeTotal ) {
                $("arrowRight").style.visibility = 'visible';
                $("arrowRight").className = 'mask arrow arrowRight';
            } else {
                $("arrowRight").style.visibility = 'hidden';
            }
            $("page").innerHTML = '第 ' + String(currentPage) + ' 页 / 共 ' + String(computeTotal)　+ ' 页';
        }
        ...
        function init(){
            ...
            $.buildUserInterface = function(option){
                ...
                html += "<div id='arrowLeft'></div><div id='arrowRight'></div>";
                html += "<div id='page' class='pageCount'></div>";
            ...
            $.buildUserInterface({flowed:[0,1,2],popup:undefined,vote:undefined,video:undefined,position:undefined,external:{after:undefined,before:undefined}}); //'<div class="after"></div>'
            ...
            if( !strFocus.isEmpty() )
            {
                ...
                focusable[3].focus = Number(strFocus[4]);
            } else {
                blocked = 1;
            }
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-12-30.jpg') no-repeat;" onUnload="exit();"></body>
</html>