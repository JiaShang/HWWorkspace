...
<%
    ...
    final String[] types = {"10000100000000090000000000107420"};
...
<head>
    <title>美女与野兽</title>
    <style>
        .mask {top:324px;width:145px;height:45px;position:absolute;background:transparent url('images/mask-2017-03-17.png') no-repeat;background-position: 0px 0px;}
        .mask1 {left:380px;background-position: 0px 0px;}
        .mask2 {left:528px;background-position: 0px -50px;}

        .item{ width:210px;height:285px;margin:0px 6px; float: left;}
        .itemContainer{width:200px;height:245px; position:relative;}
        .item .image,.item .maskImage{ width:200px;height:245px;left:0px;top:-230px;position:relative; background:transparent url('images/mask-TempRowYellow.png') no-repeat;}
        .item .image {background-position: -6px -10px;}
        .item .maskImage{background:transparent url('images/mask-TempRowYellow.png') no-repeat -6px -260px;}
        .text {width:190px;height:35px; font-size:22px; color:#FEE5f0; line-height:30px; position:relative; top:-225px; overflow: hidden;left:5px;text-align: center;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,items : [
                {name:'狂欢迪士尼',style:'mask mask1',playType:-3,link:'http://192.168.34.18:8081/activity/tpl_activity/beautybeast/index.do'},
                {name:'周边送不停',style:'mask mask2',playType:-3,link:'http://192.168.35.129:4567/CyberCloud_Dispatcher.htm?CyberEntryID=1&CyberSiteCode=CS00003&tag=i12'}
            ]},
            <%
            value = "";
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && !$.vote.tooltiped && blocked == focusable.length - 1 && focus == 1 ) focus = focus + 1;
                    ...
                    if( blocked == 0 && ( index == 11 || index == -1 && focus == 0 || index == 1 && focus >= 1 ) ||
                        blocked == 1 && ( index == -11 || index == -1 && focus <= 0 || index == 1 && focus + 1 >= focusable[blocked].items.length )) return;
                    if( index == -1 || index == 1 ){
                        focus += index;
                    } else if( index == -11 && blocked == 0 ){
                        blocked = 1;
                        focus = focusable[blocked].focus;
                    } else if( index == 11 && blocked == 1 ) {
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-03-17.jpg') no-repeat;" onUnload="exit();"></body>
</html>