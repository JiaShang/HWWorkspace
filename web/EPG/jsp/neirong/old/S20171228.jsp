...
<head>
    ...
    <script language="javascript" type="text/javascript">
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && !$.vote.tooltiped && blocked == focusable.length - 1 && focus == 1 ) focus = focus + 1;
                    ...
                    if( blocked == 0 && ( index == -11 || index == 11 || index == -1 && focus <= 0 || index == 1 && focus + 1 >= focusable[blocked].items.length )) return;
                    focus += index;
                }
        ...
        function init(){
            ...
            var links = [{text:'战狼2',length:<%= StringUtil.length("战狼2")%>,shortText:'<%= StringUtil.limitStringLength("战狼2", popItemMaxCharLength)%>',playType:-3,picture:'images/item-2017-12-28-1.jpg',link:'http://192.168.17.155/nn_cms/web_template/index.html?nns_page_name=movie_detail&nns_video_id=5a1e0da3ea29c36292cf2d8efc3114ba',link1:'com.hunantv.operator,com.starcor.hunan.SplashActivity,cmd_ex###show_video_detail###video_id###4c2d4141d50fceb284767be99ff32253###video_type###0###video_ui_style###0'},
                         {text:'空天猎',length:<%= StringUtil.length("空天猎")%>,shortText:'<%= StringUtil.limitStringLength("空天猎", popItemMaxCharLength)%>',playType:-3,picture:'images/item-2017-12-28-2.jpg',link:'http://192.168.17.155/nn_cms/web_template/index.html?nns_page_name=movie_detail&nns_video_id=5a1541c6e4cbee488c8501ec704deb9d',link1:'com.hunantv.operator,com.starcor.hunan.SplashActivity,cmd_ex###show_video_detail###video_id###E1934320###video_type###0###video_ui_style###0'},
                         {text:'猩球崛起3',length:<%= StringUtil.length("猩球崛起3")%>,shortText:'<%= StringUtil.limitStringLength("猩球崛起3", popItemMaxCharLength)%>',playType:-3,picture:'images/item-2017-12-28-3.jpg',link:'http://192.168.17.155/nn_cms/web_template/index.html?nns_page_name=movie_detail&nns_video_id=43b129bdfdf90b05b83a6f19172a3618',link1:'com.hunantv.operator,com.starcor.hunan.SplashActivity,cmd_ex###show_video_detail###video_id###bc52e23c839214e30d81273af028795f###video_type###0###video_ui_style###0'},
                         {text:'极盗车神',length:<%= StringUtil.length("极盗车神")%>,shortText:'<%= StringUtil.limitStringLength("极盗车神", popItemMaxCharLength)%>',playType:-3,picture:'images/item-2017-12-28-4.jpg',link:'http://192.168.42.52/epg/hollywood/biz_76826310/category/ccms_category_37547342/item/vod/62108.do'},
                         {text:'神奇女侠',length:<%= StringUtil.length("神奇女侠")%>,shortText:'<%= StringUtil.limitStringLength("神奇女侠", popItemMaxCharLength)%>',playType:-3,picture:'images/item-2017-12-28-5.jpg',link:'http://192.168.17.155/nn_cms/web_template/index.html?nns_page_name=movie_detail&nns_video_id=5a0ea0de21dacf263c859d4a913b36a0',link1:'com.hunantv.operator,com.starcor.hunan.SplashActivity,cmd_ex###show_video_detail###video_id###bf49688b06ad98a5da88a6c6d9b05b0f###video_type###0###video_ui_style###0'}];

            focusable[0].items.insertAt(1,links[0]);
            focusable[0].items.insertAt(2,links[1]);
            focusable[0].items.insertAt(3,links[2]);
            focusable[0].items.insertAt(5,links[3]);
            focusable[0].items.insertAt(6,links[4]);
