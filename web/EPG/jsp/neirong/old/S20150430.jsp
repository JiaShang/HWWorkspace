...
<%
    ...
    final String[] types = {"10000100000000090000000000105085","10000100000000090000000000105085"};
    final int[] position = {6,0,6,0};
    //TODO:每次需要修改
...
<head>
    <title>五月观影指南</title>
    <style>
        .mask,.maskVote {width:200px;height: 268px;position:absolute;background:transparent url('images/mask-2015-04-30.png') no-repeat fixed;background-position: 0px 0px;}
        .mask1 {left:643px; top:68px;}
        .mask2 {left:836px; top:68px;}
        .mask3 {left:1028px; top:68px;}
        .mask4 {left:464px; top:362px;}
        .mask5 {left:657px; top:362px;}
        .mask6 {left:849px; top:362px;}

        .maskVote{width:51px;height:51px;background-position: 0px -300px;}
        .maskVote1{left:718px; top:303px;}
        .maskVote2{left:910px; top:303px;}
        .maskVote3{left:1102px; top:303px;}
        .maskVote4{left:539px; top:596px;}
        .maskVote5{left:731px; top:596px;}
        .maskVote6{left:923px; top:596px;}

        .flowed {position: absolute;left:113px;top:281px;width:1390px;height: 362px;}
        .itemText,.maskText {width:478px;height: 60px;font-size:24px;font-weight: bold; color:#ffffff;overflow: hidden; float:left; padding: 15px;word-wrap: normal;white-space:nowrap; }
        .maskText{background:transparent url('images/mask-2015-04-30.png') no-repeat fixed 0px 0px;}
        .maskMarquee {width:455px; height: 60px;font-size:24px;font-weight: bold; color:#ffffff;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            <%
            value = "";
            final String[][] actions = {"[0,0,1,0,0,0,0,1]|[0,1,1,1,0,0,0,2]|[0,2,1,2,0,1,0,2]|[0,3,1,3,0,3,0,4]|[1,0,1,4,0,3,0,5]|[1,1,1,5,0,4,-1,0]".split("\\|"),"[0,0,0,4,1,0,1,1]|[0,1,0,5,1,0,1,2]|[0,2,-1,0,1,1,1,2]|[0,3,1,3,1,3,1,4]|[0,4,1,4,1,3,1,5]|[0,5,1,5,1,4,-1,0]".split("\\|")};
                    ...
                    for( int j = 0; j < vodList.size() ; j++ ){
                        ...
                        String str = "{ mid:'" + vod.getId() + "'," +
                              ...
                              //"shortText:\"" + StringUtil.limitStringLength(vod.getName(), popItemMaxCharLength) + "\"" + "," +
                                  "style:'" + (i == 0 ? "mask mask" : "maskVote maskVote") + (j + 1) + "'" + "," +
                                 "action:" + actions[i][j] + "," +
                               "playType:" + ( i == 0 ? vod.getIsSitcom() : -2 ) ;
                            str += "}" ;
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked <= 1) {
                        ...
                        } else { blocked = item.action[6]; focus = item.action[7]; }
                        if( blocked == -1 ) { consign = temp; blocked = focusable.length - 1;}
                        if( blocked === focusable.length - 1 ) { flowedShow(0);flowedShow(1); }
                    } else if( blocked == focusable.length - 1) {
                        blocked = consign;
                        focus = index == -1 ? focusable[blocked].items.length - 1 : 2;
                    }
                    if( temp != blocked ) { flowedShow(0); flowedShow(1);}
                    else flowedShow(blocked);
                }
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:undefined,popup:undefined,vote:[{blocked:1,item:undefined,id: 208}],video:undefined,position:undefined,external:{after:undefined,before:'<div style="width:614px;height:344px;position: absolute;left: 539px;top:303px;background:transparent url(images/mask-2015-04-30.png) no-repeat fixed -539px -303px;"></div>'}});
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-04-30.jpg') no-repeat;" onUnload="exit();"></body>
</html>