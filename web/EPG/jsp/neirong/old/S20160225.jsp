...
<%
    ...
    final String[] types = {"10000100000000090000000000105875"};
...
<head>
    <title>一横排列表专题带投票模板</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2016-02-25.png') no-repeat fixed;background-position: 0px 0px;}
        .normal1 {width:300px;height:104px;left:67px;top:167px;background-position:0px 0px;}
        .mask1 {width:217px;height:104px;left:67px;top:167px;background-position:0px -110px;}
        .item{ width:212px;height:291px;margin:0px 6px; float: left;}
        .itemContainer{width:212px;height:291px; position:relative;}
        .itemImage{position:relative;width:180px;height: 263px; left:11px; top:6px;}
        .backNormal{position:absolute;width:251px;height:52px;left:1003px;top:629px;background:transparent url('images/mask-Template3.png') no-repeat fixed -200px -400px;}
        .item .image,.item .maskImage{ width:212px;height:291px;left:0px;top:-263px;position:relative; background:transparent url('images/mask-Template3.png') no-repeat fixed; }
        .item .image {background-position: 0px 0px;}
        .item .maskImage{background-position: -250px 0px;}
        ...
        .ruleNormal,.maskRole {width:197px;height:65px;left:942px;top:206px;position:absolute;}
        .ruleNormal {background:transparent url('images/mask-Template3.png') no-repeat fixed -250px -300px;}
        .maskRole {background:transparent url('images/mask-Template3.png') no-repeat fixed 0px -300px; background-position: 0px -300px;}

        .vote,.maskVote {width:82px;height:38px;position:relative; left:60px;top:-260px;background:transparent url('images/mask-Template3.png') no-repeat fixed 0px -400px;}
        .maskVote {background-position: -100px -400px;}

        .flowed {width:1200px;left:75px;top:275px;height:350px;overflow:hidden;position: absolute;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,items : [
                {name:'',style:'mask mask1',playType:-3,link:'http://192.168.35.129:4567/CyberCloud_Dispatcher.htm?CyberAppID=130&CyberEntryID=1 '},
                {name:'活动细则',style:'mask maskRole',playType:-3,link:'/EPG/jsp/neirong/STemplate3Rule.jsp?typeId=<%= typeId%>'}
            ]},
            <%
            value = "";
                ...
                for(int i = 0; i < list.size() ; i++){
                    ...
                    for( int j = 0; j < vodList.size() ; j++ ){
                        ...
                        String str = "{ mid:'" + vod.getId() + "'," +
                            ...
                            str += "}" ;
                        str += ",{text:'" + vod.getName() + "',playType:-2}";
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && (index == 11 || index == -1 && focus == 0 || index == 1 && focus == 1 ) ||
                        blocked == 1 && ( focus % 2 == 1 && focus % pageCount != ( pageCount - 1 ) && index == -11 || focus == 0 && index == -1 )) return;
                    if( blocked == 0 ) {
                        if( index == 1 || index == -1 ) { focus += index; }
                        else {
                            blocked = 1;
                            focus = focusable[blocked].focus;
                            focus = pageCount * ( Math.floor( focus / pageCount) );
                            if( focusable[0].focus == 1 ){
                                focus += pageCount - 2;
                                if( focus >= focusable[blocked].items.length )
                                   focus =  focusable[blocked].items.length - 2;
                            }
                        }
                    } else if( blocked == 1 ){
                        if( index == 11 ) {
                            if( focus % 2 == 0 ) { blocked = 0; focus = focus % pageCount == 0 ? 0 : 1 ; }
                            else { focus -= 1 }
                        } else if( index == -11 ){
                            if( focus % pageCount == ( pageCount - 1 ) ) { blocked = focusable.length - 1; focus = 0;}
                            else {focus += 1};
                        }
                        else {
                            focus += index * 2;
            ...
            $("mask").style.visibility = blocked == 1 ? "hidden" : "visible";
            flowedShow(1);
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += rows) {
                var item = items[i];
                html += '<div class="item"><div class="itemContainer">';
                html += '<img src="' + item.picture + '" class="itemImage"/>'
                html += '<div class="' + ( ( i == focus && blocked == index ) ? 'maskImage' : 'image' ) + '"></div>';
                html += '<div class="' + ( ( i + 1 == focus && blocked == index ) ? 'maskVote' : 'vote' ) + '"></div>';
                html +='</div></div>';
            }
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:[1],popup:undefined,vote:[{blocked:1,item:undefined,id: <%= StringUtils.isEmpty(voteId) ? "-1" : voteId %>}],video:undefined,position:undefined,gatewayCheck:undefined,external:{after:undefined,before:'<div class="mask normal1"></div><div class="ruleNormal"></div><div class="backNormal"></div>'}});
