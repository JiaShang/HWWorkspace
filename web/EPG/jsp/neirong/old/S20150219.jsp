...
<%
    ...
    final String[] types = {"10000100000000090000000000104935","10000100000000090000000000104936","10000100000000090000000000104936","10000100000000090000000000104937"};
    final int[] position = {100,0,1,0,100,1,100,0};
    //TODO:每次需要修改
...
<style>
    .mask {position:absolute;background:transparent url('images/mask-2015-02-19.png') no-repeat fixed;background-position: 0px 0px;}
    .maskRole {width:133px;height:120px;left:804px;top:215px;background-position: 0px 0px;}
    .maskComplete {width:412px;height:74px;left:104px;top:618px;background-position: 0px -150px;}
    .maskEach{width:239px;height:70px;left:517px;top:620px;background-position: 0px -230px;}
    .maskFunny{width:220px;height:74px;left:759px;top:618px;background-position: 0px -300px;}

    .flowed {position: absolute;left:207px;top:366px;width:900px;height: 370px;}
    .itemText,.maskText,.itemVote,.maskVote,.maskMarquee {width:348px;height: 53px;margin: 13px 0px 17px 12px;font-size:23px;font-weight: bold; color:#8400E9;overflow: hidden; float:left;}
    .maskText,.maskVote,.maskMarquee {color:#e80d0d;}
    .maskMarquee {width:346px; margin:0px 0px 0px 13px;}
    .itemVote,.maskVote{width:60px;}
...
<script language="javascript" type="text/javascript">
    ...
    function focused(index,initilized){
        ...
        if(typeof index == "number" && !initilized){
            ...
            if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                ...
                if( blocked == 0 && index != -11 || blocked == 2 && ( focus == 0 && index == -1 || index == -11 ) ||
                    blocked == 1 && ( startedFlowed < 16 && focus % 4 == 0 && index == -1 || ( focus % 4 == 3 && startedFlowed + 16 > focusable[blocked].length || focus == focusable[blocked].items.length -1) && index == 1 ) ) return;
                if( blocked == 0 || blocked == 2 && index == 11 ) {
                    blocked = 1 ;focus = focusable[blocked].focus;
                    if(index == 11 && focus == 0) {
                        if(focus = focusable[blocked].items.length > 12 ) focus = 12;
                        else if( focusable[blocked].items.length > 2) focus = focusable[blocked].items.length - 2;
                        else blocked = focus = 0;
                    }
                }
                else if( blocked == 2 ) {
                    focus += index;
                ...
                else  if( blocked == focusable.length - 1) { blocked = 2; focus = focusable[blocked].focus; }
                else if( blocked == 1 ){
                    if( index == 1 ) {
                        //startedFlowed
                        if(focus % 4 == 3  && index == 1){
                            if(focus + 13 < focusable[blocked].items.length ) focus += 13;
                            else focus = startedFlowed + 16;
                        } else  focus += 1;
                    }
                    else if( index == -1 ) focus += (focus % 4 == 0 && focus >= 16 ? -13 : -1);
                    else if( index == 11 ) {
                        if( focus <  startedFlowed + 4  ) { blocked = 0; focus = focusable[blocked].focus;  flowedShow();}
                        else focus -= 4;
                    }
                    else if( index == -11 ){
                        if(  focus + 4 >= startedFlowed + 16 || focus + 4 >= focusable[blocked].items.length ) { blocked = 2; focus = focusable[blocked].focus;flowedShow(); }
                        else focus += 4;
                    }
                }
            }
            ...
            if( blocked != 1 && blocked != focusable.length -2 ) flowedShow();
        }

        if(blocked == 1 && $("mask").style.visibility != "hidden")  $("mask").style.visibility = "hidden";
        else if (( blocked == 0 || blocked == 2 || blocked == focusable.length - 1) && $("mask").style.visibility != "visible")
    ...
    var flowedShow = function(){
        ...
        startedFlowed = Math.floor(focus / 16.0) * 16;
        var html = '';
        var leng = items.length - startedFlowed >= 16 ? 16 : items.length - startedFlowed;
        for(var i = 0; i < leng; i+=2) {
            var current = startedFlowed + i;
            if( current == focus && blocked == 1  ){
                html += '<div class="maskText">';
                html += items[current].length > 28 ? ( '<marquee class="maskMarquee">' + items[current].text + '</marquee>') : items[current].text;
                html +=  '</div>';
            } else {
                html += '<div class="itemText">' + items[current].shortText + '</div>';
            }

            html += '<div class="' + (current + 1 == focus && blocked == 1 ? "maskVote":"itemVote" ) + '">投票</div>';
        }
    ...
    function init(){
        ...
        $.buildUserInterface({flowed:[1],popup:[{blocked:2,item:1,target:3,style:'mask maskEach'},{blocked:2,item:2,target:4,style:'mask maskFunny'}],vote:[{blocked:1,item:undefined,id: 197}],video:undefined,position:undefined,external:{after:undefined,before:undefined}});
        if( focusable.length == 2 ) {blocked = focusable.length - 1;}

        //这句下次要删除
        focusable[2].items[0].style = "mask maskComplete";
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-02-19.jpg') no-repeat;" onUnload="exit();"></body>
</html>