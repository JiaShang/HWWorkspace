...
<%
    String typeId = "10000100000000090000000000108403";

    //TODO:每次需要修改
    int focused0 = 0,focused1 =0,focused2 = 0, area = 0;
    final int popItemMaxCharLength = 40;
    ...
    try {
        ...
        if(null != focus ){
            ...
            if( focus.length > 1 && focus[1] != null )
                focused0 = Integer.parseInt(focus[1]);
            if( focus.length > 2 && focus[2] != null )
                focused1 = Integer.parseInt(focus[2]);
            if( focus.length > 2 && focus[2] != null )
                focused1 = Integer.parseInt(focus[2]);
        }
        ...
        columns = getTypeList(metaHelper,typeId,99, 0 );
        if( columns != null && columns.size() > 0 ) {
            list = new ArrayList<List<Vod>>();
            typeId = columns.get(columns.size() - 2).getId();
            list.add( getVodList( metaHelper, typeId, 99 , 0 ) );
            typeId = columns.get(columns.size() - 1).getId();
            list.add( getVodList( metaHelper, typeId, 99 , 0 ) );
...
<head>
    ...
    <style>
        ...
        .flowedTypeList {width:1150px;height:110px;position: absolute;left:77px;top:272px; overflow: hidden;}

        .columnItem {width:574px;height:52px;overflow: hidden;float: left;}
        .columnItemSel{width:549px;height:42px;background-color:#f8ea05;}
        .columnItemContainer {width:509px;height:42px;line-height: 36px;font-size: 24px;color:#bd0415;margin: 0px 20px 0px 20px;overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;}
        .marqueeTypeList{width:509px;line-height: 36px;}

        .flowedVodList {width:540px;height:220px;position:absolute;top:420px;overflow: hidden;}
        .flowedVodList1{left:80px;}
        .flowedVodList2 {left:657px;}
        .vodItem{width:540px;height:44px;overflow: hidden;float: left;background:transparent url('images/mask-2017-09-22.png') no-repeat;background-position: 0px -239px;}
        .vodItemContainer {width:450px;height:43px; margin:0px 0px 0px 65px;line-height: 36px;color:white; font-size: 22px;overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;}
        .marqueeItemList {width:450px;line-height: 36px;color:black;}
        .voteItemFocus1{left:43px;}
        .voteItemFocus2{left:273px;}
        .voteItemFocus3{left:503px;}
        .voteItemFocus4{left:733px;}
        .voteItemFocus5{left:963px}

        .vodItemFocus{background-position: 0px -285px; color:black;}

        .arrowLeftTypeList{width:16px;height:37px;left:37px;top:305px;background-position: -360px 0px;}
        .arrowRightTypeList{width:16px;height:37px;left:1215px;top:305px;background-position: -390px 0px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
           <%
            value = "";
            if( columns != null && columns.size() > 0 ) {
                value += "{focus:" + focused0 +",items:[";
                for( int i = 0; i < columns.size() ; i++ ){
                    Column col = columns.get(i);
                    String str = "{" +
                               "text:\"" + col.getName() + "\"," +
                          "shortText:\"" + StringUtil.limitStringLength(col.getName(), popItemMaxCharLength) + "\"" + "," +
                             "playType:-3," +
                             "link:'/EPG/jsp/neirong/S20170922List.jsp?typeId="+ col.getId() + "'";
                        str += "}" ;
                        str += ( i + 1 < columns.size()  ? "," : "");
                    value += str;
                }
                value += "]},";
            }
            if( list != null && list.size() > 0){
                for(int i = 0; i < list.size() ; i++){
                    value += "{focus:" + ( i == 0 ? focused1 : focused2) + ",typeId:'" + typeId + "',items:[";
                    ...
                    for( int j = 0; j < vodList.size() ; j++ ){
                        ...
                        String str = "{ mid:'" + vod.getId() + "'," +
                              ...
                              "shortText:\"" + StringUtil.limitStringLength(vod.getName(), 40) + "\"" + "," +
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && !$.vote.tooltiped && blocked == focusable.length - 1 && focus == 1 ) focus = focus + 1;
                    ...
                    var pageCount = blocked == 0 ? 4 : 5;
                    if( blocked == 0 && ( index == -1 && ( focus == 0 || focus == 2 ) || index == 11 && focus % 4 <= 1 || index == 1 && ( focus + 1 >= focusable[blocked].items.length || focus % 4 == 1 && focus + 3 >= focusable[blocked].items.length )  ) ||
                        blocked == 1 && ( index == -1 || index == -11 && focus + 1 >= focusable[blocked].items.length ) ||
                        blocked == 2 && ( index == 1 || index == -11 && focus + 1 >= focusable[blocked].items.length )) return;
                    if( blocked == 1 || blocked == 2) {
                        if( index == 11 || index == -11 ) {
                            if( index == 11 && focus == 0 ) {
                                focusable[0].from = blocked;
                                blocked = 0; focus = focusable[0].focus;
                            } else {
                                focus += index > 0 ? -1 : 1;
                            }
                        } else {
                            blocked = index == 1 ? 2 : 1; focus = focusable[blocked].focus;
                        }
                    } else {
                        if( index == 1 ) {
                            focus += focus % 2 == 0 ? 1 : 3;
                            ...
                            var from = typeof focusable[0].from === 'undefined' ? 1 : focusable[0].from;
                            if( focus % 4 <= 1 ) {
                                focus += 2;
                                if( focus >= focusable[blocked].items.length ) {
                                    blocked = from; focus = focusable[blocked].focus;
                                }
                            }  else {
                                blocked = from; focus = focusable[blocked].focus;
                            }
        ...
        var flowedShow = function(index){
            ...
            if( index == 0 ) {
                ...
                for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                    var item = items[i];
                    html += '<div class="columnItem">';
                    if(blocked == index && focus == i){
                        html += '<div class="columnItemSel"><div class="columnItemContainer">' ;
                        if(item.shortText != item.text){
                            html += "<marquee scrolldelay='75'  class='marqueeTypeList'>" + item.text + "</marquee>";
                        } else
                            html += item.text;
                        html += "</div></div>"
                    } else {
                        html += '<div class="columnItemContainer">' + item.shortText + "</div>";
                    }
                    html += '</div>';
                }
                $("flowedTypeList").innerHTML = html;

                var arrowLeft = 'arrowLeftTypeList';
                var arrowRight = 'arrowRightTypeList';
                if( flowCursorIndex + pageCount < items.length ){
                    $( arrowRight ).style.visibility = 'visible';
                    $( arrowRight ).className = 'mask ' + arrowRight;
                } else
                    $( arrowRight ).style.visibility = 'hidden';

                if( flowCursorIndex >= pageCount){
                    $( arrowLeft ).style.visibility = 'visible';
                    $( arrowLeft ).className = 'mask ' + arrowLeft;
                } else
                ...
                for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                    var item = items[i];
                    if(blocked == index && focus == i) {
                        html += '<div class="vodItem vodItemFocus">';
                        html += '<div class="vodItemContainer">';
                        if(item.shortText != item.text){
                            html += "<marquee scrolldelay='75'  class='marqueeItemList'>" + item.text + "</marquee>";
                        } else
                            html += item.text;
                        html += "</div></div>";
                    } else {
                        html += '<div class="vodItem">';
                        html += '<div class="vodItemContainer">';
                        html += item.shortText;
                        html += "</div></div>";
                    }
                }
                $("flowedVodList" + index ).innerHTML = html;
            }
        }
        ...
        function init(){
            ...
            $.vote.focus = {}
            $.current = function (){
                return "/EPG/jsp/defaultHD/en/SaveCurrFocus.jsp?currFoucs=" + blocked + "," + focusable[0].focus + "," +  focusable[1].focus + "," +focusable[2].focus + "&url=";
            };
            $.buildUserInterface = function(option){
                ...
                if(typeof option.flowed != 'undefined' ) {
                    html += "<div id='flowedTypeList' class='flowedTypeList'></div>";
                    html += "<div id='flowedVodList1' class='flowedVodList flowedVodList1'></div>";
                    html += "<div id='flowedVodList2' class='flowedVodList flowedVodList2'></div>";
                    focusable.flowed = [];
                    for(var i = 0; i< option.flowed.length; i++){ focusable[option.flowed[i]].flowed = true; focusable.flowed[i] = option.flowed[i];}
                }
                html += "<div id='arrowLeftTypeList'></div><div id='arrowRightTypeList'></div><div id='arrowLeftVodList'></div><div id='arrowRightVodList'></div>";
            ...
            $.buildUserInterface({flowed:[0,1,2],vote:undefined,video:undefined,position:undefined,external:{after:undefined,before:undefined}}); //'<div class="after"></div>'
            focused(<%= area == 0 ? focused0 : focused1 %>, true);
        }
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-09-22.jpg') no-repeat;" onUnload="exit();"></body>
</html>