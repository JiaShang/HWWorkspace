...
<%
    ...
    final String[] types = {"10000100000000090000000000108140"};
...
<head>
    ...
    <style>
        .mask {background:transparent url('images/mask-2017-08-09.png') no-repeat;background-position: 0px 0px;}
        .itemContainer {width:188px;height:45px;float:left;overflow: hidden;float:left;}
        .flowed {width:380px;height:95px;position:absolute;top:605px;left:99px;overflow: hidden;}
        .icon,.iconFocus,.text{width:30px;height: 25px;float:left;}
        .icon{background-position: -10px -10px;}
        .iconFocus{background-position: -10px -50px;}
        .text,.textFocus{width:140px;height: 25px;height: 25px;line-height: 25px;color:white;font-size: 18px;overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;}
        .textFocus {color:#85141a;}
        .marquee{line-height: 25px;}

        .txt {position:absolute;width:296px;height:44px;line-height: 20px;color:white;left:845px; text-align: left;overflow:hidden;text-overflow:ellipsis;}
        .text1 {width:354px;height:62px;color:black;top:135px}
        .text2 {top:218px;}
        .text3 {top:278px;}
        .text4 {top:338px;}
        .text5 {top:408px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var columns = 2; rows = 2;              //定义行列，　几行几列
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                ...
                if( i == focus && blocked == index ){
                    html += "<div class='mask iconFocus'></div>";
                    html += "<div class='textFocus'>" + (item.shortText != item.text ? ("<marquee class='mqrquee'>" + item.text + "</marquee>") : item.text) + "</div>";
                } else {
                    html += "<div class='mask icon'></div>";
                    html += "<div class='text'>" + item.shortText + "</div>";
                }
                html += "</div>";
            }
        ...
        var ansk = '<%=column != null ? column.getIntroduce() : "你们三个人，最新单曲有在准备吗？"%>'.split(":");

        var showAnsks = function( init ){
            var first = ansk[0];
            for( var i = 0; i < ansk.length - 1; i ++ )
                ansk[i] = ansk[i + 1];
            ansk[ansk.length - 1] = first;
            for( var i= 0; i < 5; i ++ ) {
                $("text" + (i + 1)).innerText = ansk[i];
            }
            setTimeout("showAnsks()",3000);
        };
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:[0],vote:undefined,video:undefined,position:undefined,external:{after:'<div id="text1" class="txt text1"></div><div id="textAnt" class="textAnt"></div><div id="text2" class="txt text2"></div><div id="text3" class="txt text3"></div><div id="text4" class="txt text4"></div><div id="text5" class="txt text5"></div>',before:undefined}}); //
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-08-09.jpg') no-repeat;" onUnload="exit();"></body>
</html>