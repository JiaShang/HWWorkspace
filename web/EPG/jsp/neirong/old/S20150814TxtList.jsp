...
<%
    ...
    final int popItemMaxCharLength = 46;
...
<head>
    ...
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2015-08-14-TxtList.png') no-repeat fixed;background-position: 0px 0px;}

        .flowed {width:684px;height:415px;left:280px;top:136px;position: absolute;}
        .item,.itemFocus {width:682px; height:51px; font-size: 24px;color:#656363;line-height: 51px; padding:0px 30px 0px 58px;font-weight: bold;overflow: hidden; word-break:keep-all;white-space:nowrap;}
        .itemFocus{color: #ffffff; background:transparent url('images/mask-2015-08-14-TxtList.png') no-repeat fixed 0px 0px; }
        ...
        .pageUp,.pageDown {width:78px;height:39px;top:558px;left:855px;background-position: 0px -60px;}
        .pageDown {left:928px;background-position: -100px -60px;}
        .pageSize {width:100px; height:20px;font-size:20px;color:#ffffff;position: absolute;left:729px;top:564px; text-align: right;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && ( index == 1 || index == -1 || focus % 8 == 0 && index == 11 ) || blocked == 1 && ( index == -1 && focus == 0 || index == 1 && focus == 1)) return;
                    if( blocked == 0 ) {
                        if( ( focus % 8 == 7 || focus + 1 >= focusable[0].items.length ) && index == -11 ){
                            if( focus + 1 >= focusable[0].items.length && focusable[0].items.length <= 8 ) {
                                blocked = focusable.length - 1; focus = 1;
                            } else {
                                blocked = 1; focus =  0;
                            }
                            flowedShow(0);
                        ...
                        if( index == 1 ) focus = 1;
                        else if( index == -1 ) focus = 0;
                        if( index == 11 ) { blocked = 0; focus = focusable[blocked].focus; flowedShow(0); }
                        else if( index == -11 ) { blocked = focusable.length - 1; focus = 1; }
                    } else if ( blocked == focusable.length - 1 ) {
                        if( focusable[0].items.length <= 8 ) {
                            blocked = 0; focus = focusable[0].items.length - 1;
                        } else {
                            blocked = 1; focus = focusable[1].focus;
                        }
                    }
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                ...
                if( i == focus && blocked == index )
                    html += "itemFocus'>" + item.shortText;
                else
                    html += "item'>" + item.shortText;
                html += "</div>"
            }
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:[0],popup:undefined,vote:undefined,video:undefined,position:undefined,external:{after:'<div class="Title" id="Title"><%= column.getName() %></div><div class="pageSize" id="pageSize"></div>',before:undefined}});
        ...
        function doEnter(){
            ...
            if( blocked == 1 ) {
                ...
                if( focusable[blocked].focus == 0 ) {
                    if( focus - 8 >= 0 ) focus -= 8;
                } else if( focusable[blocked].focus == 1 ){
                    if( focus + 1 < focusable[0].items.length ) focus += 8;
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-08-14-TxtList.jpg') no-repeat;" onUnload="exit();"></body>
</html>