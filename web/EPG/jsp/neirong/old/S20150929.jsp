...
<%
    ...
    final String[] types = {"10000100000000090000000000105562"};
    final int[] position = {28,0};
    //TODO:每次需要修改
...
<head>
    <title>国庆七天乐</title>
    <style>
        .mask {width:153px;height:38px;position:absolute;background:transparent url('images/mask-2015-09-29.png') no-repeat fixed;background-position: 0px 0px;}
        .column0 {left:52px;}
        .column1 {left:222px;}
        .column2 {left:392px;}
        .column3 {left:563px;}
        ...
        .mask1{width:121px;height:55px;top:634px;left:228px;background-position: -200px 0px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            ...
            %>
            { focus:0, items : [
                {playType:-3,link:'http://192.168.49.144:8081/GamePortal/GUOQINGHuoDong.jsp',style:'mask mask1'}
            ]},
        ...
        var columns = 7; rows = 4;              //定义行列，　几行几列
        ...
        function focused(index,initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    focus = focus - 1;
                     ...
                     if( blocked == 0 && (
                         ...
                         index == -11 && focus != 22 && (focus + columns >= focusable[blocked].items.length && arrows[0] == 0 || arrows[0] == 1 && focus + 1 != focusable[blocked].items.length && focus + columns + 1 > focusable[blocked].items.length ) //按下光标键时
                     ) || blocked == 1 && ( index == -11 || index == -1 ) ) return;
                     if( blocked == focusable.length - 1) { blocked = index == -1 ? 1 : 0 ;focus = blocked == 1 ? 0 : focusable[0].items.length -1; }
                     else if( blocked == 0 ){
                         if( index == 1 || index == -1 ) focus += index;
                         else if( index == 11 ) focus -= columns;
                         else if( index == -11 ) { if( focus == 22) { blocked = 1; focus = 0; } else focus += columns; }
            ...
            if(typeof item != 'undefined' && typeof item.style == 'string' )
                ...
                var col = focus % 7;
                var row = Math.floor( focus / 7.0 );
                $('mask').className = 'mask row' + row + ' column' + col;
            }
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:undefined,popup:undefined,vote:undefined,video:undefined,position:undefined,gatewayCheck:undefined,external:{after:'<div style="width:1204px;height:149px;position: absolute;left: 26px;top:421px;background:transparent url(images/mask-2015-09-29.png) no-repeat fixed 0px -60px;"></div>',before:undefined}});
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-09-29.jpg') no-repeat;" onUnload="exit();"></body>
</html>