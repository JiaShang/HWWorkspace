...
<%
    ...
    final String[] types = {"10000100000000090000000000105083"};
...
<head>
    <title>尼泊尔地震新闻专题</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2015-04-29.png') no-repeat fixed;background-position: 0px 0px;}
        ...
        .maskText{background:transparent url('images/mask-2015-04-29.png') no-repeat fixed 0px 0px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == focusable.length - 1 ) {blocked = 0; focus = focusable[blocked].focus;$("mask").style.visibility = "hidden"; }
                    else {
                        var lastPageItemCount = focusable[blocked].items.length;
                        lastPageItemCount = Math.floor(lastPageItemCount / 12.0) * 12;
                        if( focus < 2 && index == 11 || index == -1 && focus % 2 == 0 && focus < 11 || index == 1 && focus % 2 == 1 && focus > lastPageItemCount) return;
                        if( index == 11 ) focus -= 2;
                        else if( index == -11 ) {
                            focus += 2;
                            if( focus + 1 > focusable[blocked].items.length ) { blocked = focusable.length - 1; focus = focusable[blocked].focus; flowedShow(0); $("mask").style.visibility = "visible"; }
                        } else if ( index == -1 ) { focus -=  focus % 2 == 1 ? 1 : 12; }
                        else if( index == 1 ) {
                            focus +=  focus % 2 == 0 ? 1 : 12;
        ...
        var flowedShow = function(index){
            ...
            var styleStr = ' style="margin-left:60px;" '
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                if( i == focus && blocked == index ){
                    html += '<div class="maskText"' + (i % 2 == 1 ? styleStr : "") +'>';
                    html += items[i].length > <%=popItemMaxCharLength%> ? ( '<marquee class="maskMarquee">' + items[i].text + '</marquee>') : items[i].text;
                    html +=  '</div>';
                } else {
                    html += '<div class="itemText"' + (i % 2 == 1 ? styleStr : "") +'>' + items[i].shortText + '</div>';
                }
            }
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-04-29.jpg') no-repeat;" onUnload="exit();"></body>
</html>