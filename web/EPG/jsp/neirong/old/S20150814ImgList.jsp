...
<%
    ...
    try {
        ...
        isGoGatewayDetail = (null != isGateway && "true".equalsIgnoreCase(isGateway) ? "true" : "false");
        // 返回时获取焦点信息数据
        ...
        list = getVodList( metaHelper, typeId , 200,  0);
    } catch (Exception e){}
%>
<html>
...
<head>
    ...
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2015-08-14-ImgList.png') no-repeat fixed;background-position: 0px 0px;}

        .flowed {width:1130px;height:500px;left:79px;top:89px;position: absolute;overflow:hidden;}
        .listItem {width:225px; height:257px;float:left;overflow: hidden;position: relative; }
        .flowedItem,.flowedItemFocus {position:absolute; top:0px; left:19px;width:187px;height:240px;}
        .flowedItem {background:transparent url('images/mask-2015-08-14-ImgList.png') no-repeat fixed 0px 0px;}
        .flowedItemFocus{background:transparent url('images/mask-2015-08-14-ImgList.png') no-repeat fixed -2px -250px; }
        .flowedImage{position:absolute; top:4px; left:23px;width:178px;height:232px;}
        .flowedText {width:176px;height:34px;left:23px;top:200px;position: absolute;line-height: 33px;font-size: 22px; color:#ffffff; text-align: center;}
        .pageSize {width:100px; height:20px;font-size:20px;color:#ffffff;position: absolute;left:952px;top:597px; text-align: right;}
        .Title {width:229px;height: 33px;line-height: 33px;left:116px;font-weight: bold;font-size: 28px;color:#ffffff;position: absolute;top:48px; text-align: center;}

        .pageUp,.pageDown {width:72px;height:37px;top:590px;left:1066px;background-position: 0px -500px;}
        .pageDown {left:1134px;background-position: -100px -500px;}
        ...
        .arrowLeft {width:44px;height:47px;left:27px;top:396px;position:absolute;background:transparent url('images/mask-2015-08-14-ImgList.png') no-repeat fixed -7px -180px; }
        .arrowRight {width:44px;height:47px;left:1203px;top:396px;position:absolute;background:transparent url('images/mask-2015-08-14-ImgList.png') no-repeat fixed -123px -180px; }
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            ...
            if( list != null && list.size() > 0){
                value += "{focus:0,typeId:'" + typeId + "',items:[";
            ...
            %>
            { focus:0, items : [
                {name:'上一页',style:'mask pageUp'},
                {name:'下一页',style:'mask pageDown'}
            ]},
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if(
                    ...
                    blocked == focusable.length - 1 && (focus == 0 && index == -1 || index == -11  ) ||
                    //如果焦点在投票上,按上下左右键时
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && (
                        ccx < 5 && index == 11 ||
                        ccx > 5 && focus + 1 < focusable[0].items.length && ccx < 9 && index == -11 ||
                      ( focus == 0 || focus == 5 ) && index == -1 ||
                      ( focus + 1 >= focusable[0].items.length || focus + 6 >= focusable[0].items.length && ccx == 4 ) && index == 1 ) ||
                       blocked == 1 && ( focus == 1 && index == 1 || index == -11 )
                    ) return;

                    if( blocked == 0 ) {
                        if( index == 1 ) {
                            if( ccx == 4 || ccx == 9 ) {
                                if( focus + 6 <= focusable[0].items.length ) focus += 6;
                                else focus += 1;
                            }
                            else focus += 1;
                        } else if( index == -1 ) {
                            if( ccx == 0 || ccx == 5 ) {
                                if( focus - 6 >= 0 )  focus -= 6; else focus -= 1;
                            }  else { focus -= 1; }
                        } else if( index == 11 )
                            focus -= 5;
                        else {
                            if( ccx == 0 && focus + 5 >= focusable[0].items.length && focus + 1 < focusable[0].items.length || ccx == 5 ) {
                                blocked = focusable.length -1 ; focus = focusable[blocked].focus;flowedShow(0);
                            } else if( ccx == 9 || focus + 1 >= focusable[0].items.length && ccx != 5 || ccx == 4 && focus + 5 >= focusable[0].items.length  ) {
                                blocked = 1; focus = focusable[blocked].focus; flowedShow(0);
                            } else if( focus + 5 >= focusable[0].items.length ) {
                                focus = focusable[0].items.length - 1 ;
                            } else focus += 5;
                        }
                    } else if(blocked == 1){
                        if( index == 1 ) focus = 1;
                        else if( index == -1 ) { if( focus == 1 ) focus = 0; else { blocked = focusable.length - 1; focus = 1; } }
                        else { blocked = 0; focus = Math.floor(focusable[blocked].focus / 10.0) * 10 + 9; if( focus >= focusable[blocked].items.length ) focus = focusable[blocked].items.length - 1; flowedShow(0); }
                    } else if ( blocked == focusable.length - 1 ) {
                        if( index == 1 ){
                            blocked = 1; focus = 0;
                        } else {
                            blocked = 0;focus = Math.floor(focusable[blocked].focus / 10.0) * 10 + 5; if( focus >= focusable[blocked].items.length ) focus = Math.floor(focusable[blocked].focus / 10.0) * 10 ;flowedShow(0);
                        }
                    }

                    if( blocked != 0 && $("mask").style.visibility != 'visible') $("mask").style.visibility = 'visible';
                    else if( blocked == 0 &&  $("mask").style.visibility == 'visible') $("mask").style.visibility = 'hidden';
                }
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                var item = items[i];
                html += "<div class='listItem'><div class='flowedImage'> <img  src='" + item.picture + "' style='width:178px;height:232px;' /></div>";
                html += '<div class="' + ((i == focus && blocked == index) ? 'flowedItemFocus">' : 'flowedItem">') + "</div>";
                html += '<div class="flowedText">';
                if( i == focus && blocked == index && item.length > 12 ) {
                    html += '<marquee style="width:100%;height:100%">' + item.text + "</marquee>";
                } else
                    html += item.shortText;
                html += "</div></div>"
            }
            $("pageSize").innerHTML = ( Math.floor(focus / size) + 1 ) + "&nbsp;/&nbsp;" + Math.ceil( items.length / size );
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:[0],popup:undefined,vote:undefined,video:undefined,position:undefined,external:{before:undefined,after:'<div class="Title" id="Title"><%= column.getName() %></div><div class="pageSize" id="pageSize"></div>'}});
            if( $.isGoGatewayDetail ) { iPanel.eventFrame.isEpg3 = true; }
        ...
        function doEnter(){
            ...
            if( blocked == 1 ) {
                ...
                if( focusable[blocked].focus == 0 ) {
                    if( focus - 10 >= 0 ) focus -= 10;
                } else if( focusable[blocked].focus == 1 ){
                    if( focus + 1 < focusable[0].items.length ) focus += 10;
                    if( focus >= focusable[0].items.length ) focus = focusable[0].items.length - 1;
                }
                focusable[0].focus = focus;
                flowedShow(0);
                return;
            }
            //首页，返回
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-08-14-ImgList.jpg') no-repeat;" onUnload="exit();"></body>
</html>