...
<%
    ...
    final String[] types = {"10000100000000090000000000105484"};
    ...
    try {
        ...
        if( focused == 0 && list.size() > 0 && list.get(0).size() > 0 ) focused = list.get(0).size() - 1;
    } catch (Exception e){}
%>
<html>
...
<head>
    <title>师者如诗</title>
    <style>
        .mask {width:34px;top:64px;position:absolute;background:transparent url('images/mask-2015-09-10.png') no-repeat fixed;background-position: 0px 0px;}

        .mask1 {height:295px;left:158px;background-position:0px 0px;}
        .mask2 {height:199px;left:204px;background-position:-40px 0px;}
        .mask3 {height:263px;left:248px;background-position:-80px 0px;}
        .mask4 {height:354px;left:424px;background-position:-120px 0px;}
        .mask5 {height:276px;left:470px;background-position:-160px 0px;}
        .mask6 {height:104px;left:514px;background-position:-200px 0px;}
        .mask7 {height:310px;left:693px;background-position:-240px 0px;}
        .mask8 {height:328px;left:733px;background-position:-280px 0px;}
        .mask9 {height:260px;left:849px;background-position:-320px 0px;}
        .mask10 {height:195px;left:895px;background-position:-360px 0px;}
        .mask11 {width:194px;height:87px;left:992px;top:567px;background-position:-400px 0px;}
        ...
        .btnHome {position:absolute;background: transparent url("images/navBG.png") no-repeat fixed; top:616px;left:147px; width:161px;height:41px; background-position: 0px 0px;}
        .btnReturn{position:absolute;background: transparent url("images/navBG.png") no-repeat fixed; top:616px;left:147px; width:161px; height:41px; background-position: 0px -42px;}
    </style>
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            ...
            %>
            { focus:0, items : [
                {name:'play',style:'mask mask11',playType:-3,link:'http://192.168.42.52/epg/index/newEast.do'},
            ]},
        ...
        var columns = 10; rows = 1;              //定义行列，　几行几列
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && ( index == 11 || focus == 0 && index == -1 ) || blocked == 1 && ( index == 1 || index == -11 )) return;
                    if( blocked == 0 ) {
                        if( index === 1 || index === -1 ) {
                            focus += index;
                            if( focus >= focusable[0].items.length ) { blocked = 1; focus = 0; }
                        } else if( index === -11 ){
                            if( focus <= 2 ) {
                                blocked = focusable.length - 1; focus = 0;
                            } else if( focus = focusable[0].items.length -1 ) {
                                blocked = 1; focus = 0;
                            } else { return; }
                        }
                    } else if( blocked == 1 ){
                        if( index == -1 ) { blocked = focusable.length - 1; focus = 1; }
                        else { blocked = 0; focus = focusable[0].items.length - 1; }
                    } else if( blocked === focusable.length - 1 ) {
                        if( index == 11 ) { blocked = 0 ; focus = 0; }
                        else if( focus == 1 && index == 1 ) { blocked = 1; focus =  0; }
                        else return;
                    }
                }
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-09-10.jpg') no-repeat;" onUnload="exit();"></body>
</html>