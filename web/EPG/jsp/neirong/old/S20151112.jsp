...
<%
    ...
    final String[] types = {"10000100000000090000000000105644"};
    final int[] position = {50,0};
    //TODO:每次需要修改
    int focused = 1, area = 0;
...
<head>
    <title>007大长腿系列之幽灵党</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2015-11-12.png') no-repeat fixed;background-position: 0px 0px;}
        .mask1{width:164px;height:52px;left:851px;top:53px;background-position: 0px 0px}
        .mask2{width:227px;height:34px;left:845px;top:189px;background-position: 0px -60px}
        .mask3{width:227px;height:34px;left:845px;top:227px;background-position: 0px -100px}
        .mask4{width:227px;height:34px;left:845px;top:265px;background-position: 0px -140px}

        .flowed {width:1280px;height:319px;left:36px;top:304px; position: absolute; overflow: hidden;}
        .itemContainer,.itemBody,.overItem {border:none; width:229px;height:319px;float: left;margin:0px 4px;}
        .item {position:absolute;left:9px;top:8px;width:211px;height:303px;border:none;}
        .overItem {margin: 0px;position:absolute;float:none;left:0px;top:0px;}
        .itemBody{position: relative;}
        .overItem {background:transparent url('images/mask-2015-11-12.png') no-repeat fixed;background-position: 0px -180px;}
        .text,.focusText{position:absolute;width:206px;height:39px;line-height: 39px;left:11px;top: 256px; color:#333333;font-size:22px; text-align: center;overflow: hidden;}
        .focusText {color:#ffffff;}
        .focusText .marqueed{width:100%;color:#FFFFFF;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            ...
            if( list != null && list.size() > 0){
                for(int i = 0; i < list.size() ; i++){
                    value += "{focus:0,flowedFocus:7,typeId:'" + types[i] + "',items:[{text:'抢票',playType:-2,style:'mask1'},";
                    List<Vod> vodList = list.get(i);
                    for( int j = 0; j < vodList.size() ; j++ ){
                        Vod vod = vodList.get(j);
                        String str = "{ mid:'" + vod.getId() + "'," ;
                                if( j < 3 ) { str += "style:'mask mask" + (j + 2) + "'" + "," ;}
                                else {
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && (
                        focus == 0  && index == 11 ||
                        focus <= 4 && index == -1 ||
                        ( focus <= 3 || focus == focusable[0].items.length - 1 ) && index == 1 ) ) return;

                    if( blocked == 0 ) {
                        if( index == 1 || index == -1 ) {
                            focus += index;
                            focusable[0].flowedFocus = focus;
                        } else if( index == -11 ){
                            focus += 1 ;
                            if( focus == 4 ) {
                                focus = focusable[0].flowedFocus;
                            } else if(focus > 4 ){
                ...
                if( blocked == 0 && focus > 3 ) focusable[0].flowedFocus = focus;
            ...
            $("mask").style.visibility = blocked == 0 && focus >= 4 ? "hidden" : "visible";
            flowedShow(0);
        }
        var flowedShow = function(index){
            ...
            //每页显示数量
            var size = 5.0;
            focusable[index].page = Math.floor((focusable[0].flowedFocus - 4 ) / size);
            flowCursorIndex = focusable[index].page * size + 4 ;
            if( items.length - flowCursorIndex < size) flowCursorIndex = items.length - 5 ;
            var html = ''; var length =  size ;
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                var item = items[i];
                html += "<div class='itemContainer'><div class='itemBody'><div class='item' style='background: url(\"" + item.picture + "\");'></div><div class='overItem'><div class='";
                if( i == focus && blocked == index ) {
                    html += "focusText'>" ;
                    html += item.length > 16 ? ( "<marquee class='marqueed'>" + item.text + "</marquee>" ) : item.text;
                } else
                    html += "text'>" + item.shortText ;
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:[0],popup:undefined,vote:[{blocked:0,item:undefined,id: 253}],video:undefined,position:undefined,gatewayCheck:undefined,external:{after:undefined,before:undefined}});
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-11-12.jpg') no-repeat;" onUnload="exit();"></body>
</html>