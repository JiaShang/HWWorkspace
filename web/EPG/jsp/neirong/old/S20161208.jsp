...
<%
    ...
    final String[] types = {"10000100000000090000000000107004"};
    final int[] position = {50,0};
    //TODO:每次需要修改
    int focused = 0, area = 0;
    final int popItemMaxCharLength = 24;
...
<head>
    ...
    <style>
        ...
        .bgLeft{position:absolute;width:386px;height:720px;left:0px;top:0px;background:transparent url('images/bg-2016-12-08-1.jpg') no-repeat 0px 0px; }
        .bgTop{position:absolute;width:496px;height:213px;left:386px;top:0px;background:transparent url('images/bg-2016-12-08-2.jpg') no-repeat 0px 0px; }
        .bgBottom{position:absolute;width:496px;height:227px;left:386px;top:493px;background:transparent url('images/bg-2016-12-08-3.jpg') no-repeat 0px 0px; }
        .bgRight{position:absolute;width:398px;height:720px;left:882px;top:0px;background:transparent url('images/bg-2016-12-08-4.jpg') no-repeat 0px 0px; }

        .flowedItem {width:349px;height:58px;left:894px;font-size:24px;line-height:58px;overflow: hidden;padding:0px 10px 0px 10px;}
        .item{color:white;background: transparent none;}
        .itemquee {width:329px;height:59px;line-height:58px;}
        ...
        .focus {color:#9fb606;background:transparent url('images/mask-2016-12-08.png') no-repeat;background-position: -50px 0px;}

        .arrow {width:20px;height:18px;left:1208px;}
        .arrowLeft{top:446px;background-position:0px 0px;}
        .arrowRight{top:467px;background-position:0px 30px;}

        .mask2x {width:272px;height:135px;top:506px;background:transparent url('images/mask-2016-12-08.png') no-repeat;}
        .mask21 {left:398px;background-position:0px -60px;}
        .mask22 {left:677px;background-position:0px -200px;}
        .mask23 {left:959px;background-position:0px -340px;}

        .flowed {position:absolute;width:1280px;height:720px;left:0px;top:0px;}
        .after {}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            ...
            %>
            { focus:0,items : [
                {name:'第一场',style:'mask mask2x mask21',playType:-3,link:'/EPG/jsp/neirong/S20161208Detail.jsp?typeId=10000100000000090000000000107005'},
                {name:'第二场',style:'mask mask2x mask22',playType:-3,link:'/EPG/jsp/neirong/S20161208Detail1.jsp?typeId=10000100000000090000000000107006'},
                {name:'第三场',style:'mask mask2x mask23',playType:-3,link:'/EPG/jsp/neirong/S20161208Detail2.jsp?typeId=10000100000000090000000000107007'}
            ]},
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && ( index == 1 || index == -1 || index == 11 && focus <= 0 ) || blocked == 1 && ( index == -1 && focus == 0 || index == 1 && focus >= 3 ) ) return;
                    if( blocked === focusable.length - 1 ) {
                        blocked = 1; focus = focusable[blocked].focus;
                    } else if( blocked === 0) {
                        if( index == 11 ) focus -= 1 ;
                        else if( index == -11 && focus + 1 >= focusable[blocked].items.length ) {
                            blocked = 1 ; focus = focusable[blocked].focus;
                        } else focus += 1;
                    } else if( blocked == 1 ) {
                        if( index == 1 || index == -1 ) focus += index ;
                        else {
                            blocked = index > 0 ? 0 : focusable.length - 1;
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                var item = items[i];
                html += "<div class='mask flowedItem item item" + ( i % 4 + 1 ) + ( i == focus && blocked == index ? " focus" : "" ) +  "'>";
                html += i == focus && blocked == index ? ( item.text == item.shortText ? item.text : ( "<marquee class='itemquee'>" + item.text + "</marquee>" ) ) : item.shortText;
                html += "</div>"
                //if( ) 焦点
            }
        ...
        function init(){
            ...
            cricleId = "" + focusable[0].items[0].mid;
            focusable[0].items = focusable[0].items.splice(1,focusable[0].items.length - 1);
            $.buildUserInterface({flowed:[0],popup:undefined,vote:undefined,video:cricleId,position:[385,213,496,280],external:{after:undefined,before:'<div class="bgLeft"></div><div class="bgTop"></div><div class="bgBottom"></div><div class="bgRight"></div>'}}); //
        ...
        function play_ajax(){
            if( typeof iPanel == 'undefined' ) return;
            var getMovieRtspUrl = iPanel.eventFrame.pre_epg_url+"/defaultHD/en/go_authorization.jsp?playType=1&progId="+ cricleId +"&contentType=0&business=1&baseFlag=0";
        ...
        function eventHandler(eventObj, __type){
            switch(eventObj.code){
                ...
                case "EIS_VOD_PROGRAM_END":play_ajax();break;
            }
...
<body leftmargin="0" topmargin="0" style="background: transparent url('images/translateBg.png') no-repeat; overflow:hidden;" onUnload="exit();"></body>
</html>