...
<%
    ...
    final String[] types = {"10000100000000090000000000108244"};
...
<head>
    ...
    <style>
        .mask {width:360px;height:75px;position:absolute;background:transparent url('images/mask-2017-08-18.png') no-repeat;background-position: 0px 0px;}

        .mask1 {left:99px;top:313px;background-position:0px 0px;}
        .mask2 {left:99px;top:380px;background-position:0px -75px;}
        .mask3 {left:99px;top:444px;background-position:0px -150px;}
        .mask4 {left:99px;top:510px;background-position:0px -225px;}
        .mask5 {left:99px;top:576px;background-position:0px -300px;}

        .mask6 {left:260px;top:297px;background-position:-400px 0px;}
        .mask7 {left:260px;top:364px;background-position:-400px -75px;}
        .mask8 {left:260px;top:430px;background-position:-400px -150px;}
        .mask9 {left:260px;top:495px;background-position:-400px -225px;}
        .mask10 {left:260px;top:561px;background-position:-400px -300px;}
    ...
    <script language="javascript" type="text/javascript">
                ...
                for(int i = 0; i < list.size() ; i++){
                    ...
                    for( int j = 0; j < vodList.size() ; j++ ){
                        ...
                        String str = "{ mid:'" + vod.getId() + "'," +
                                    "text:\"" + vod.getName() + "\"," +
                                   "style:'mask mask" + (j + ( j >= 4 ? 4 : ( j >= 3 ? 3 : ( j >= 1 ? 2 : 1 ) ) )) + "'" + "," +
                                 "playType:" + vod.getIsSitcom() ;
                            str += "}" ;
                        if( j == 1 ) {
                            value += "{name:'微微一笑(电视剧)',style:'mask mask2',playType:-3,link:'http://192.168.17.74/youku/search_load.htm?vodId=PACKAGEP201612000075-youku'},";
                        } else if( j == 3 ) {
                            value += "{name:'何以笙箫默(电视剧)',style:'mask mask5',playType:-3,link:'http://192.168.17.57:9888/sohuepg/queryDetail?assetID=SHTV17282-sohu&source=mgtv'},";
                        } else if( j == 4 ) {
                            value += "{name:'微微一笑(电影)',style:'mask mask7',playType:-3,link:'http://192.168.17.155/nn_cms/web_template/index.html?nns_page_name=movie_detail&nns_video_id=7df819f55d935c8e9b1368e1244c428b'},";
                        }
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && !$.vote.tooltiped && blocked == focusable.length - 1 && focus == 1 ) focus = focus + 1;
                    ...
                    if( index == 11 && focus % 5 == 0 || index == -1 && focus <= 4 || index == 1 && focus >=5 || index == -11 && focus % 5 == 4 )
                        return;
                    if( index == 1 || index == -1 )
                        focus += index * 5;
                    else
            ...
            document.body.style.backgroundImage =  focus >= 5 ? "url('images/bg-2017-08-18-2.jpg')" : "url('images/bg-2017-08-18-1.jpg')";
        }
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:undefined,vote:undefined,video:undefined,position:undefined,external:{after:undefined,before:'<div id="after" style="position:absolute;top:720px;left:0px;width:0px;height:0px;overflow:hidden"><img src="images/bg-2017-08-18-<%=focused >= 5 ? 1 : 2%>.jpg" /></div>'}});
            ...
            switch(item.playType){
                ...
                case -3:
                    ...
                    if( ! item.link.startWith('http') ){
                        ...
                        url = item.link;
                        url += url.indexOf("?") > 0 ? '&' : '?';
                        url += url.indexOf('youku') >= 0 ? 'backUrl=' : 'backURL=';
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-08-18-<%= focused >= 5 ? 2 : 1%>.jpg') no-repeat;" onUnload="exit();"></body>
</html>