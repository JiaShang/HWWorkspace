...
<%
    ...
    final String[] types = {"10000100000000090000000000105213"};
    final String[] names = {"陈展鹏","马国明","吴卓羲","李思捷"};
    final int[] position = {4,0};
    //TODO:每次需要修改
...
<head>
    <title>TVB明星见面会</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2015-06-12.png') no-repeat fixed;background-position: 0px 0px;}
        .mask1 {width:55px;height:54px;left:156px;top:471px; background-position: 0px 0px;}
        .mask2 {width:55px;height:54px;left:454px;top:471px; background-position: 0px 0px;}
        .mask3 {width:55px;height:54px;left:764px;top:471px; background-position: 0px 0px;}
        .mask4 {width:55px;height:54px;left:1062px;top:471px; background-position: 0px 0px;}

        .maskVote1 {width:108px;height:54px;left:42px;top:547px; background-position: 0px -70px;}
        .maskVote2 {width:108px;height:54px;left:341px;top:547px; background-position: 0px -70px;}
        .maskVote3 {width:108px;height:54px;left:639px;top:547px; background-position: 0px -70px;}
        .maskVote4 {width:108px;height:54px;left:939px;top:547px; background-position: 0px -70px;}

        .maskLink {width:208px;height:44px;left:1025px;top:365px;background-position: 0px -140px;}
        .maskRole {width:114px;height:29px;left:532px;top:607px;background-position: 0px -210px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        //可获得焦点的区域个数
        var focusable = focusable ? focusable : [//点击立即进入TVB专区
            { focus:0,items : [{name:'点击立即进入TVB专区',style:'mask maskLink',playType:-3,link:'http://chongqingtvb-utc.wasu.cn/index.jsp'}]},
            <%
            value = "";
                ...
                for(int i = 0; i < list.size() ; i++){
                    ...
                    for( int j = 0; j < vodList.size() ; j++ ){
                        ...
                        String str = "{ mid:'" + vod.getId() + "'," +
                            ...
                            str += "}" ;
                        if(i == 0) str += ",{text:'" + names[j] + "',style:'mask maskVote" + ( j + 1) + "',playType:-2}";
            ...
            %>
            { focus:0,items : [{name:'活动细则',style:'mask maskRole',playType:-3,link:'/EPG/jsp/neirong/S20150612_1.jsp'}]},
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked === 0 && index != -11 ||
                        ...
                        blocked === 1 && ( focus <= 1 && index == -1 || focus >= 6 && index == 1  || focus % 2 == 0 && focus != 6 && index == 11 || focus == 1 && index == -11 ) )
                    return;
                    if( blocked === 0 ) {
                        blocked = 1; focus = 6;
                    } else if( blocked === 2 ) {
                        if( index == 11 ) { blocked = 1; focus = 3; }
                        else { blocked = focusable.length - 1; focus = 0;}
                    } else if( blocked == focusable.length -1 ){
                        blocked = index == 11 ? 1 : 2; focus = focusable[blocked].items.length - 1;
                    } else if( blocked === 1 ) {
                        if(index === 1 || index === -1) focus += index * 2;
                        else {
                            if( index === 11 && focus === 6 ) { blocked = 0; focus = 0; }
                            else if( index === -11 && ( focus === 3 || focus === 5 ||focus === 7  )) {
                                blocked = focus === 7 ? focusable.length - 1 : 2 ;
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:undefined,popup:undefined,vote:[{blocked:1,item:undefined,id: 219}],video:undefined,position:undefined,external:{after:undefined,before:undefined}});
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-06-12.jpg') no-repeat;" onUnload="exit();"></body>
</html>