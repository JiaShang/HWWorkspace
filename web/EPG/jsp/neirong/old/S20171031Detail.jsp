...
<head>
    <title>赛程详情</title>
    <style>
        .mask {width:468px;height:489px;left:591px;top:169px;position:absolute;background:transparent url('images/mask-2017-10-31-detail.png') no-repeat;background-position: 0px 0px;}
        .page1 {background-position: 0px 0px;}
        .page2 {background-position: -470px 0px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var consign = 1;                //用来缓存数据的
        var isKorean = <%= isKorean%>;                   //是否为韩剧
        var columns = 0; rows = 0;              //定义行列，　几行几列
        var pageCount = columns * rows;
        function focused(index,initilized){
            if( index == -1 || index == 1 || consign == 1 && index == 11 || consign == 2 && index == -11) return;
            if( ! initilized ) {
                consign += index > 0? -1 : 1;
            }
            $("mask").className = "mask page" + consign;
        }
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-10-31-detail.jpg') no-repeat;" onUnload="exit();"></body>
</html>