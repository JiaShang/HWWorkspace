...
<%
    ...
    final String[] types = {"10000100000000090000000000105082"};
...
<head>
    <title>银幕劳模光荣榜</title>
    <style>
        .mask {width:146px;height:39px;top:344px; position:absolute;background:transparent url('images/mask-2015-04-28.png') no-repeat fixed;background-position: 0px -30px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && ( index == 11 || index == -1 && focus == 0 || focus + 1 == focusable[blocked].items.length && index == 1 || index == -11 && focus + 1 != focusable[blocked].items.length )) return;
                    if( blocked == focusable.length - 1 ) { blocked = 0; focus = focusable[blocked].focus ;}
                    else {
                        if( index == -11 ) { blocked = focusable.length - 1; focus = focusable[blocked].focus ; }
                        else  focus += index ;
                    }
                }
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:undefined,popup:undefined,vote:undefined,video:undefined,position:undefined,external:{after:'<div style="width:1175px;height:27px;position: absolute;left: 63px;top:350px;background:transparent url(images/mask-2015-04-28.png) no-repeat fixed 0px 0px;">',before:undefined}});
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-04-28.jpg') no-repeat;" onUnload="exit();"></body>
</html>