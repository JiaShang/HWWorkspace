...
<head>
    <title></title>
    <style>
        .mask {width:114px;height:130px;position:absolute;background:transparent url('images/mask-2016-03-07Rule.png') no-repeat fixed;background-position: 0px 0px;}
        .mask1{left:259px;top:137px;}
        .mask2{left:395px;top:137px;}
        .mask3{left:533px;top:137px;}
        .mask4{left:671px;top:137px;}
        .mask5{left:807px;top:137px;}
        .mask6{left:942px;top:137px;}
        .mask7{left:189px;top:269px;}
        .mask8{left:325px;top:269px;}
        .mask9{left:463px;top:269px;}
        .mask10{left:601px;top:269px;}
        .mask11{left:737px;top:269px;}
        .mask12{left:872px;top:269px;}
        .mask13{left:259px;top:406px;}
        .mask14{left:395px;top:406px;}
        .mask15{left:533px;top:406px;}
        .mask16{left:671px;top:406px;}
        .mask17{left:807px;top:406px;}
        .mask18{left:942px;top:406px;}
        .mask19{width:175px;height:42px;left:848px;top:576px;background-position: 0px -150px;}

        .focusBg{width:841px;height:442px;left:210px;top:166px;background:transparent url('images/focusBg-2016-03-07Rule.png') no-repeat fixed 0px 0px;position:absolute; }
        .voteFocusBg{width:443px;height:720px;left:837px;top:0px;background:transparent url('images/focusBg-2016-03-07RuleVote.jpg') no-repeat fixed 0px 0px;position:absolute; }

        .voteItem{width:100px;height:28px;color:#E33E3F; font-size:18px;float:left;line-height:25px;}
        .voteItemContainer {left:175px;top:102px;width:100px;height:506px;position:absolute; overflow: hidden;}
        .flowed {}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,items : [
                {name:'简政放权',style:'mask mask1',playType:-2},
                {name:'医疗改革',style:'mask mask2',playType:-2},
                {name:'社会保障',style:'mask mask3',playType:-2},
                {name:'攻坚扶贫',style:'mask mask4',playType:-2},
                {name:'打虎拍蝇',style:'mask mask5',playType:-2},
                {name:'经济新常态',style:'mask mask6',playType:-2},
                {name:'居民收入',style:'mask mask7',playType:-2},
                {name:'教育公平',style:'mask mask8',playType:-2},
                {name:'新型城镇化',style:'mask mask9',playType:-2},
                {name:'一带一路',style:'mask mask10',playType:-2},
                {name:'创业创新',style:'mask mask11',playType:-2},
                {name:'全面二孩',style:'mask mask12',playType:-2},
                {name:'公共安全',style:'mask mask13',playType:-2},
                {name:'金融风险',style:'mask mask14',playType:-2},
                {name:'环境保护',style:'mask mask15',playType:-2},
                {name:'军事改革',style:'mask mask16',playType:-2},
                {name:'司法改革',style:'mask mask17',playType:-2},
                {name:'住房',style:'mask mask18',playType:-2},
            ]},
            { focus:0,items : [
                {name:'查看投票结果',style:'mask mask19'}
            ]},
            { focus:0,items : [
                {name:'关闭投票查询'},
            ]},
        ...
        var columns = 6; rows = 3;              //定义行列，　几行几列
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    focus = focus - 1;
                     ...
                     if( blocked ==0  && (
                         ...
                         index == -11 && focus >= 12 && ( focus % 6 < 4) //按下光标键时
                     ) || blocked == 1 && ( index == 1 || index == -1 ) || blocked == 2) return;
                     if( blocked == focusable.length - 1) { blocked = 1 ;focus = 0; }
                     else if( blocked == 0 ){
                         if( index == 1 || index == -1 ) focus += index;
                         else if( index == 11 ) focus -= columns;
                         else if( index == -11 ){
                             focus += columns;
                             if( focus >= focusable[blocked].items.length ) { blocked = 1;focus = 0; }
                         }
                     } else if( blocked == 1 ) {
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:undefined,popup:undefined,vote:[{blocked:0,item:undefined,id: 196}],video:undefined,position:undefined,gatewayCheck:undefined,external:{after:'<div class="focusBg"></div><div class="voteFocusBg" id="voteFocusBg"><div id="voteItemContainer" class="voteItemContainer"></div></div>',before:undefined}});
            if( focusable.length == 2 ) {blocked = focusable.length - 1;}
            $("voteFocusBg").style.visibility = 'hidden';
            focused(<%= focused %>, true);
        }
        function queryVoteResult( callback ){
            $.ajax('http://192.168.49.56:8080/voteNew/external/queryVoteDatas.ipanel?columnA=lh&isable=1&startIndex=0&pageSize=50&total=true',function(result){
                var columns = result.result;
                for( var i = 0; i < columns.length; i++ ){
                    if( typeof focusable[0].items[i].voteId === 'undefined' ){
                        focusable[0].items[i].voteId = columns[i].ID;
                    }
                    focusable[0].items[i].total = columns[i].total;
                }
        ...
        function voteFunction () {
            var cardId = CA.card.serialNumber;
            var item = focusable[0].items[focusable[0].focus];
            var url = "http://192.168.49.56:8080/voteNew/external/addVote.ipanel?icid=" + cardId + "&repeat=false&phone=13999999999&classifyID=" + item.voteId + "&content=" + encodeURIComponent(item.name);
            $.ajax(url, function(result){
                if(result.recode != '002' || result.result == false ){
                    $.vote.tooltip($.vote.fail);
                } else {
        ...
        var voteOpened = false;
        function doEnter(){
            try{ E.is_HD_vod = true; } catch (e) {}
            //首页，返回
            if( blocked == 0 ) { if( voteOpened ) {$.vote.hidden(); voteOpened = false; } else { voteFunction ( );  voteOpened = true;} return;}
            if( blocked == 1 ) { $("voteFocusBg").style.visibility = 'visible'; $("mask").style.visibility = 'hidden'; blocked = 2; queryVoteResult(function(){
                var html = '';
                var items = focusable[0].items;
                for( var i = 0; i < items.length; i++ )
                    html += '<div class="voteItem">' + items[i].total + '</div>';
                $("voteItemContainer").innerHTML = html;
            });return;}
            if( blocked == 2 )  { $("voteFocusBg").style.visibility = 'hidden'; $("mask").style.visibility = 'visible';  blocked = 1; return;}
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-03-07Rule.jpg') no-repeat;" onUnload="exit();"></body>
</html>