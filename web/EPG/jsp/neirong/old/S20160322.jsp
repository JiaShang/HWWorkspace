...
<%
    ...
    final String[] types = {"10000100000000090000000000106000"};
    final int[] position = {6,0};
    //TODO:每次需要修改
    int focused = 5, area = 0;
...
<head>
    <title>权力游戏S6</title>
    <style>
        .mask {width:70px;height:26px;top:602px;position:absolute;background:transparent url('images/mask-2016-03-22.png') no-repeat fixed;background-position: 0px 0px;}

        .image {width:142px;height:238px;top:301px;position:absolute;overflow: hidden;background-color: red;}
        .image1 {left:81px;}
        .image2 {left:344px;}
        .image3 {left:580px;}
        .image4 {left:810px;}
        ...
        .normal1{background-position:0px 0px;}
        .mask1{background-position:0px -30px;}
        .normal2{background-position:0px -60px;}
        .mask2{background-position:0px -90px;}
        .normal3{background-position:0px -120px;}
        .mask3{background-position:0px -150px;}
        .normal4{background-position:0px -180px;}
        .mask4{background-position:0px -210px;}
        .normal5{background-position:0px -240px;}
        .mask5{background-position:0px -270px;}
        .normal6{background-position:0px -300px;}
        .mask6{background-position:0px -330px;}

        .flowed {position:absolute;top:0px;left:0px;width:0px;height:0px;}
        .before {position: absolute;width: 1280px;height:720px;top:0px;left:0px;background:transparent url('images/bg-2016-03-22.png') no-repeat fixed 0px 0px;}
    ...
    <script language="javascript" type="text/javascript">
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 ) {
                        if( index == -11 ) {
                            ...
                            focus += index ;
                            if( index == -1 && focus < 0 ) focus = focusable[blocked].items.length - 1;
                            else if( index == 1 && focus >= focusable[blocked].items.length ) focus = 0;
                        }
                    } else {
        ...
        var flowedShow = function(index){
            ...
            //每页显示数量
            var html = '';
            for( var i = focus - 2, l = 0; l < 5; i++,l++ ) {
                var cur = i;
                if( cur < 0 ) cur += items.length;
                else if( cur >= items.length ) cur -= items.length ;
                var item = items[cur];
                $("image" + ( l + 1)).src = item.picture;
                $("text" + ( l + 1)).className = "mask text" + (l + 1) + " " + ( cur == focus ? 'mask' : 'normal' ) +  (cur + 1);
            }
        }
        function init(){
            ...
            $.buildUserInterface({flowed:[0],popup:undefined,vote:undefined,video:undefined,position:undefined,gatewayCheck:undefined,external:{after:'',before:'<img class="image image1" id="image1" /><img class="image image2" id="image2" /><img class="image image3" id="image3" /><img class="image image4" id="image4" /><img class="image image5" id="image5" /><div class="before" id="before"></div><div id="text1"></div><div id="text2"></div><div id="text3"></div><div id="text4"></div><div id="text5"></div>'}});
...
<body leftmargin="0" topmargin="0" style="background:black url('images/bg-2016-03-22.jpg') repeat;overflow:hidden;" onUnload="exit();"></body>
</html>