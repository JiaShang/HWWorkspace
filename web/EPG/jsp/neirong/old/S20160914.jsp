...
<%
    ...
    final String[] types = {"10000100000000090000000000106690","10000100000000090000000000106690"};
    final int[] position = {3,0,4,3};
    //TODO:每次需要修改
...
<head>
    <title>大话西游重回人间</title>
    <style>
        .mask {width:257px;height:334px;top:297px;position:absolute;background:transparent url('images/mask-2016-09-14.png') no-repeat;background-position: 0px 0px;}

        .mask1 {width:229px;height:31px;left:41px;top:368px;background-position: 0px 0px;}
        .mask2 {width:421px;height:31px;left:41px;top:404px;background-position: 0px -40px;}
        .mask3 {width:256px;height:31px;left:41px;top:440px;background-position: 0px -80px;}
        .mask4 {left:430px;background-position: 0px -120px;}
        .mask5 {left:625px;background-position: 0px -120px;}
        .mask6 {left:820px;background-position: 0px -120px;}
        .mask7 {left:1015px;background-position: 0px -120px;}
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && ( index == -1 || focus == 0 && index == 11 || focus + 1 >= focusable[blocked].items.length && index == -11) ||
                        blocked == 1 && ( index == 11 || index == -11 && focus + 1 < focusable[blocked].items.length  || focus + 1 >= focusable[blocked].items.length && index == 1)) return;
                    if( blocked == 0 ) {
                        ...
                        if( index == 1 || index == -1 ) {
                            if( index == -1 && focus == 0 ) { blocked = 0; focus = focusable[blocked].focus; }
                            else focus += index ;
                        } else if( index == -11 ) {
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-09-14.jpg') no-repeat;" onUnload="exit();"></body>
</html>