...
<%
    ...
    final String[] types = {"10000100000000090000000000105634"};
...
<head>
    <title>TFBOYS少年正当红</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2015-11-06.png') no-repeat fixed;background-position: 0px 0px;}

        .flowed {position: absolute;width:520px;height: 260px;overflow: hidden; left:40px;top:365px;}
        .flowedItem{width:520px;height:43px; text-align: center; overflow: hidden;}
        .item,.focusItem{height: 43px; margin: 0px auto;}
        .left{float: left;width:74px;height:43px;}
        .cent{float: left;width: auto; max-width: 423px; min-width: 50px;  height:43px;line-height:36px; font-size:24px;color: #7A5433; overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;}
        .right{float: left;width:23px;height:43px;}
        .focusItem .left {background:transparent url('images/mask-2015-11-06.png') no-repeat fixed 0px -1px;}
        .focusItem .cent {color: #FEE5C0;background:transparent url('images/mask-2015-11-06.png') repeat-x fixed 0px -47px;}
        .focusItem .cent .marqueed {height:40px;line-height:36px; color: #FEE5C0;width:100%;}
        .focusItem .right {background:transparent url('images/mask-2015-11-06.png') repeat-x fixed -100px -1px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && ( focus == 0 && index == 11 || index == -1 || index == -11 && focus >= focusable[0].items.length - 1) ) return;
                    if( blocked == 0 ) {
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                ...
                var isFocus = i == focus && blocked == index;
                html += "<div class='" +  (isFocus ? "focusItem" : "item") + "'>";
                html += "<div class='left'></div><div class='cent'>";
                html += isFocus && item.length > 32 ? ( "<marquee class='marqueed'>" + item.text + "</marquee>" ) : item.text;
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:[0],popup:undefined,vote:undefined,video:undefined,position:undefined,gatewayCheck:undefined,external:{after:undefined,before:undefined}});
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-11-06.jpg') no-repeat;" onUnload="exit();"></body>
</html>