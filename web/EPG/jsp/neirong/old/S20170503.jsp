...
<%
    ...
    final String[] types = {"10000100000000090000000000107640"};
    final int[] position = {5,0};
    //TODO:每次需要修改
...
<head>
    <title>818那些好看的律政剧</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2017-05-03.png') no-repeat;background-position: 0px 0px;}

        .mask1 {width:105px;height:52px;left:119px;}
        .mask11 {top:308px;background-position: 0px 0px;}
        .mask12 {top:366px;background-position: 0px -60px;}
        .mask13 {top:422px;background-position: 0px -120px;}
        .mask14 {top:477px;background-position: 0px -180px;}

        .image {width:176px;height:234px;top:31px; position:absolute;}
        .image1 {left:266px;}
        .image2 {left:477px;}

        .mask2 {width:182px;height:240px;top:28px;}
        .mask21,.mask22,.mask21Focus,.mask22Focus{left:263px;background-position: -110px 0px;}
        .mask21,.mask22{background-position: -300px 0px;}
        .mask22,.mask22Focus {left:474px;}

        .text {color:#87817F;text-align: center;height: 32px;position:absolute;top:280px; overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;}
        .text1,.text2,.marquee,.text1Focus,.text2Focus {width:180px;line-height: 30px;font-size: 22px;}
        .text1,.text1Focus{left:263px;}
        .text2,.text2Focus{left:474px;}
        .text1Focus,.text2Focus{color:#FFE765;}

        .flowed {width:1280px;height:320px;left:0px;top:300px;position:absolute;}
        .after {}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,items : [
                {name:'内地',style:'mask mask1 mask11'},
                {name:'香港',style:'mask mask1 mask12'},
                {name:'英国',style:'mask mask1 mask13'},
                {name:'美国',style:'mask mask1 mask14'}
            ]},
            <%
            value = "";
                ...
                for(int i = 0; i < list.size() ; i++){
                    ...
                    for( int j = 0; j < vodList.size() ; j++ ){
                        ...
                        if(j == 1) {
                            ...
                            str += ",{" +
                                   "text:\"" + lName + "\"," +
                                 "length:" + StringUtil.length(lName) + "," +
                              "shortText:\"" + StringUtil.limitStringLength(lName, popItemMaxCharLength) + "\"" + "," +
                              "picture:\"images/item-2017-05-03-1.jpg\"" + "," +
                                "link:'http://chongqingtvb-utc.wasu.cn/a?f=72502&t=37&a=30981135'" + "," +
                               "playType:-3}";
                            lName = "完美叛侣";
                            str += ",{" +
                                   "text:\"" + lName + "\"," +
                                 "length:" + StringUtil.length(lName) + "," +
                              "shortText:\"" + StringUtil.limitStringLength(lName, popItemMaxCharLength) + "\"" + "," +
                              "picture:\"images/item-2017-05-03-2.jpg\"" + "," +
                                "link:'http://chongqingtvb-utc.wasu.cn/a?f=72502&t=37&a=28497848'" + "," +
                               "playType:-3}";
                        }
        ...
        var columns = 2; rows = 1;              //定义行列，　几行几列
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && !$.vote.tooltiped && blocked == focusable.length - 1 && focus == 1 ) focus = focus + 1;
                    ...
                    if( blocked == 0 && (index == -1 || index == 11 && focus <= 0 || index == -11 && focus + 1 >= focusable[blocked].items.length ) ||
                        blocked == 1 && ( index == -11 || index == 11 || index == 1 && ( focus % 2  == 1 || focus  + 1 >= focusable[blocked].items.length ) )) return;
                    if( blocked  == 0 ) {
                        if( index == 11 || index == -11 ) { focus += index > 0 ? -1 : 1 ;focusable[1].focus = focus * 2 ;}
                        else {
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                var item = items[i];
                var fs = "";
                if( i == focus && blocked == index ) fs = "Focus";
                html += "<img class='image image" + ( i % 2 + 1) + "' src='" + item.picture + "' />";
                html += "<div class='mask mask2 mask2" + ( i   % 2 + 1)  + fs + "'></div>";
                html += "<div class='text text" + ( i   % 2 + 1) + fs + "'>" + ( fs != "" && item.shortText != item.text  ? ("<marquee class='marquee'>" + item.text + "</marquee>") : item.shortText ) + "</div>";
            }
        ...
        function init(){
            ...
            $.vote.focus = {}
            $.current = function (){
                return "/EPG/jsp/defaultHD/en/SaveCurrFocus.jsp?currFoucs=1," + focusable[1].focus + "&url=";
            };
            ...
            focusable[1].focus = <%= focused %>;
            focusable[0].focus = Math.floor( focusable[1].focus / 2 );
            $.buildUserInterface({flowed:[1],vote:undefined,video:undefined,position:undefined,external:{after:undefined,before:undefined}}); //'<div class="after"></div>'
        ...
        function doEnter(){
            ...
            //首页，返回
            if( blocked == 0 ) {
                blocked = 1; focusable[1].focus = focusable[0].focus * 2 ;
                focused(focusable[1].focus, true);
                return;
            }
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-05-03.jpg') no-repeat;" onUnload="exit();"></body>
</html>