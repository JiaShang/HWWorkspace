...
<%
    ...
    final String[] types = {"10000100000000090000000000105421"}; //
...
<head>
    <title>暑假动画也疯狂</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2015-08-25.png') no-repeat fixed;background-position: 0px -250px;}
        .mask1{width:80px;height:47px;left:455px;top:191px;}
        .mask2{width:80px;height:47px;left:455px;top:226px;}
        .mask3{width:80px;height:47px;left:455px;top:262px;}
        .mask4{width:80px;height:47px;left:455px;top:300px;}
        .mask5{width:80px;height:47px;left:455px;top:334px;}
        .mask6{width:80px;height:47px;left:455px;top:369px;}
        .mask7{width:80px;height:47px;left:455px;top:408px;}
        .mask8{width:28px;height:28px;left:1007px;top:594px;background-position: -100px -250px;}
        ...
        .btnHome {position:absolute;background: transparent url("images/navBG.png") no-repeat fixed; top:636px;left:121px; width:161px;height:41px; background-position: 0px 0px;}
        .btnReturn{position:absolute;background: transparent url("images/navBG.png") no-repeat fixed; top:636px;left:121px; width:161px; height:41px; background-position: 0px -42px;}
    </style>
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            ...
            %>
            { focus:0, items : [
                {name:'play',style:'mask mask8',playType:-3,link:'http://192.168.42.52/epg/index/newEast.do'},
            ]},
        ...
        var columns = 8; rows = 1;              //定义行列，　几行几列
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    focus = focus - 1;
                     ...
                     if( blocked == 1  && ( index == 1 || index == -11 ) || blocked == 0 && focus == 0 && index == 11 ) return;
                    if( blocked == 0 ) {
                        if( index == -1 ) { blocked = focusable.length - 1; focus = 1; }
                        else if( index == 1 ) { blocked = 1; focus = 0; }
                        else if( index == 11 ) { focus -= 1; }
                        else { focus += 1; if( focus >= focusable[0].items.length ) { blocked = 1; focus = 0;} }
                    } else {
                        if( index == 11 ) { blocked = 0; focus = focusable[0].items.length - 1; }
                        else if( blocked == 1 && index == -1 ) { blocked = focusable.length - 1; focus = 1 ;}
                        else { blocked = 1; focus = 0; }
                    }
                }
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:undefined,popup:undefined,vote:undefined,video:undefined,position:undefined,gatewayCheck:undefined,external:{after:'<div style="position:absolute;width:246px;height:245px;left:523px;top:202px; background:transparent url(\'images/mask-2015-08-25.png\') no-repeat fixed 0px 0px;"></div>',before:undefined}});
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-08-25.jpg') no-repeat;" onUnload="exit();"></body>
</html>