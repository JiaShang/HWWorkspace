...
<%
    ...
    try {
        ...
        sc = StringUtils.isNotEmpty( request.getParameter( "sc" )) ? request.getParameter("sc").split("\\,") : new String[0];
...
<head>
    <title><%= column == null ? "一竖排的专题模板" : column.getName() %></title>
    <style>
    ...
    <script language="javascript" type="text/javascript">
            ...
            if( list != null && list.size() > 0){
                for(int i = 0; i < list.size() ; i++){
                    ...
                    for( int j = 0; j < vodList.size() ; j++ ){
                        ...
                        String[] names = vod.getName().split("：");
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked < focusable.length - 2  && (
                     ...
                     if( blocked == focusable.length - 1) { blocked = 0;focus = focusable[blocked].focus; }
                         ...
                         if( index == 1 ) { if( columns == 1 ) { if( <%=hm%> == 1 ) return; blocked = focusable.length - 1;focus = focusable[blocked].focus; } else focus += index; }
        ...
        function init(){
            ...
            if( typeof iPanel == "undefined" ) {
                document.onkeydown = function(event){
                    switch (event.keyCode) {
                        ...
                        case 8:
                        case 36:top.window.location.href = iPanel.eventFrame.portalUrl; return 0;break;
                    }
                }
            }
            ...
            $.buildUserInterface = function(option){
                ...
                <%if( sc.length >= 5 ){%>
                html += "<div id='scrollBarContainer' class='scrollBarContainer'><div id='scrollBar'></div></div>";
                <%}%>
            ...
            $.buildUserInterface({flowed:[0],popup:undefined,vote:undefined,video:undefined,position:undefined,external:{after:undefined,before:'<%= hm.equalsIgnoreCase("1") ? "" : "<div class=\\'backNormal\\'></div>"%>'}});
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('<%= Utils.pictureUrl("", column.getPosters(), "7", request) %>') no-repeat;" onUnload="exit();"></body>
</html>