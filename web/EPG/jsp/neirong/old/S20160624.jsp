...
<%
    ...
    final String[] types = {"10000100000000090000000000106403"};
    ...
    try {
        ...
        columns = getTypeList( metaHelper, types[0], position[0], position[1]);

        column = columns.get(0);
        List<Vod> list = getVodList( metaHelper, column.getId(), 1,0);
        if( list != null && list.size() > 0 ) vod = list.get(0);
...
<head>
    <title>欧洲杯小组赛</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2016-06-24.png') no-repeat 0px 0px;}

        .mask0{width:193px;height:49px;left:891px;top:301px;}
        ...
        .item .image,.item .maskImage{ width:200px;height:245px;left:0px;top:-230px;position:relative;}
        .item .image { background:transparent url('images/mask-TemplateRangle.png') no-repeat 0px 0px;}
        .item .maskImage{background:transparent url('images/mask-2016-06-24.png') no-repeat -5px -60px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,typeId:'<%= column.getId() %>',items : [{name:'十佳进球',style:'mask mask0'<%= vod == null ? "" : ",playType:" + vod.getIsSitcom() + ",mid:" + vod.getId() + ""%>}]},
            <%
            value = "";
            if( columns != null && columns.size() > 0){
                ...
                for( int i = 0; i < columns.size() ; i++ ){
                    ...
                    String str = "{ text:\"" + col.getName() + "\"," +
                             "length:" + StringUtil.length(col.getName()) + "," +
                          "shortText:\"" + StringUtil.limitStringLength(col.getName(), popItemMaxCharLength) + "\"" + "," +
                              "picture:'" + Utils.pictureUrl("images/default_preview_lb.png", col.getPosters(), "1", request)+ "'" + "," +
                           "playType:-3," +
                            "link:\"/EPG/jsp/defaultHD/en/iCatch_yulq_list.jsp?typeId=" + col.getId() + "&pageLength=8\"";
                        str += "}" ;
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                ...
                html += '<div class="text" ' + ( ( i == focus && blocked == index ) ? ' style="color:#57c71c;"' : '' ) + '>';
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-06-24.jpg') no-repeat;" onUnload="exit();"></body>
</html>