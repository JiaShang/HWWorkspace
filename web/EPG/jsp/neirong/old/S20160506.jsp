...
<%
    ...
    final String[] types = {"10000100000000090000000000106202"};
...
<head>
    ...
    <style>
        .mask {width:52px;height:47px;position:absolute;background:transparent url('images/mask-2016-05-06.png') no-repeat;background-position: 0px 0px;}
        .mask1{left:174px;top:169px;}
        .mask2{left:406px;top:172px;}
        .mask3{left:926px;top:170px;}
        .mask4{left:250px;top:404px;}
        .mask5{left:409px;top:400px;}
        .mask6{left:688px;top:403px;}
        .mask7{left:940px;top:404px;}
        .mask8{left:1152px;top:402px;}
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && ( index == 11 && focus <= 2 ||
                        index == -1 && ( focus == 0 || focus == 3 ) ||
                        index == 1 && ( focus == 2 || focus == focusable[0].items.length - 1 ) ) ||
                        index == -11 && ( focus > 2 && focus < focusable[0].items.length - 2 )) return;
                    if( blocked == focusable.length - 1  ) { blocked = 0; focus = focusable[blocked].focus;}
                    else {
                        if( index == 1 || index == -1 ) { focus += index; }
                        else if( index == -11 ) {
                            if( focus <= 1 ) focus += 3;
                            else focus = focusable[blocked].items.length - 2;
                            if( focus >= focusable[blocked].items.length  ) { blocked = focusable.length - 1; focus = 0;}
                        } else {
                            if( focus == 3 ) focus = 0 ;
                            else if( focus == 4 || focus == 5) focus = 1;
                            else focus = 2;
                        }
                    }
                }
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-05-06.jpg') no-repeat;" onUnload="exit();"></body>
</html>