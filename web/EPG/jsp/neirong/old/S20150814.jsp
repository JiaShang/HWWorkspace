...
<%
    String typeId = "10000100000000090000000000105380";
    final String[] types = {"10000100000000090000000000105385","10000100000000090000000000105386"};
    final int[] position = {8,0,3,0};
    //TODO:每次需要修改
    ...
    try {
        ...
        columns = getTypeList( metaHelper, typeId, 1, 6 );
        addColumn = columns.get(0);
        columns = getTypeList( metaHelper, typeId, 4, 0 );
        int i = 0;
...
<head>
    <title>抗战胜利70周年</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2015-08-14.png') no-repeat fixed;background-position: 0px 0px;}

        .mask1{width:221px;height:71px;left:90px;top:166px;background-position: -235px -500px;}
        .mask2{width:221px;height:71px;left:308px;top:166px;background-position: -235px -500px;}
        .mask3{width:221px;height:71px;left:526px;top:166px;background-position: -235px -500px;}
        .mask4{width:221px;height:71px;left:744px;top:166px;background-position: -235px -500px;}
        .mask5{width:221px;height:71px;left:962px;top:166px;background-position: -235px -500px;}

        .mask11{width:313px;height:51px;left:83px;top:253px;background-position: -235px -410px;}
        .mask12{width:313px;height:51px;left:83px;top:298px;background-position: -235px -410px;}
        .mask13{width:313px;height:51px;left:83px;top:343px;background-position: -235px -410px;}
        .mask14{width:313px;height:51px;left:83px;top:388px;background-position: -235px -410px;}
        .mask15{width:313px;height:51px;left:83px;top:434px;background-position: -235px -410px;}
        .mask16{width:313px;height:51px;left:83px;top:479px;background-position: -235px -410px;}
        .mask17{width:313px;height:51px;left:83px;top:524px;background-position: -235px -410px;}
        .mask18{width:313px;height:51px;left:83px;top:569px;background-position: -235px -410px;}

        .mask21{width:543px;height:401px;left:418px;top:246px;background-position: 0px 0px;}
        .mask22{width:235px;height:187px;left:972px;top:242px;background-position: 0px -401px;}
        .mask23{width:235px;height:187px;left:972px;top:432px;background-position: 0px -401px;}

        .image1{width:536px;height:395px;left:0px;top:2px;position: absolute;}
        .image2{width:223px;height:176px;left:556px;top:0px;position: absolute;}
        .image3{width:223px;height:176px;left:556px;top:190px;position: absolute;}

        .topTitle {width:218px;height:50px;line-height: 48px;font-weight:bold; font-size: 26px; color: #ffffff;float: left;text-align: center}
        .leftTitle{width:252px;height:45px;line-height: 45px;font-weight: bold;font-size: 22px;color:#52514E;text-align: left;}

        .topTitleText {line-height:70px; font-weight:bold; font-size: 26px; color: #ffffff;padding:0px; text-align: center;}
        .leftTitleText {height:45px;line-height: 45px;font-weight: bold;font-size: 22px;color:#ffffff;text-align: left; padding: 0px 10px 0px 48px;}
        .leftMarquee {width:252px;height:100%;height:45px;line-height: 45px;font-weight: bold;font-size: 22px;color:#ffffff;text-align: left;}
        .Empty {line-height:0px; font-weight:normal; font-size: 0px; color: #ffffff;padding:0px;}
        .title2,.title3 {position: absolute;left:979px;top:386px;text-align: center;}
        .title2,.title3,.titleMarquee {width:221px; height:36px;line-height:36px;font-size: 22px;color:#ffffff;}
        .title3 {top:576px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            ...
            if( columns != null && columns.size() > 0 ) {
                ...
                Column column = columns.get(i);
                    value += "{ style:'mask mask" + (i + 1) + "'" + "," +
                        "text:\"" + column.getName() + "\"," +
                        "playType:-3," +
                        "link:'/EPG/jsp/neirong/S20150814" + ( i < 2 ? "ImgList" : "TxtList" ) + ".jsp?typeId=" + column.getId() + "'},";
                }
                value += "{ style: 'mask mask5',text:'有 奖 竞 答',playType:-3,link:'/EPG/jsp/neirong/S20150814Act.jsp'}";
                value += "]},";
                out.write(value);
            }
            value = "";
            if( list != null && list.size() > 0){
                for(int i = 0; i < list.size() ; i++){
                    ...
                    for( int j = 0; j < vodList.size() ; j++ ){
                        String str = "";
                        if( i == 1 && j == 1 ) {
                            str += "{ text:'" + addColumn.getName() + "',shortText:'" + addColumn.getName() + "',playType:-3," + "style:'mask mask" + (i + 1) + (j + 1) + "'" + ",link:'/EPG/jsp/neirong/S20150814TxtList.jsp?typeId=" + addColumn.getId() + "',picture:'" + Utils.pictureUrl("images/default_preview_lb.png", addColumn.getPosters(), "0", request)+ "'},";
                        } else {
                            Vod vod = vodList.get(j);
                                str = "{ mid:'" + vod.getId() + "'," +
                                       "text:\"" + vod.getName() + "\"," +
                                     "length:" + StringUtil.length(vod.getName()) + "," +
                                  "shortText:\"" + StringUtil.limitStringLength(vod.getName(), i == 0 ? 20 : 16 ) + "\"" + "," +
                                      "style:'mask mask" + (i + 1) + (j + 1) + "'" + "," +
                    (i == 0 ? "" : ("picture:'" + Utils.pictureUrl("images/default_preview_lb.png", vod.getPosters(), "0", request) + "'" + "," )) +
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    focus = focus - 1;
                     ...
                     if( blocked == 0  && ( index == 11 || index == -1 && focus == 0 || index == 1 && focus + 1 >= focusable[0].items.length) ||
                         blocked == 1 && ( index == -1 || index == -11 && focus + 1 >= focusable[1].items.length ) ||
                         blocked == 2 && ( index == -11 && focus == 0 || index == 1 && focus > 0 ) ) return;
                     if( blocked == 0 ) {
                         if( index == 1 || index == -1 ) focus += index;
                         if( index == -11 ) {
                             if( focus < 2 ) { blocked = 1; focus = 0; }
                             else if( focus == 4 ) { blocked = 2 ; focus = 1; }
                             else { blocked = 2 ; focus = 0; }
                         }
                     } else if( blocked == 1 ) {
                         if( index == 11 || index == -11 ) {
                             if( index == 11 && focus == 0 ) { blocked = 0; focus = 0; }
                         ...
                         if( index == 11 ) {
                             ...
                             if( focus == 1 ) focus = 2;
                             else { blocked = focusable.length - 1; focus = focusable[blocked].focus; }
                         } else if( index == -1 ){
                             if( focus == 0 ) {blocked = 1; focus = focusable[blocked].focus; }
                             else focus = 0;
                         } else focus = 1;
            ...
            {
                if( blocked > 1 )$('mask').innerHTML = "";

                try {
                    $("Title2").innerHTML = focusable[2].items[1].shortText ;
                    $("Title3").innerHTML = focusable[2].items[2].shortText ;
                } catch (e) {}

                if( blocked == 0 || blocked == 1 ) {
                    $('mask').innerHTML = blocked == 1 && item.length > 20 ? ( '<marquee class="leftMarquee">' + item.text + "</marquee>" ) : item.text ;
                    $('mask').className = item.style + ( blocked == 0 ? " topTitleText" : " leftTitleText");
                } else {
                    $('mask').className = item.style + " Empty";
                    if( typeof item.length == 'number' && item.length > 16 && (item.style == 'mask mask22' || item.style == 'mask mask23')) {
                        $("Title" + ( focus + 1)).innerHTML = '<marquee class="titleMarquee">' + item.text + "</marquee>"
                    }
                }
            }
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:undefined,popup:undefined,vote:undefined,video:undefined,position:undefined,external:{
                after:'<div style="width:894px;height:200px;left: 76px;top:193px;position:absolute;background:transparent url(images/focus-2015-08-14-2.png) no-repeat"></div><div id="Title2" class="title2"></div><div id="Title3" class="title3"></div>',
                before:'<div id="ImageContainer" style="width:779px;height:397px;left:422px;top:247px;position:absolute;"></div><div style="width:788px;height:405px;left: 418px;top:242px;position:absolute;background:transparent url(images/focus-2015-08-14-1.png) no-repeat"></div><div id="TopContainer" style="width:1090px;height:50px;left:92px;top:183px;position:absolute;"></div><div id="TitleContainer" style="width:255px;height:366px;left:131px;top:254px;position:absolute;"></div>'}});
            ...
            for(;i< items.length; i++ )
                html += '<div class="topTitle">' + items[i].text + '</div>';
            $("TopContainer").innerHTML = html;

            items = focusable[1].items; html='';
            for(i=0; i< items.length; i++)
                html += '<div class="leftTitle">' + items[i].shortText + '</div>';
            $("TitleContainer").innerHTML = html;

            items = focusable[2].items; html='';
            for(i=0; i< items.length; i++){
                if( i != 0) $("Title" + ( i + 1)).innerText = items[i].shortText;
                html += '<img src="' + items[i].picture + '" class="image' + ( i + 1) + '" />';
            }
        ...
        function doEnter(){
            ...
            switch(item.playType){
                ...
                case -2:
                    ...
                    break;
                case -3:
                    top.window.location.href = $.current() + item.link + ( !$.isGoGatewayDetail && !iPanel.eventFrame.isEpg3 || item.link.indexOf('?') < 0 ? '' : '&isGateway=true');
                    break;
                case 1:
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-08-14.jpg') no-repeat;" onUnload="exit();"></body>
</html>