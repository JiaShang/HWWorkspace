...
<%
    ...
    final String[] types = {"10000100000000090000000000105462"};
...
<head>
    <title>迎开学</title>
    <style>
        .mask {width:182px;height:78px;position:absolute;background:transparent url('images/mask-2015-09-02.png') no-repeat fixed;background-position: 0px -410px;}
        .mask1 {left:487px;top:257px;}
        .mask2 {left:738px;top:223px;}
        .mask3 {left:989px;top:188px;}
        .mask4 {left:105px;top:432px;}
        .mask5 {left:354px;top:475px;}
        .mask6 {left:602px;top:517px;}
        .mask7 {left:851px;top:556px;}
        .mask8 {width:308px;height:87px;left:49px;top:578px;background-position: -230px -410px;}
        .flowed {}
    ...
    <script language="javascript" type="text/javascript">
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if(
                    ...
                    blocked == focusable.length - 1 && ( arrows[0] == 0 && index == 11 || index == -11 || focus == 1 && index == 1 ) ||
                    //如果焦点在投票上,按上下左右键时
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && ( focus == 3  && index == -1 || focus < 3 && index == 11 || focus > 3 && focus != focusable[0].items.length -1 && index == -11 || focus == 2 && index == 1 ) ||
                        blocked == 1 && ( index == -1 || index == -11 )) return;
                    if( index == -1 || index == 1 ) {
                        if( focus == 0 && index == -1 ){ focus = blocked == focusable.length - 1 ? 0 : 3 ;  blocked = blocked == focusable.length - 1 ? 1 : 0;}
                        else if( blocked == 1 && index == 1) { blocked = focusable.length - 1; focus = 0;}
                        else if (blocked == 0 && index == 1 && focus == focusable[0].items.length - 1 ){ blocked = focusable.length - 1; focus = 0;}
                        else  focus += index;
                    }
                    if( index == 11 ){
                        if( blocked == focusable.length -1 ) { blocked = 0; focus = focusable[0].items.length - 1; }
                        else if( blocked == 1 ) { blocked = 0; focus = 3; }
                        else if(focus == 3 ) focus = 0; else focus -= 4;
                    } else if( index == -11 ){
                        if( focus == focusable[0].items.length -1 ){ blocked = focusable.length - 1; focus = 0;}
                        else if( focus == 3 ) { blocked = 1; focus = 0; }
                        else focus += 4;
                    }
                }
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:undefined,popup:undefined,vote:undefined,video:undefined,position:undefined,gatewayCheck:undefined,external:{after:'<div style="width:1085px;height:401px;position: absolute;left:70px;top:208px;background:transparent url(images/mask-2015-09-02.png) no-repeat fixed 0px 0px;"></div>',before:undefined}});
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-09-02.jpg') no-repeat;" onUnload="exit();"></body>
</html>