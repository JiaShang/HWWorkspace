...
<head>
    <title>刺客信条信仰之跃</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2017-02-22.png') no-repeat;background-position: 0px 0px;}
        .mask1 {width:410px;height:260px;left:175px;top:221px;background-position:0px 0px;}
        .mask2 {width:410px;height:260px;left:695px;top:221px;background-position:0px 0px;}
        .mask3 {width:206px;height:86px;left:537px;top:572px;background-position:0px -270px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,items : [{name:'游戏入口',style:'mask mask3',playType:-3,link:'http://192.168.35.129:4567/CyberCloud_Dispatcher.htm?CyberEntryID=1&CyberSiteCode=CS00007&tag=ckxt'}]},
            <%
            value = "";
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 1 && ( index == -1 && focus == 0 || index == 1 && focus == 1 || index == 11 ) ||
                        blocked == 0 && index != 11 ) return;
                    if( blocked == 1 ) {
                        if( index == 1 || index == -1 ) {
                            focus += index;
                        } else {
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-02-22.jpg') no-repeat;" onUnload="exit();"></body>
</html>