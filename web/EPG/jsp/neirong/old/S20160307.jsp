...
<%
    ...
    final String[] types = {"10000100000000090000000000105944"};
    final int[] position = {6,0};
    //TODO:每次需要修改
...
<head>
    <title>2016两会</title>
    <style>
        .mask {width:200px;height:49px;position:absolute;background:transparent url('images/mask-2016-03-07.png') no-repeat fixed;background-position: 0px 0px;}
        .mask1{left:55px;top:181px;background-position: 0px 0px;}
        .mask2{left:1032px;top:143px;background-position: 0px 0px;}
        .mask3{left:1032px;top:226px;background-position: 0px 0px;}
        .mask4{width:47px;height:146px;left:982px;top:338px;background-position: 0px -50px;}
        .focusBg{width:1143px;height:327px;left:81px;top:148px;background:transparent url('images/focusBg-2016-03-07.png') no-repeat fixed 0px 0px;position:absolute;}

        .flowed {position: absolute;width:350px;height:315px;left:60px;top:240px;}
        .flowedItem {width:330px;height: 52px; line-height: 48px; float: left;overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;font-size: 24px;color:black;}
        .flowedItem .marqueed {width:330px;height: 52px;color: red;line-height: 48px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,items : [{name:'聚焦两会',style:'mask mask1',playType:-3,link:'/EPG/jsp/defaultHD/en/iCatch_yulq_list.jsp?typeId=10000100000000090000000000105941&pageLength=8'}]},
            <%
            value = "";
            ...
            %>
            { focus:0,items : [
                {name:'代表委员之声',style:'mask mask2',playType:-3,link:'/EPG/jsp/defaultHD/en/iCatch_yulq_list.jsp?typeId=10000100000000090000000000105942&pageLength=8'},
                {name:'重庆代表团',style:'mask mask3',playType:-3,link:'/EPG/jsp/defaultHD/en/iCatch_yulq_list.jsp?typeId=10000100000000090000000000105943&pageLength=8'},
                {name:'热点调查',style:'mask mask4',playType:-3,link:'/EPG/jsp/neirong/S20160307Rule.jsp'},
            ]},
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if(blocked == 0 && ( index == -1 || index == 11) ||
                       blocked == 1 && ( index == -1 || focus >= focusable[1].items.length - 1 && index == -11 ) ||
                       blocked == 2 && ( index == 1 || focus == 0 && index == 11 )
                    ) return;
                    if( blocked == focusable.length - 1 ) {
                        ...
                        focus += index > 1 ? -1 : 1;
                        if( index == 11 && blocked == 1 && focus < 0 ){
                            blocked = 0; focus = 0;
                        } else if(index == -11 ){
                            if( focus >= focusable[blocked].items.length ){
                                blocked = blocked == 2 ? focusable.length - 1 : 1; focus = 0;
                            }
                        }
                    } else if( index == -1){
                        if( focus <= 1 ) { blocked = 0;focus = 0; }
                        else {  blocked = 1; focus = focusable[blocked].focus; }
                    } else {
                        focus = blocked == 0 ? 0 : 2;blocked = 2;
                    }
                }
                ...
                if(typeof focusable.flowed != 'undefined' )  flowedShow( 1 );
            }
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                var item = items[i];
                html += '<div class="flowedItem"'
                if( i == focus && blocked == index ){
                    html += ' style="color:red;">'
                    if( item.length > 26 ) {
                      html +=  "<marquee class='marqueed'>" + item.text + "</marquee>";
                    } else
                        html += item.text;
                } else
                    html += '>' + item.shortText;
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:[1],popup:undefined,vote:undefined,video:undefined,position:undefined,gatewayCheck:undefined,external:{after:'<div class="focusBg"></div>',before:undefined}});
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-03-07.jpg') no-repeat;" onUnload="exit();"></body>
</html>