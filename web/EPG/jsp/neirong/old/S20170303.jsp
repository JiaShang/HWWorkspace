...
<%
    ...
    final String[] types = {"10000100000000090000000000107323","10000100000000090000000000107324","10000100000000090000000000107325","10000100000000090000000000107326"};
    final int[] position = {99,0,99,0,99,0,99,0};
    //TODO:每次需要修改
...
<head>
    <title>聚焦2017全国两会</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2017-03-03.png') no-repeat;background-position: 0px 0px;}
        .mask1{ width:234px;height:30px;left:957px;top:185px;background-position:0px 0px;}
        .mask2{ width:234px;height:30px;left:957px;top:284px;background-position:0px -30px;}
        .mask3{ width:234px;height:30px;left:957px;top:382px;background-position:0px -60px;}
        .mask4{ width:284px;height:30px;left:956px;top:488px;background-position:0px -90px;}

        .focusBg{width:1280px;height:720px;position: absolute;left:0px;top:0px;background:transparent url('images/focusBg-2017-03-03.png') no-repeat 0px 0px;}
        .focusBgTitle {width:112px;height:24px;top:130px;left:609px;}
        .title2 {background-position: 0px -120px;}
        .title3 {background-position: 0px -150px;}
        .title4 {background-position: 0px -180px;}

        .flowed{position:absolute;}
        .flowed1 {width:360px;top:220px;height:255px;left:540px;}
        .flowed2 {width:355px;top:163px;height:255px;left:550px;}

        .item1 {width:360px;height:51px;}
        .item2 {width:355px;height:51px;}

        .circle,.circleFocus {width:20px;height:51px;float: left;background:transparent url('images/mask-2017-03-03.png') no-repeat -150px -130px;}
        .circleFocus {background-position: -210px -130px;}
        .text{height:51px;line-height: 47px;font-size:20px;color:#000000;float:left;overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;}
        .text1,.textFocus1{width:340px;}
        .text2,.textFocus2{width:335px;}
        .textFocus1,.textFocus2{color:#023756;}
        .scrollBar{position:absolute;width:6px;}
        .scrollBar1{left:918px;}
        .scrollBar2{left:920px;}
        .scrollBarTop,.scrollBarBottom{width:6px;left:0px;height:3px; border:none;}
        .scrollBarTop {top:0px;background-position: -290px 0px;}
        .scrollBarMiddle{width:6px;left:0px;border:none;top:3px;background-position: -300px 0px; background-repeat: repeat-y;}
        .scrollBarBottom{bottom:0px;background-position: -290px -207px;}

        .marqueed{line-height: 47px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,items : [
                {name:'两会',style:'mask mask1'},
                {name:'两会声音',style:'mask mask2'},
                {name:'重庆发展',style:'mask mask3'},
                {name:'凤凰两会直通车',style:'mask mask4',playType:-3,link:'/EPG/jsp/defaultHD/en/ifengnew/Category.jsp?url=ifeng_index.jsp'}
            ]},
            <%
            value = "";
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( index == -1 && blocked != 0 || index == 1 && blocked != 1 || index == 11 && focus == 0 || index == -11 && focus + 1 >= focusable[blocked].items.length ) return;
                    if( index == -1 ) { blocked = 1; focus = focusable[blocked].focus; }
                    else if( index == 1 ) { blocked = 0; focus = focusable[blocked].focus; }
            ...
            flowedShow(blocked == 0 ? 1 : blocked);
        }
        var flowedShow = function(index){
            ...
            var pageCount = index == 1 ? 5 : 7;

            $("flowed").className = "flowed flowed" + (index == 1 ? 1 : 2);
            $("focusBg").style.visibility = index == 1 ? "hidden" : "visible";
            if( index > 1 ) {
                $("focusBgTitle").className = "mask focusBgTitle title" + index;
            }
            ...
            if( items.length <= pageCount ) {
                ...
                $("scrollBar").className = "scrollBar scrollBar" + (index == 1 ? 1 : 2);

                var scrollBarHeight = index == 1 ? 233 : 312;
                var scrollTop = index == 1 ? 229 : 190;
                var h = Math.ceil( pageCount * 1.0 / items.length * scrollBarHeight);
                var le = scrollBarHeight - h;
                $("scrollBar").style.height = h + "px";
                $("scrollBarMiddle").style.height = ( h - 6 < 6 ? 6  : h - 6) + "px";

                $("scrollBar").style.top = Math.ceil( focus * 1.0 / (items.length - 1) * le  + scrollTop ) + "px";
            }

            var html = '';
            var length = items.length - flowCursorIndex >= pageCount ? pageCount : items.length - flowCursorIndex;
            var stemp = index == 1 ? 1 : 2;
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                var item = items[i];
                html += "<div class='item" + stemp + "'>";
                html += "<div class='circle" + ( ( i == focus && blocked == index ) ? 'Focus' : '' ) + "'></div>";
                html += "<div class='text text" + ( ( i == focus && blocked == index ) ? 'Focus' : '' ) + stemp + "'>"
                html += (i == focus && blocked == index ) ? (item.shortText == item.text ? item.text : ( "<marquee class='marqueed'>" + item.text + "</marquee>") ) : item.shortText ;
                html += "</div></div>";
            }
        ...
        function init(){
            ...
            $.buildUserInterface = function(option){
                ...
                html += "<div id='focusBg' class='focusBg' style='visibility: hidden;'><div id='focusBgTitle'></div></div>";
                ...
                html += "<div id='scrollBar'><div class='mask scrollBarTop'></div><div id='scrollBarMiddle' class='mask scrollBarMiddle'></div><div class='mask scrollBarBottom'></div></div>"
            ...
            $.buildUserInterface({flowed:[1,2,3,4],popup:undefined,vote:undefined,video:undefined,position:undefined,external:{after:undefined,before:undefined}}); //'<div class="after"></div>'
            if( blocked > 1 ) focusable[0].focus = blocked - 2;
        ...
        function doEnter(){
            ...
            if( blocked == 0 && typeof item.playType == 'undefined'){
                blocked = focusable[0].focus + 2;
                focused(focusable[blocked].focus, true);
                return;
            }
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-03-03.jpg') no-repeat;" onUnload="exit();"></body>
</html>