...
<%
    ...
    final String[] types = {"10000100000000090000000000106649"};
...
<head>
    <title>中秋合家乐</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2016-09-01.png') no-repeat;background-position: 0px 0px;}

        .border {width:193px;height:257px;top:18px; background-position: 0px 0px;}
        .border1 {left:117px;}
        .border2 {left:329px;}
        .border3 {left:542px;}
        .border4 {left:755px;}
        ...
        .image {width:192px;height:249px;top:0px;position:absolute;overflow: hidden;border: none;}
        .image1 {left:21px;}
        .image2 {left:233px;}
        .image3 {left:446px;}
        .image4 {left:659px;}
        ...
        .focusItem {width:229px;height:293px;top:0px; background-position: -200px 0px;}
        .item1 {left:99px;}
        .item2 {left:311px;}
        .item3 {left:524px;}
        .item4 {left:736px;}
        ...
        .before {width:1100px;height:249px;top:319px;left:90px;position:absolute;overflow: hidden;}
        .flowed {width:1280px;height:293px;top:297px;position:absolute; overflow: hidden;background:transparent url('images/focusBg-2016-09-01.png') no-repeat 111px 15px;}
    ...
    <script language="javascript" type="text/javascript">
            ...
            if( list != null && list.size() > 0){
                for(int i = 0; i < list.size() ; i++){
                    ...
                    for( int j = 0; j < vodList.size() ; j++ ){
                        ...
                        String str = "";
                        if( j == 0 ) str += "{name:'我们诞生在中国',playType:-3,link:'http://192.168.5.229/dyyx/search_content.htm?vodId=PACKAGE201608000039-gehua',picture:'images/item-2016-09-01-1.jpg'},";
                        str += "{ mid:'" + vod.getId() + "'," +
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && ( index == 11 || focus % pageCount != ( pageCount - 1 ) && index == -11 || focus == 0 && index == -1 )) return;
                    if( blocked == focusable.length - 1 ) {
                        ...
                        if( index == -1 || index == 1 ){
                            focus += index;
                            if( focus >= focusable[0].items.length ){
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                var item = items[i];
                image += '<img src="' + item.picture + '" class="image image' + ( i - flowCursorIndex + 1) + '" />';
                if( i == focus && blocked == index ){
                    html += '<div class="mask focusItem item' + ( i - flowCursorIndex + 1) + '" ></div>';
                } else {
                    html += '<div class="mask border border' + ( i - flowCursorIndex + 1) + '" ></div>';
                }
            }
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:[0],popup:undefined,vote:undefined,video:undefined,position:undefined,external:{after:undefined,before:'<div class="before" id="imageContainer"></div>'}}); //
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-09-01.jpg') no-repeat;" onUnload="exit();"></body>
</html>