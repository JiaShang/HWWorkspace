...
<%
    ...
    if(StringUtils.isEmpty(typeId)) typeId = "10000100000000090000000000107189";
...
<head>
    ...
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2017-10-31-Review.png') no-repeat;background-position: 0px 0px;}

        .flowed {width:1110px;height:594px;position:absolute;overflow: hidden;top:166px;left:85px;}
        .item {width:368px;height:247px;float: left;position: relative;}
        .itemImg {width:348px;height:231px;position:absolute;left:1px;top:1px;overflow: hidden;}
        .itemImg img {width:348px;height:231px;}
        .itemMask {width:350px;height:233px;left:0px;top:0px;overflow: hidden;background-position: 0px 0px;}
        .maskItem {width:368px;height:250px;background-position: -400px 0px; }

        .scrollBar{position:absolute;width:5px;left:1215px;}
        .scrollBarTop,.scrollBarBottom{width:5px;left:0px;height:4px; border:none;}
        .scrollBarTop {top:0px;background-position: -800px 0px;}
        .scrollBarMiddle{width:5px;left:0px;border:none;top:4px;background-position: -800px -4px; background-repeat: repeat-y;}
        .scrollBarBottom{bottom:0px;background-position: -800px -246px;}

        .mask1 {left:76px;top:158px;}
        .mask2 {left:444px;top:158px;}
        .mask3 {left:812px;top:158px;}
        .mask4 {left:76px;top:405px;}
        .mask5 {left:444px;top:405px;}
        .mask6 {left:812px;top:405px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        function focused(index,initilized){
            ...
            flowedShow(0, previous);
        }
        var flowedShow = function(index, previous){
            if( typeof index == 'undefined' ) return;
            var focus = focusable[index].focus;
            var items = focusable[index].items;
            var currentPage = Math.floor(focus / pageCount);
            var previousPage = Math.floor(previous / pageCount);
            if( currentPage != previousPage || $("flowed").innerHTML == '' ) {
                //每页显示数量
                ...
                for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                    var item = items[i];
                    html += '<div class="item">';
                    html += '<div class="itemImg"><img src="' + item.picture + '" /></div>';
                    html += '<div class="mask itemMask"></div>';
                    html += '</div>';
                }
                ...
                if( items.length < pageCount ) {
                    ...
                    var pageTotal = Math.ceil( items.length / pageCount);
                    var scrollBarHeight = 482;
                    //每一页的高度为 h
                    var heightPer = Math.floor( scrollBarHeight / pageTotal );
                    $("scrollBar").style.height = heightPer + "px";
                    $("scrollBarMiddle").style.height = ( heightPer - 8 < 8 ? 8  : heightPer - 8) + "px";
                    $("scrollBar").style.top = Math.floor( currentPage * heightPer   + 166 ) + "px";
                }
            }
            $("mask").className = "mask maskItem mask" + (focus % pageCount + 1);
        }
        ...
        function init(){
            ...
            $.buildUserInterface = function(option){
                ...
                html += "<div id='scrollBar' class='scrollBar' style='visibility: hidden'><div class='mask scrollBarTop'></div><div id='scrollBarMiddle' class='mask scrollBarMiddle'></div><div class='mask scrollBarBottom'></div></div>";
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-10-31-Review.jpg') no-repeat;" onUnload="exit();"></body>
</html>