...
<%
    ...
    final String[] types = {"10000100000000090000000000108461"};
    ...
    try {
        ...
        if(null != focus ){
            ...
            focus = new String[]{"0","0","0","2","3","4","5","6","7"};
        }
        ...
        String jsFile = request.getSession().getServletContext().getRealPath("jsp/defaultHD/en/hddb/hddb_topic.txt");
        try {
            JSONObject object = (JSONObject)  parser.parse(new InputStreamReader(new FileInputStream(jsFile), "GBK"));
            JSONArray array = (JSONArray) object.get("data");
            Iterator<?> iterator = array.iterator();
            while (iterator.hasNext()) {
                JSONObject current =  (JSONObject)iterator.next();
                hashtable.put(String.valueOf(current.get("name")),String.valueOf(current.get("url")));
            }
        } catch (Throwable e) {
            e.printStackTrace();
        }
...
<head>
    ...
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2017-09-29.png') no-repeat;background-position: -800px 0px;}
        .group {width:153px;height:214px;top:197px;}

        .groupImage {width:153px;height:210px;left:0px;top:2px;background-position: 0px 0px; overflow: hidden;}
        .groupImage img {width:153px;height:210px;}
        .groupMask,.groupMaskFocus {width:153px;height:214px;left:0px;top:0px;background-position: 0px 0px;}

        .groupMaskFocus {background-position: -180px 0px;}
        ...
        .maskItem {width:211px;height:102px;background-position:-500px 0px;}
        .mask1 {left:-14px;top:418px;}
        .mask2 {left:-14px;top:463px;}

        .mask3 {left:160px;top:418px;}
        .mask4 {left:334px;top:418px;}
        .mask5 {left:508px;top:418px;}
        .mask6 {left:681px;top:418px;}

        .mask7 {left:855px;top:418px;}

        .mask8 {left:1029px;top:418px;}
        .mask9 {left:1029px;top:463px;}

        .flowed {}
        .after {width:1187px;height:59px;left:50px;top:429px;position:absolute;overflow: hidden;background:transparent url('images/mask-2017-09-29.png') no-repeat;background-position: 0px -220px;}
    ...
    <script language="javascript" type="text/javascript">
                ...
                for(int i = 0; i < list.size() ; i++){
                    ...
                    for( int j = 0; j < vodList.size() ; j++ ){
                        ...
                        String str = "{ mid:'" + vod.getId() + "'," +
                                   "text:\"" + vod.getName() + "\"," +
                                 "style:'mask maskItem mask" + (j + 1) + "'" + "," +
                                 "picture:'" + Utils.pictureUrl("images/default_preview_lb.png", vod.getPosters(), "1", request)+ "'" + "," +
                               "playType:-3,link:'" + (hashtable.containsKey(vod.getName()) ? hashtable.get(vod.getName()) : "") + "'";
                            str += "}" ;
        ...
        var consign = [[0,2,<%=focus[2]%>],[2,1,<%=focus[3]%>],[3,1,<%=focus[4]%>],[4,1,<%=focus[5]%>],[5,1,<%=focus[6]%>],[6,1,<%=focus[7]%>],[7,2,<%=focus[8]%>]];                //用来缓存数据的
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && !$.vote.tooltiped && blocked == focusable.length - 1 && focus == 1 ) focus = focus + 1;
                    ...
                    ix = parseFocus(focus);
                    if( index == -1 && ix <= 0 || index == 1 && ix + 1 >= consign.length || index == -11 && focus + 1 >= consign[ix][0] + consign[ix][1] || index == 11 && focus - 1 < consign[ix][0]) return;
                    if( index == 11 || index == -11 ){
                        focus += index > 0 ? -1 : 1;
                    } else {
                        consign[ix][2] = focus;
                        $("groupMask" + (ix+1)).className = "mask groupMask";
                        ix += index;
                        focus = consign[ix][2];
                        $("groupMask" + (ix+1)).className = "mask groupMaskFocus";
                    }
                }
                focusable[blocked].focus = focus;
            } else if( initilized ){
                for(var i = 0; i < consign.length; i++)
                {
                    $("groupImage" + (i+1)).innerHTML = '<img src="' + focusable[0].items[consign[i][2]].picture + '" />';
                    $("groupMask" + (i+1)).className = "mask groupMask" + ( parseFocus(focus, i) ? "Focus" : "");
                }
            ...
            $("groupImage" + (ix+1)).innerHTML = '<img src="' + item.picture + '" />';

    }
        var parseFocus = function(current, index){
            if( typeof index == 'undefined' ) {
                var cu = 0;
                for( var i = 0; i < consign.length ; i ++ ){
                    if( current < consign[i][0] || current >= consign[i][0] + consign[i][1] ) continue;
                    cu = i;break;
                }
                return cu;
            } else {
                if( current < consign[index][0] || current >= consign[index][0] + consign[index][1] ) return false;
                return true;
            }
        }
        ...
        function init(){
            ...
            $.vote.focus = {}
            $.current = function (){
                var fcu = blocked + "," + focusable[blocked].focus;
                for( var i = 0; i < consign.length; i ++ ) {
                    fcu += "," + consign[i][2];
                }
                return "/EPG/jsp/defaultHD/en/SaveCurrFocus.jsp?currFoucs=" + fcu + "&url=";
            };
            $.buildUserInterface = function(option){
                var html = "";
                if(typeof option.external.before == 'string') html += option.external.before;
                for(var i = 0; i < consign.length; i ++ ){
                    html += '<div id="group' + (i+1) + "\" class='mask group group" +  (i+1)+ "'>";
                    html += "<div id='groupImage" + (i+1) + "' class='mask groupImage'></div>";
                    html += "<div id='groupMask" + (i+1) + "'></div>";
                    html += "</div>";
                }
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-09-29.jpg') no-repeat;" onUnload="exit();"></body>
</html>