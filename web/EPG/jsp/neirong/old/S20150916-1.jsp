...
<%
    ...
    final String[] types = {"10000100000000090000000000105497"};
    final int[] position = {100,0};
    //TODO:每次需要修改
    int focused = 0, area = 0;
    final int popItemMaxCharLength = 42;
...
<head>
    <title>金婚弹出页面改</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2015-09-16-1.png') no-repeat fixed;background-position: 0px 0px;}

        .flowed {position:absolute;width:681px;height:372px;left:284px;top:190px;}
        .flowedItem,.flowedMaskitem{width:679px;height:46px;line-height: 42px;color:#55555;font-size:25px;padding: 0px 30px;}
        .flowedMaskitem{color:#ffffff;background:transparent url('images/mask-2015-09-16-1.png') no-repeat fixed -1px -3px;}
        .flowedMarqueed {font-size: 25px; height: 42px; width: 580px;color: #ffffff;line-height:42px;}
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                ...
                if( i == focus && blocked == index ) {
                    html += "<div class='flowedMaskitem'>";
                    if ( item.length > 42   ) {
                        html += "<marquee class='flowedMarqueed'>" + item.text + "</marquee>"
                    } else {
                        html += item.shortText;
                    }
                    html += "</div>";
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-09-16-1.jpg') no-repeat;" onUnload="exit();"></body>
</html>