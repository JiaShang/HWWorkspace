...
<%
    ...
    final String[] types = {"10000100000000090000000000108106","10000100000000090000000000108107"};
    if( !StringUtils.isEmpty(typeId))
        types[0] = typeId;

    final int[] position = {20,0,20,0};
    //TODO:每次需要修改
    int focused = 0, area = 0;
    int popItemMaxCharLength = 18;
    ...
    try {
        ...
        w = !StringUtils.isNumeric( request.getParameter("w") ) ? 237 : Integer.valueOf(request.getParameter("w"));
        h = !StringUtils.isNumeric( request.getParameter("h") ) ? 73 : Integer.valueOf(request.getParameter("h"));
        ih = !StringUtils.isNumeric( request.getParameter("ih") ) ? 40 : Integer.valueOf(request.getParameter("ih"));
        fs = !StringUtils.isNumeric( request.getParameter("fs") ) ? 22 : Integer.valueOf(request.getParameter("fs"));
        mr = !StringUtils.isNumeric( request.getParameter("mr") ) ? 8 : Integer.valueOf(request.getParameter("mr"));

        lt = StringUtils.isNumeric( request.getParameter("lft")) ? Integer.valueOf(request.getParameter("lft")) : ( StringUtils.isNumeric( request.getParameter("lt")) ? Integer.valueOf(request.getParameter("lt")) : 73 );
        tp = !StringUtils.isNumeric( request.getParameter("tp") ) ? 353 : Integer.valueOf(request.getParameter("tp"));
        cl = StringUtils.isEmpty( request.getParameter("cl") ) ? "ffffff" : request.getParameter("cl");
        fc = StringUtils.isEmpty( request.getParameter("fc") ) ? "ffffff" : request.getParameter("fc");
        bc = StringUtils.isEmpty( request.getParameter("bc") ) ? "F29B87" : request.getParameter("bc");
        bg = StringUtils.isEmpty( request.getParameter("bg") ) ? "transparent" : request.getParameter("bg");
        al = StringUtils.isEmpty( request.getParameter("al") ) ? "0" : request.getParameter("al");
        hm = StringUtils.isEmpty( request.getParameter("hm") ) ? "0" : "1";
        ...
        al = al.equalsIgnoreCase("0") ? "left" : (al.equalsIgnoreCase("1") ? "center" : "right");
        ...
        pg = StringUtils.isEmpty( request.getParameter("pg") ) ? list.get(0).size() : Integer.valueOf( request.getParameter("pg") );

        h = pg * (ih + mr);
        popItemMaxCharLength = (int)Math.floor( ( w - 20 ) / fs / 1.15 ) * 2;
    } catch (Exception e){}
%>
<html>
<meta name="page-view-size" content="1280*720">
<head>
    <title><%= column == null ? "唐晶穿搭指南" : column.getName() %></title>
    <style>
        .mask {width:186px;height:47px;top:251px;position:absolute;position:absolute;background:transparent url('images/mask-2017-07-28.png') no-repeat;background-position: 0px 0px;}

        .block1 {left:360px;background-position: 0px 0px;}
        .block2 {left:528px;background-position: 0px -50px;}
        ...
        .focusItem .container .marquee,.item .container .inner,.focusItem .container .inner{width:<%= w-20 %>px; height:<%= ih%>px;font-size:<%=fs %>px;line-height:<%= ih-6 %>px; overflow:hidden;}
        ...
        .focusItem .marquee{margin:0px 0px 0px 10px;}
        .after {}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            ...
            if( list != null && list.size() > 0){
                for(int i = 0; i < list.size() ; i++){
                    value += "{focus:" + ( focus == null || focus.length >= i ? "0" : focus[i + 1] )  + ",typeId:'" + types[i] + "',items:[";
        ...
        var columns = 1; rows = <%= pg %>;              //定义行列，　几行几列
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if(
                    ...
                    blocked == focusable.length - 1 && ( <%=lt%> > 750 && focus == 0 && index == -1 || <%=lt%> <= 750 && index == 11 || index == -11 || focus == 1 && index == 1 ) ||
                    //如果焦点在投票上,按上下左右键时
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked < focusable.length - 2  && (
                         ...
                         index == -1 && blocked == 0 || //按左光标键时
                         index == 1 && blocked == 1 || //按右光标键时
                         index == -11 && <%=lt%> <= 750 && focus + columns >= focusable[blocked].items.length  //按下光标键时   && focus + columns != focusable[blocked].items.length (最后一个未对齐时)
                     ) ) return;
        ...
        var flowedShow = function(index){
            ...
            $("after").className = "mask block" + (index + 1);
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                var item = items[i];
                html += '<div class="' + ( ( i == focus && blocked == index ) ? 'focusItem' : 'item' ) + '"><div class="container">';
                if( i == focus && blocked == index && item.shortText != item.text){
                    html += '<marquee class="marquee">' + item.text + '</marquee>';
                } else {
                    html += '<div class="inner">' + item.shortText + '</div>';
                }
                html +='</div></div>';
            }
        ...
        function init(){
            ...
            $.vote.focus = {}
            $.current = function (){
                return "/EPG/jsp/defaultHD/en/SaveCurrFocus.jsp?currFoucs=" + blocked + "," + focusable[0].focus + "," + focusable[1].focus + "&url=";
            };
            ...
            $.buildUserInterface({flowed:[0],popup:undefined,vote:undefined,video:undefined,position:undefined,external:{after:'<div id="after"></div>',before:'<%= hm.equalsIgnoreCase("1") ? "" : "<div class=\\'backNormal\\'></div>"%>'}});
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-07-28.jpg') no-repeat;" onUnload="exit();"></body>
</html>