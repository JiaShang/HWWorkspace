...
<%
    ...
    final String[] types = {"10000100000000090000000000107561","10000100000000090000000000107561"};
    final int[] position = {4,0,4,4};
    //TODO:每次需要修改
    int area = 0;
...
<head>
    <title>木乃伊：卷土重来</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2017-06-09.png') no-repeat;background-position: 0px 0px;}
        .mask1 {width:230px;height:25px;left:144px;}
        .mask11 {top:398px;background-position:0px 0px;}
        .mask12 {top:459px;background-position:0px -50px;}
        .mask13 {top:518px;background-position:0px -100px;}
        .mask14 {top:579px;background-position:0px -150px;}

        .mask2 {width:161px;height:234px;top:410px;}
        .mask21 {left:474px;background-position: -250px 0px;}
        .mask22 {left:653px;background-position: -410px 0px;}
        .mask23 {left:835px;background-position: -570px 0px;}
        .mask24 {left:1018px;background-position: -730px 0px;}
                ...
                for(int i = 0; i < list.size() ; i++){
                    ...
                    for( int j = 0; j < vodList.size() ; j++ ){
                        ...
                        String str = "{ mid:'" + vod.getId() + "'," +
                                   "style:'mask mask" + ( i + 1 ) + " mask" + (i + 1) + "" + ( j + 1) + "'" + "," +
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && !$.vote.tooltiped && blocked == focusable.length - 1 && focus == 1 ) focus = focus + 1;
                    ...
                    if( blocked == 0 && ( index == -1 || index == 11 && focus <= 0 || index == -11 && focus + 1 >= focusable[blocked].items.length ) ||
                        blocked == 1 && ( index == 11 || index == -11 || index == 1 && focus + 1 >= focusable[blocked].items.length )) return;
                    if( blocked == 0 ) {
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-06-09.jpg') no-repeat;" onUnload="exit();"></body>
</html>