...
<%
    ...
    final String[] types = {"10000100000000090000000000105228"};
...
<head>
    <title>完美表姐养成记</title>
    <style>
        .mask {width:424px;height:41px;position:absolute;background:transparent url('images/mask-2015-06-18.png') no-repeat fixed;background-position: 0px 0px;}
        .mask1{left:91px;top:47px;background-position: 0px 0px;}
        .mask2{left:91px;top:83px;background-position: 0px -50px;}
        .mask3{left:91px;top:119px;background-position: 0px -100px;}
        .mask4{left:91px;top:158px;background-position: 0px -150px;}
        .mask5{left:91px;top:194px;background-position: 0px -200px;}
        .mask6{left:91px;top:230px;background-position: 0px -250px;}
        .mask7{left:91px;top:268px;background-position: 0px -300px;}
        .mask8{left:91px;top:304px;background-position: 0px -350px;}
        .mask9{left:812px;top:448px;background-position: 0px -400px;}
        .mask10{left:812px;top:487px;background-position: 0px -450px;}
        .flowed {}
        ...
        var focusable = focusable ? focusable : [
           ...
           if( list != null && list.size() > 0){
                for(int i = 0; i < list.size() ; i++){
                    ...
                    value += ",{text:'',playType:-3,link:'/EPG/jsp/defaultHD/en/iCatch_yulq_list.jsp?typeId=10000100000000090000000000100898&pageLength=8',style:'mask mask10'}]},";
                }
                out.write(value);
            }
            %>
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked === 0 && ( ( focus === 0 || focus === 8 ) && index === 11 ||
                            focus <= 7 && index === -1 || focus >=8 && index === 1 ||
                            focus === 7  && index === -11 )
                    )  return;
                    if( blocked === focusable.length - 1) {
                        blocked = 0; focus = focusable[blocked].items.length - 1;
                    } else {
                        if( Math.abs( index ) == 11 ) {
                            focus += index > 0 ? -1 : 1;
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-06-18.jpg') no-repeat;" onUnload="exit();"></body>
</html>