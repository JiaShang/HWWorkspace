...
<head>
    <title>48小时</title>
    <style>
        .focus,.normal {width:259px;height:172px;left:0px;top:0px;position:absolute;background:transparent url('images/mask-2016-11-26.png') no-repeat;overflow: hidden;}

        .flowed {width:1150px;height:420px;left:75px;top:216px;position:absolute;}
        .item {width:259px;height:172px;float: left;margin:16px 12px 16px 12px;position:relative;}
        .normal {background-position: 0px 0px;}
        .focus {background-position: 0px -180px;}
        .item img {position:absolute;width:253px;height:168px;top:2px;left:3px;}
        .text {width:250px;height: 23px;position:absolute;left:5px;color:white;text-align: center;overflow: hidden;font-size:18px;line-height: 21px;}
        .name {top:146px;}
        .author{top:176px;}
        .after {}
    ...
    <script language="javascript" type="text/javascript">
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    focus = focus - 1;
                     ...
                     var len = focusable[0].items.length;
                     if( blocked == 0 && ( index == 11 && focus % pageCount <= 3 || index == -11 && focus % pageCount >= 4 && focus % pageCount != pageCount - 1 && ( focus + len % 4 + 1 < len ) || index == -1 && ( focus == 0 || focus == 4 ) || index == 1 && focus + 1 >= len  ) ) return;
                     //TODO:当光标在首页、返回按钮上时，可同时按上，左键返回到列目中时，需要重写光标移动代码
                     if( blocked == focusable.length - 1) { blocked = 0;focus = focusable[blocked].focus; }
                     else {
                         if( index ==  -1 ){
                             if( focus % 4 == 0 && ( ( focus - focus % 8 == 0 ? 1 : 5 ) <= 0  ) ) return;
                             else if( focus % 4 == 0 ) focus -= 5;
                             else focus -= 1;
                         } else if ( index == 1 ) {
                             if( focus % 4 == 3 && ( Math.floor(focus * 1.0 / pageCount) != Math.floor( ( focus + ( focus % 8 == 7 ? 1 : 5 ) ) / pageCount ))){
                                 focus += focus + 5 >= focusable[blocked].items.length ? 1 : 5;
                             } else focus += 1;
                         } else if( index == 11 ) focus -= columns;
                         else if( index == -11 ) {
                             if( focus % pageCount == pageCount - 1 || focus + len % pageCount >= len ) {
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                ...
                html += "<div class='" + ( i == focus && blocked == index ? "focus" : "normal" )  + "'></div>";
                html += "<div class='text name'>" + item.text + "</div>";
                html += "<div class='text author'>"  + item.text + "</div>";
                html += "</div>";
            }
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-11-26.jpg') no-repeat;" onUnload="exit();"></body>
</html>