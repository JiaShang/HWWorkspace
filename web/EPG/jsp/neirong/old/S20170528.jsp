...
<%
    ...
    final String[] types = {"10000100000000090000000000107188"};
    ...
    try {
        ...
        kd = StringUtils.isEmpty(request.getParameter("playType"))? "0": request.getParameter("playType");
...
<head>
    ...
    <style>
        .mask1 {width:229px;height:125px;left:95px;top:47px;position:absolute;background: transparent url("images/mask-2017-05-28.png") no-repeat left top;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,items : [{name:'活动细则',style:'mask1',playType:-3,link:'http://192.168.98.65:10004/IPTVGameEPG/index.jsp?recoCode=LTR_10255'}]},
            <%
            value = "";
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && !$.vote.tooltiped && blocked == focusable.length - 1 && focus == 1 ) focus = focus + 1;
                    ...
                    if( blocked == 0 && index != -11 || blocked == 1 && ( index == -11 || index == -1 && focus <= 0 || index == 1 && focus + 1 >= focusable[blocked].items.length ) ) return;
                    if( index == 11 || index == -11) {
                        blocked = index == 11 ? 0 : 1; focus = focusable[blocked].focus;
                    } else {
                        focus += index;
                    }
                }
        ...
        function init(){
            ...
            $.vote.focus = {}
            $.current = function (){
                return "/EPG/jsp/defaultHD/en/SaveCurrFocus.jsp?currFoucs=" + blocked + "," + focusable[1].focus + "&url=";
            };
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-05-28.jpg') no-repeat;" onUnload="exit();"></body>
</html>