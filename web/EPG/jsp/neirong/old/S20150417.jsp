...
<%
    ...
    final String[] types = {"10000100000000090000000000105060"};
...
<head>
    <title>“先生”夺人</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2015-04-17.png') no-repeat fixed;background-position: 0px 0px;}

        .mask1{width:77px;height:26px;left:591px;top:386px;background-position:0px 0px;}
        .mask2{width:77px;height:26px;left:591px;top:426px;background-position:0px -40px;}
        .mask3{width:102px;height:26px;left:591px;top:466px;background-position:0px -80px;}
        .mask4{width:26px;height:26px;left:591px;top:506px;background-position:0px -120px;}
        .mask5{width:128px;height:26px;left:771px;top:386px;background-position:0px -160px;}
        .mask6{width:153px;height:26px;left:771px;top:426px;background-position:0px -200px;}
        .mask7{width:127px;height:26px;left:771px;top:466px;background-position:0px -240px;}
        .mask8{width:77px;height:26px;left:771px;top:505px;background-position:0px -280px;}
        .mask9{width:129px;height:24px;left:519px;top:606px;background-position:0px -320px;}
        .mask10{width:129px;height:24px;left:641px;top:606px;background-position:0px -360px;}
        .mask11{width:129px;height:24px;left:760px;top:606px;background-position:0px -400px;}
        .mask12{width:129px;height:24px;left:884px;top:606px;background-position:0px -440px;}
        .mask13{width:129px;height:24px;left:528px;top:630px;background-position:0px -480px;}
        .mask14{width:129px;height:24px;left:650px;top:630px;background-position:0px -520px;}
        .mask15{width:129px;height:24px;left:769px;top:630px;background-position:0px -560px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            ...
            {
                focus:0, items:[
                {text:"A. 青春偶像类",playType:-2,style:"mask mask9"},
                {text:"B. 虐恋苦爱类",playType:-2,style:"mask mask10"},
                {text:"C. 轻松搞笑类",playType:-2,style:"mask mask11"},
                {text:"D. 家庭伦理类",playType:-2,style:"mask mask12"},
                {text:"E. 超能变异类",playType:-2,style:"mask mask13"},
                {text:"F. 侦探破案类",playType:-2,style:"mask mask14"},
                {text:"G. 其它类",playType:-2,style:"mask mask15"}
            ]},
        ...
        var consign = undefined;
        var isKorean = true;                   //是否为韩剧
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == focusable.length - 1 ){
                        ...
                        if( blocked == 0 && ( index == 11 && (focus <= 0 || focus == 4 )  || index == -1 && focus < 4) || blocked == 1 && ( ( focus <= 0 && focus == 4 ) && index == -1 || focus >= 4 && index == -11 ) ) return;
                        if( blocked == 0 ) {
                            if( index == -11 ) { if(  focus == 3 || focus + 1 == focusable[blocked].items.length ) { blocked = 1; focus = focusable[blocked].focus; } else focus += 1; }
                            if( index == 11 ) focus -= 1;
                            if( index == -1 ) focus -= 4;
                            if( index == 1 ){
                                focus += 4 ;
                                if( focus > focusable[blocked].items.length ){ consign = blocked; blocked = focusable.length - 1; focus = focus = focusable[blocked].focus; }
                            }
                        } else {
                            if( index == 1 ){
                                focus += 1;
                                if( focus == 4 || focus == 7 ) { consign = blocked; blocked = focusable.length - 1; focus =  focusable[blocked].focus; }
                            }
                            if( index == -1  && focus > 0) focus -= 1;
                            if( index == -11 ) { focus += 4; if( focus > 6 ) focus = 6; }
                            if( index == 11 ){ focus -= 4; if( focus < 0 ) { blocked = 0; focus =  focusable[blocked].focus; } }
                        }
                    }
                }
        ...
        function init(){
            ...
            //vote:[{blocked:0,item:undefined,id: 196}],投票ID
            //video:'视频ID'
            //position:[0,0,0,0]:视频显示坐标
            //external:{after:'',before:'见S20150211.jsp'}}
            $.buildUserInterface({flowed:undefined,popup:undefined,vote:[{blocked:1,item:undefined,id: 207}],video:undefined,position:undefined,external:{after:undefined,before:undefined}});
        ...
        function doEnter(){
            ...
            switch(item.playType){
                ...
                case -3:
                    ...
                    break;
                case 1:
                    top.window.location.href = $.current() + "/EPG/jsp/defaultHD/en/" + (isKorean ? "hddb/hjzq/hj_tvD" : "high_TV_d" ) + "etail.jsp?vodId=" + item.mid + "&typeId=" + typeId;
                    break;
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-04-17.jpg') no-repeat;" onUnload="exit();"></body>
</html>