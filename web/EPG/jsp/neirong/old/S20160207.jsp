...
<%
    ...
    if(StringUtils.isEmpty(typeId))
        typeId = "10000100000000090000000000105873";

    final String[] types = {"10000100000000090000000000105877","10000100000000090000000000105878","10000100000000090000000000105878"};
    final int[] position = {100,0,1,0,100,1};
    //TODO:每次需要修改
    int focused = 0, area = 0;
    final int popItemMaxCharLength = 30;
...
<head>
    <title>春晚</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2016-02-07.png') no-repeat fixed;background-position: 0px 0px;}

        .maskComplete {width:413px;height:83px;left:261px;top:599px;background-position: 0px -100px;}
        .maskLocalTV{width:282px;height:83px;left:673px;top:600px;background-position: 0px -200px;}

        .flowed {position: absolute;left:207px;top:329px;width:900px;height: 280px;}
        .itemText,.maskText,.maskMarquee {width:417px; height:54px;font-size:24px; line-height:46px; color:#e80d0d;overflow: hidden; float:left;}
        .itemText,.maskText {padding:0px 15px; overflow: hidden;}
        .maskText {background-position: 0px 0px;}
        .maskText,.maskMarquee {color:#ffffff;}
        .maskMarquee {width:420px; margin-left: 15px;}
    ...
    <script language="javascript" type="text/javascript">
                    ...
                    for( int j = 0; j < vodList.size() ; j++ ){
                        ...
                        String str = "{ mid:'" + vod.getId() + "'," +
                              ...
                              "shortText:\"" + StringUtil.limitStringLength(vod.getName(), popItemMaxCharLength) + "\"" + "," +
                                  (i != 1 ? "" : ( "style:'mask maskComplete',")) +
                                 "playType:" + vod.getIsSitcom() ;
                            str += "}" ;
                            str += ( j + 1 < vodList.size()  ? "," : "");
                        value += str;
                    }
                    if(i == 1) value += ",{text:'全场完整视频',style:'mask maskLocalTV'}";
                    value += "]},";
                }
                out.write(value);
            }
        ...
        var pageCount = columns * rows;         //每页显示条目数量
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && ( index == 11 && ( focus % pageCount - columns < 0) ||
                                          index == -1 && (focus % columns == 0 && focus - pageCount < 0) ||
                                          index == 1 && ( focus % columns == ( columns - 1 ) && Math.ceil( focus / pageCount ) >= Math.ceil( focusable[blocked].items.length / pageCount ) )
                        ) || blocked == 1 && ( index == -11 || focus == 0 && index == -1 )) return;
                    if( blocked == 0 ) {
                        if( index == 1 ) {
                            focus += focus % columns == ( columns - 1 ) ? ( pageCount - 1 ) : 1;
                            if( focus >= focusable[blocked].items.length )
                                focus = focusable[blocked].items.length -1;
                        } else if( index == -1 ){
                            focus -= focus % columns == 0 ? ( pageCount - 1) : 1;
                        } else if( index == 11 ) focus -= columns;
                        else if( index == -11 ) {
                            if( focus % pageCount + columns >= pageCount || focus + columns >= focusable[blocked].items.length ){
                                blocked = 1;
                                focus = focus % columns;
                                flowedShow(0);
                            } else
                        ...
                        blocked = 1; focus = focusable[blocked].items.length - 1;
                    }
                    $("mask").style.visibility = ( blocked == 0 || blocked == 2 ) ? "hidden" : "visible";
                }
            ...
            if( focusable[blocked].flowed ) flowedShow(0);
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                ...
                if( i == focus && blocked == index ){
                    html += '<div class="mask maskText">';
                    html += item.length > 30 ? ( '<marquee class="maskMarquee">' + item.text + '</marquee>') : item.text;
                    html +=  '</div>';
                } else {
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:[0],popup:[{blocked:1,item:1,target:2,style:'mask maskLocalTV'}],vote:undefined,video:undefined,position:undefined,gatewayCheck:undefined,external:{after:undefined,before:undefined}});
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-02-07.jpg') no-repeat;" onUnload="exit();"></body>
</html>