...
<%
    ...
    final String[] types = {"10000100000000090000000000106746"};
    final int[] position = {2,0};
    //TODO:每次需要修改
...
<head>
    <title>西部世界科幻新神剧</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2016-10-18.png') no-repeat;background-position: 0px 0px;}
        .blocked1 {width:272px;height:351px;top:249px;background-position:7px 0px;}
        .blocked1Focus {width:272px;height:372px;top:248px;background-position:-260px 0px;}

        .item11 {left:76px;}
        .item12 {left:372px;}

        .blocked2,.blocked2Focus {width:292px;height:117px;left:675px;}
        .blocked2 {background-position: 0px -380px;}
        .blocked2Focus {background-position: 0px -500px;}
        .item21 {top:248px;}
        .item22 {top:376px;}
        .item23 {top:503px;}
        ...
        .flowed {}
        .after {position: absolute;width:286px;left:681px;top:240px;height:361px;background:transparent url('images/focusBg-2016-10-18.png') no-repeat 0px 0px; }
        .popup {position:absolute;width:1280px;height:720px;left:0px; top:0px;background-repeat: no-repeat;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            ...
            {
                focus:0, items:[
                {name:'导演篇',playType:-3,link:'/EPG/jsp/neirong/S201610181.jsp'},
                {name:'制作人篇',playType:-3,link:'/EPG/jsp/neirong/S20161021.jsp'},
                {name:'主演篇',playType:-3,link:'/EPG/jsp/neirong/S20161028.jsp'}
            ]},
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && ( index == 11 || index == -11 || focus == 0 && index == -1 ) ||
                        blocked == 1 && ( index == 11 && focus == 0 || index == -11 && focus + 1 == focusable[blocked].items.length )
                    ) return;
                    if( blocked == 0 ) {
                        if( focus == 1 && index == 1 ) {
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                var item = items[i];
                var ix = String( index + 1) + String( i + 1);
                $("item" + ix).className = "mask blocked" + (index + 1) + ( i == focus && blocked == index ? "Focus" : "" ) + " item" + ix;
            }
        ...
        function init(){
            ...
            $.buildUserInterface = function(option){
                ...
                if(typeof option.flowed != 'undefined' ) {
                    html += "<div id='item11'></div><div id='item12'></div><div id='item21'></div><div id='item22'></div><div id='item23'></div>";
                    focusable.flowed = [];
            ...
            $.buildUserInterface({flowed:[0,1],popup:undefined,vote:undefined,video:undefined,position:undefined,external:{after:'<div class="after"></div><div id="popup"></div>',before:undefined}});
            if( focusable.length == 2 ) {blocked = focusable.length - 1;}
            focused(<%= focused %>, true);
        }
        function goBack(keyBack){
            if( blocked == 1 && popup == true) {
                popup = false;
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-10-18.jpg') no-repeat;" onUnload="exit();"></body>
</html>