...
<%
    ...
    final String[] types = {"10000100000000090000000000105876"};
...
<head>
    <title>Get It Beauty一起变美吧</title>
    <style>
        .mask {position:absolute;}

        .item{ width:495px;height:44px; overflow:hidden; float: left;}

        .itemLeft {display: block;height:44px;width:75px;background:transparent url('images/mask-2016-02-05.png') no-repeat fixed 0px 0px; float: left; }
        .itemText {font-size: 22px;height:44px;line-height:44px;text-align: center; background:transparent url('images/mask-2016-02-05.png') repeat-x fixed 0px -50px; float: left; }
        .itemRight {display: block;height:44px;width:20px;background:transparent url('images/mask-2016-02-05.png') no-repeat fixed -396px 0px; float: left; }

        .flowed {width:495px;left:40px;top:371px;height:272px;overflow:hidden;position: absolute;}
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    focus = focus - 1;
                     ...
                     if( blocked < focusable.length - 2  && (
                         ...
                         index == -1 || //按左光标键时
                         index == -11 && focus + columns >= focusable[blocked].items.length  ) //按下光标键时
                     ) return;
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                ...
                if( i == focus && blocked == index ){
                    ...
                    html += '<div class="itemLeft">&nbsp;</div><div class="itemText" style="color:white;width:400px">';
                    html += item.length > 30 ? ( '<marquee class="maskMarquee">' + item.text + '</marquee>') : item.text;
                    html += '</div><div class="itemRight">&nbsp;</div>';
                } else {
                    html += '<div style="font-size:22px;width:400px;line-height:44px;color:black;clear: both;text-align: center; margin:0px 0px 0px 75px;">';
                    html += item.shortText;
                }
                html +='</div></div>';
            }
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-02-05.jpg') no-repeat;" onUnload="exit();"></body>
</html>