...
<%
    ...
    final String[] types = {"10000100000000090000000000106141"};
...
<head>
    <title>你们“老公”的黑历史</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2016-04-20.png') no-repeat;background-position: 0px 0px;}
        .mask11 {width:278px;height:187px;left:46px;top:448px;background-position: -50px 0px;}
        .arrowUp,.arrowDown {width:19px;height:13px;left:555px;}
        .arrowUp {top:361px;background-position: -10px 0px;}
        .arrowDown { top:599px;background-position: -10px -30px;}

        .scrollBar{position:absolute;width:6px;left:339px;}
        .scrollBarTop,.scrollBarBottom{width:6px;left:0px;height:3px; border:none;}
        .scrollBarTop {top:0px;background-position: 0px 0px;}
        .scrollBarMiddle{width:6px;left:0px;background:#ffffff none; border:none;top:3px;}
        .scrollBarBottom{bottom:0px;background-position: 0px -41px;}

        .flowedItem {padding:0px 8px; width:443px;height:28px; font-size: 20px; left:355px;line-height: 20px;overflow: hidden; position: absolute;word-break:keep-all;white-space:nowrap;}
        .item1 { top:387px; }
        .item2 { top:430px; }
        .item3 { top:473px; }
        .item4 { top:516px; }
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            ...
            %>
            { focus:0,items : [{name:'活动',style:'mask mask11',playType:-3,link:'http://192.168.35.17:8082/Kaisa/home?fromTitle=%E9%87%8D%E5%BA%86%E6%9C%89%E7%BA%BF-%E4%BA%92%E5%8A%A8%E4%B8%93%E5%8C%BA'}]},
        ...
        var columns = 1; rows = 100;              //定义行列，　几行几列
        var pageCount = 5;
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0  && (
                     ...
                     ) || blocked == 1 && ( index == 11 || index == -11  || index == -1)) return;
                     //TODO:当光标在首页、返回按钮上时，可同时按上，左键返回到列目中时，需要重写光标移动代码
                     if( blocked == focusable.length - 1 || blocked == 1 ) { blocked = 0;focus = focusable[blocked].focus; }
                     else {
                         if( index == 11 ) focus -= columns;
                         else if( index == -11 ) focus += columns;
                         else if( index == -1 ) {blocked = 1; focus = 0;}
                         if(index == 1 || focus >= focusable[blocked].items.length ) { blocked = focusable.length - 1;focus = focusable[blocked].focus; }
                     }
                }
        ...
        var flowedShow = function(index){
            ...
            $("scrollBar").style.visibility = items.length > 5 ? "visible" : "hidden";
            var h = Math.ceil( 5.0 / items.length * 215);
            var le = 215 - h;
            $("scrollBar").style.height = h + "px";
            $("scrollBarMiddle").style.height = ( h - 6 < 6 ? 6  : h - 6) + "px";

            $("arrowUp").className = "mask arrowUp";
            $("arrowDown").className = "mask arrowDown";
            //每页显示数量
            ...
            $("arrowUp").style.visibility = flowCursorIndex > 0 ? "visible" : "hidden";
            $("scrollBar").style.top = Math.ceil( focus * 1.0 / (items.length - 1) * le  + 384 ) + "px";
            for(var i = flowCursorIndex, j = 1; i < flowCursorIndex + pageCount; i += 1, j++) {
                var item = items[i];
                $("flowedItem" + j).className = "flowedItem item" + j;

                if( i == focus && blocked == index ){
                    $("flowedItem" + j).style.color = "#A40000";
                    $("flowedItem" + j).style.backgroundColor = "#FFFFFF";
                    $("flowedItem" + j).innerHTML = item.length > 38 ? ( "<marquee class='marqueed'>" + item.text + "</marquee>" ) : item.text;
                } else {
                    $("flowedItem" + j).innerHTML = item.shortText;
                    $("flowedItem" + j).style.color = "#FFFFFF";
                    $("flowedItem" + j).style.backgroundColor = "transparent";
                }
            }
        }
        function init(){
            ...
            $.buildUserInterface = function(option){
                ...
                if(typeof option.flowed != 'undefined' ) {
                    html += "<div id='scrollBar' class='scrollBar'><div class='mask scrollBarTop'></div><div id='scrollBarMiddle' class='mask scrollBarMiddle'></div><div class='mask scrollBarBottom'></div></div><div id='arrowUp'></div><div id='flowedItem1'></div><div id='flowedItem2'></div><div id='flowedItem3'></div><div id='flowedItem4'></div><div id='flowedItem5'></div><div id='arrowDown'></div>";
                    focusable.flowed = [];
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-04-20.jpg') no-repeat;" onUnload="exit();"></body>
</html>