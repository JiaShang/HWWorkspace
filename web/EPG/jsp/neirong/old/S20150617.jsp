...
<%
    ...
    final String[] types = {"10000100000000090000000000105221"};
...
<head>
    <title>父亲节遇上端午节 “爸”气十足</title>
    <style>
        .mask {width:246px;height:34px;position:absolute;background:transparent url('images/mask-2015-06-17.png') no-repeat fixed;background-position: 0px -350px;}

        .mask1{left:419px;top:346px;}
        .mask2{left:419px;top:382px;}
        .mask3{left:419px;top:418px;}
        .mask4{left:419px;top:454px;}
        .mask5{left:419px;top:490px;}
        .mask6{left:665px;top:457px;}
        .mask7{left:665px;top:489px;}
        .mask8{left:665px;top:521px;}
        .mask9{left:665px;top:611px;}
        .flowed {}
    ...
    <script language="javascript" type="text/javascript">
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked === 0 && ( ( focus === 0 || focus === 5 ) && index === 11 ||
                                             focus <=4 && index === -1 ||
                                           ( focus === 4 || focus + 1 == focusable[blocked].items.length ) && index === -11
                                         )
                      )  return;
                    if( blocked === focusable.length - 1) {
                        blocked = 0; focus = consign[1];
                    } else {
                        if( Math.abs( index ) == 11 ) { focus += index > 0 ? -1 : 1; }
                        else {
                            if( index == 1 ) {
                                consign[ focus <= 4 ? 0 : 1 ] = focus;
                                blocked = (focus <= 4 ? 0 : focusable.length - 1);
                                focus = (focus <= 4 ? consign[1] : 0);
                            } else {
                                consign[ 1 ] = focus;
                                focus = consign[0];
                            }
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:undefined,popup:undefined,vote:undefined,video:undefined,position:undefined,external:{after:'<div style="width:450px;height:294px;position: absolute;left: 445px;top:348px;background:transparent url(images/mask-2015-06-17.png) no-repeat fixed 0px 0px;">',before:undefined}});
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-06-17.jpg') no-repeat;" onUnload="exit();"></body>
</html>