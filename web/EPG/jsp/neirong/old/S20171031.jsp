...
<head>
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,typeId:'<%= types[0] %>',items : [{name:'直播视频',style:'mask mask11',<%= autoPlayId.equalsIgnoreCase("0") ? "playType:-3,link:'/EPG/jsp/neirong/VideoFullPlay.jsp?vodid=0'" : "playType:0,mid:" + autoPlayId%>}]},
            { focus:0,items : [{name:'往界马拉松回顾',style:'mask mask21',playType:-3,link:'/EPG/jsp/neirong/S20171031Review.jsp?typeId=10000100000000090000000000106719'}]},
            { focus:0,typeId:'<%= types[0] %>',items : [
                {name:'赛事详情',style:'mask mask3 mask31',playType:-3,link:'/EPG/jsp/neirong/S20171031Detail.jsp'},
                {name:'轻松跑半马',style:'mask mask3 mask32',playType:-3,link:'http://192.168.35.153:8080/huoDong_malasong.jsp?lcn=malas'},
                {name:'综合训练营',style:'mask mask3 mask33',playType:-3,link:'http://192.168.35.153:8080/huoDong_banma.jsp?lcn=mls'},
                {name:'马拉松跑者',style:'mask mask3 mask34',playType:0,mid:<%= vodMid %>},
            ]},
            <%
            value = "";
            ...
            $.video.play = function(){
                try{
                    ...
                    if( $.video.id != '0' ) {
                        ...
                        media.video.setPosition($.video.position[0],$.video.position[1],$.video.position[2],$.video.position[3]);
                        DVB.playAV(2750000,2602);       //频点，ServiceId
                    }
                } catch (e){}
            }
