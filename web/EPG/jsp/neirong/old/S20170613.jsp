...
<%
    ...
    final String[] types = {"10000100000000090000000000107821"};
...
<head>
    <title>好吃狗美人厨房</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2017-06-13.png') no-repeat;background-position: 0px 0px;}

        .mask1 {width:244px;height:49px;left:370px;top:98px;background-position: 0px 0px;}
        .mask2 {width:244px;height:49px;left:370px;top:168px;background-position: 0px 0px;}
        .mask3 {width:144px;height:32px;left:766px;top:653px;background-position: 0px -50px;}

        .flowed {width:1280px;height:720px;position: absolute;left:0px;top:0px; background:transparent url('images/focusBg-2017-06-13.png') no-repeat 0px 0px; }
        .flowedContainer {width:466px;height:369px;left:430px;top:214px; overflow: hidden;position: absolute;}
        .item,.itemFocus {width:466px;height:40px;font-size: 22px;line-height: 32px; text-align: left;float: left;overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;padding:0px 10px 0px 10px;}
        .item {color:#333333;background-color:transparent;}
        .itemFocus { color:white;background-color:#DE55A6;}
        .marquee {width:446px;line-height: 36px;font-size: 22px;margin:0px 0px 0px 10px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,items : [
                {name:'第一条视频',style:'mask mask1',playType:0,mid:0 },
                {name:'弹出层',style:'mask mask2'},
                {name:'报名参与',style:'mask mask3',playType:-2}
            ]},
            <%
            value = "";
        ...
        var columns = 1; rows = 9;              //定义行列，　几行几列
        ...
        function focused(index,initilized){
            ...
            if( blocked == 1 ) flowedShow( 1 );
            else { $("flowed").style.visibility = 'hidden'; }
        }
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                ...
                if( i == focus && blocked == index ){
                    html += '<div class="itemFocus">' + ( item.shortText != item.text ? ( "<marquee class='marquee'>" + item.text + "</marquee>" ) : item.text ) + '</div>';
                } else {
                    html += '<div class="item">' + item.shortText + '</div>';
                }
            }
            html += '</div>';
        ...
        function init(){
            ...
            focusable[0].items[0].mid = focusable[1].items[0].mid;
            focusable[0].typeId = focusable[1].typeId;
            focusable[1].items = focusable[1].items.slice(1);

            $.buildUserInterface({flowed:[1],vote:[{blocked:0,item:2,id: 393}],video:undefined,position:undefined,external:{after:undefined,before:undefined}}); //'<div class="after"></div>'
            focused(<%= focused %>, true);
        }
        function goBack(keyBack){
            if( blocked == 1 ) {
                blocked = 0; focused(focusable[0].focus,true);return;
            }
        ...
        function doEnter(){
            ...
            if( blocked == 0 && focusable[0].focus == 1 ) {
                blocked = 1; flowedShow(1); return;
            }
            //首页，返回
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-06-13.jpg') no-repeat;" onUnload="exit();"></body>
</html>