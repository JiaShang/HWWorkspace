...
<head>
    <title>天天向上</title>
    <style>
        .mask {width:96px;height:53px;position:absolute;background:transparent url('images/mask-2016-03-10.png') no-repeat fixed;background-position: 0px 0px;}
        .mask1 {left:327px;top:254px;background-position: 0px 0px;}
        .mask2 {left:596px;top:117px;background-position: 0px -60px;}
        .mask3 {left:919px;top:312px;background-position: 0px -120px;}
        .mask4 {left:288px;top:481px;background-position: 0px -180px;}
        .mask5 {left:750px;top:457px;background-position: 0px -240px;}
        .flowed {}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,items : [
                {name:'直击高考第二站',style:'mask mask1',playType:-3,link:'http://192.168.9.174/dvb_cqocn_53/welcome/home.htm?welcome=gaokaozhuanti'},
                {name:'备考中考第一站',style:'mask mask2',playType:-3,link:'http://192.168.9.174/dvb_cqocn_53/welcome/home.htm?welcome=zhongkaozhuanti'},
                {name:'一路向前第五站',style:'mask mask3',playType:-3,link:'http://192.168.48.217:8085/iptv/entrance.jsp?gpid=144'},
                {name:'名师助力第三站',style:'mask mask4',playType:-3,link:'http://192.168.42.52:8088/epg/index/newEast.do'},
                {name:'动脑益智第四站',style:'mask mask5',playType:-3,link:'http://192.168.98.65:10001/IPTVGameEPG/index.jsp?recoCode=LTR_10156'}
            ]},
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked < focusable.length - 2  && (
                         ...
                         index == -11 && (focus + columns >= focusable[blocked].items.length && arrows[0] == 0 || arrows[0] == 1 && focus + 1 != focusable[blocked].items.length && focus + columns + 1 > focusable[blocked].items.length && focus + columns != focusable[blocked].items.length ) //按下光标键时
                     ) ) return;
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-03-10.jpg') no-repeat;" onUnload="exit();"></body>
</html>