...
<%
    ...
    final String[] types = {"10000100000000090000000000107096"};
...
<head>
    <title>爱看电影2016观影报告</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2016-12-29.png') no-repeat;background-position: 0px 0px;}
        .arrow {width:56px;height:34px;left:605px;}
        .arrowUp {top:248px;background-position:0px 0px}
        .arrowDown {top:607px;background-position:-60px 0px}

        .flowed {width:853px;height:340px;left:201px;top:296px;position:absolute;}
        .item {height:68px;width:426px;float:left;}
        .name,.click{width:271px;float:left;line-height: 32px;font-size:18px;color:#b7b3b0; text-align: center;overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;}
        .click {width:155px;text-align: left;font-size:18px;}
        .after {}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,typeId:'<%=types[0]%>',items : [
                {name:"《熊出没之熊心归来》",click:7031830,playType:0},
                {name:"《小黄人大眼萌》",click:6780289,playType:0},
                {name:"《港囧》",click:6001192,playType:0},
                {name:"《夏洛特烦恼》",click:5218924,playType:0},
                {name:"《澳门风云3》",click:4167352,playType:0},
                {name:"《快手枪手快枪手》",click:3322010,playType:0},
                {name:"《剩者为王》",click:2778742,playType:0},
                {name:"《美人鱼》(HD)",click:2718312,playType:0},
                {name:"《蚁人》",click:2718320,playType:0},
                {name:"《老炮儿》",click:2695152,playType:0},
                {name:"《我是证人》",click:2204741,playType:0},
                {name:"《美国队长3：内战》(HD)",click:1193234,playType:0},
                {name:"《疯狂动物城》(HD)",click:1535320,playType:0},
                {name:"《大鱼海棠》",click:1468662,playType:0},
                {name:"《绝地逃亡》(HD)",click:1465802,playType:0},
                {name:"《封神传奇》(HD)",click:1453620,playType:0},
                {name:"《侏罗纪世界》(HD)",click:1423473,playType:0},
                {name:"《万万没想到》(HD)",click:1355920,playType:0},
                {name:"《寒战2》(HD)",click:1309781,playType:0},
                {name:"《碟中谍5：神秘国度》(HD)",click:1127943,playType:0}
            ]},
            <%
            value = "";
            if( list != null && list.size() > 0){
                for(int i = 0; i < list.size() ; i++){
                    ...
                    for( int j = 0; j < vodList.size() ; j++ ){
                        ...
                        String str = "{ mid:'" + vod.getId() + "',text:'" + vod.getName() + "'}" ;
        ...
        var columns = 2; rows = 5;              //定义行列，　几行几列
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                var item = items[i];
                html += "<div class='item'>";
                html += "<div class='name' " + ( i == focus && blocked == index  ? " style='color:#e1a120'" : "" ) +   ">" + item.name + "</div>";
                html += "<div class='click'>点击量:" + item.click + "</div>";
                html += "</div>";
            }
            ...
            var page = Math.ceil( (focus + 1.0) / pageSize );
            pageSize = Math.ceil( focusable[blocked].items.length * 1.0 / pageSize );
            if( pageSize > page ) {
        ...
        function init(){
            ...
            $.buildUserInterface = function(option){
                ...
                html += "<div id='arrowUp'></div><div id='arrowDown'></div>";
            ...
            for( var i = 0; i < focusable[0].items.length && i < focusable[1].items.length; i++ )
                focusable[0].items[i].mid = focusable[1].items[i].mid;
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-12-29.jpg') no-repeat;" onUnload="exit();"></body>
</html>