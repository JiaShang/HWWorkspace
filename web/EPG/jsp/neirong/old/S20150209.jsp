...
<%
    ...
    final String[] types = {"10000100000000090000000000104920"};
...
<style>
    .mask {height: 30px;position:absolute;background:transparent url('images/mask-2015-02-09.png') no-repeat fixed;background-position: 0px 0px;}
    .mask1{width: 394px;left:50px; top:161px;background-position: 0px 0px;}
    .mask2{width: 269px;left:49px; top:274px;background-position: 0px -30px;}
    .mask3{width: 296px;left:49px; top:307px;background-position: 0px -60px;}
    .mask4{width: 161px;left:369px; top:516px;background-position: 0px -90px;}
    .mask5{width: 268px;left:368px; top:550px;background-position: 0px -120px;}
    .mask6{width: 269px;left:368px; top:584px;background-position: 0px -150px;}
    .mask7{width: 184px;left:369px; top:617px;background-position: 0px -180px;}
    .mask8{width: 322px;left:692px; top:515px;background-position: 0px -210px;}
    .mask9{width: 189px;left:692px; top:549px;background-position: 0px -240px;}
    .mask10{width: 189px;left:692px; top:583px;background-position: 0px -270px;}
    .mask11{width: 162px;left:692px; top:616px;background-position: 0px -300px;}
    .mask12{width: 161px;left:692px; top:650px;background-position: 0px -330px;}
    ...
    var focusable = focusable ? focusable : [
        ...
        if( list != null && list.size() > 0){
            for(int i = 0; i < list.size() ; i ++){
                ...
                for( int j = 0; j < vodList.size() && j < 12 ; j++ ){
    ...
    function focused(index,initilized){
        ...
        if(typeof index == "number" && !initilized){
            ...
            if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                ...
                switch (index){
                case 11:
                    if(focus == 0 || focus == 7 ) return;
                    if( focus == 3 ) saved[1] = 3;
                    focus = focus - 1;
                    break;
                case -11:
                    if( focus == 6 || focus ==  11 ) return;
                    if( focus == 2 ) saved[0] = 2;
                    focus = focus + 1;
                    break;
                case 1:
                    if( blocked == 0 ) {
                        if(focus >= 7 ) { indexSaved = focus; blocked = focusable.length - 1; focus = focusable[blocked].focus; }
                        else if( focus >= 3 && focus <= 6 ) { saved[1] = focus; focus = saved[2] == 0 ? 7 : saved[2]; }
                        else { saved[0] = focus; focus = saved[1] == 0 ? 3 : saved[1]; }
                    }
                    break;
                case -1:
                    if( blocked == 0 && focus <= 2 ) return;
                    if( blocked == focusable.length - 1 ) { blocked = 0; focus = indexSaved; }
                    else if( blocked == 0 ) {
                        if( focus >= 3 && focus <= 6  ) { saved[1] = focus; focus = saved[0]; }
                        else { saved[2] = focus; focus = saved[1] == 0 ? 3 : saved[1]; }
                    }
                    break;
                }
            }
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-02-09.jpg') no-repeat;" onUnload="exit();"></body>
</html>