...
<%
    ...
    final String[] types = {"10000100000000090000000000107822"};//
...
<head>
    <title>影象重庆</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2017-06-12.png') no-repeat;background-position: 0px 0px;}
        .item{position: relative;float:left;width:200px;height:280px;border:none;overflow: hidden;}
        .img {width:174px;height:234px;float:left;}
        .img img{width:174px;height:234px;}
        .txt {width:174px;height:40px;font-size: 22px;line-height: 36px;color:#000000;text-align: center;float:left;}
        .marquee {width:174px;font-size: 22px;}
        .itemFocus {width:265px;height:326px;}
        .flowed {position:absolute;width:1050px;height:600px;overflow: hidden; left:117px;top:114px;overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;}
        .goTop {width:30px;height:80px;left:1173px;top:585px;background-position: -280px 0px;}

        .page {width:44px;height:25px;color:white;font-size: 18px;line-height:25px;position: absolute;left:1165px;overflow: hidden;word-break:keep-all;white-space:nowrap; text-align: center;}
        .number {top:120px;}
        .count {top:156px;}
        .after {}
    ...
    <script language="javascript" type="text/javascript">
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && !$.vote.tooltiped && blocked == focusable.length - 1 && focus == 1 ) focus = focus + 1;
                    ...
                    if( blocked == 0 && ( index == 11 && focus < columns || index == -1 && focus % columns == 0 || index == -11 && ( Math.ceil( focus * 1.0 / pageCount)  >= Math.ceil( focusable[blocked].items.length * 1.0 / pageCount ) )) ||
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                var item = items[i];
                html += "<div class='item'><div  class='img'><img src='" + item.picture + "'/></div>";
                if( i == focus && blocked == index ){
                    html += "<div class='txt'>" + ( item.shortText != item.text ? ( "<marquee class='marquee'>" + item.text + "</marquee>" ) : item.text ) + "</div>";
                } else {
            ...
            if( blocked == 0 ) {
                $("mask").className = "mask  itemFocus";
                $("mask").style.top = 78 + ( Math.floor(focus % pageCount / columns ) * 280) + "px";
                $("mask").style.left = 78 + ( focus % pageCount % columns * 200) + "px";
            } else {
                $("mask").className = "mask  goTop";
                $("mask").style.top = "585px";
                $("mask").style.left = "1173px";
            }
            if( items.length == 0 ) {
                $("pageNum").innerHTML = $("pageCount").innerHTML = "0";
            } else {
                $("pageNum").innerHTML = Math.ceil( ( focus + 1 ) / pageCount);
                $("pageCount").innerHTML = Math.ceil( ( items.length + 1 ) / pageCount);;
            }
        }
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:[0],vote:undefined,video:undefined,position:undefined,external:{after:'<div id="pageNum" class="page number"></div><div id="pageCount" class="page count"></div>',before:undefined}}); //
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-06-12.jpg') no-repeat;" onUnload="exit();"></body>
</html>