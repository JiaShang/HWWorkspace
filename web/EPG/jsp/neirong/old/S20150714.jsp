...
<%
    ...
    final String[] types = {"10000100000000090000000000105321"};
    final int[] position = {999,0};
    //TODO:每次需要修改
...
<head>
    <title>逐梦他乡重庆人</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2015-07-14.png') no-repeat fixed;background-position: 0px 0px;}

        .pageUp{width:107px;height:25px;top:658px;left:139px;background-position: 0px -400px;}
        .pageDown{width:99px;height:25px;top:658px;left:249px;background-position: 0px -430px;}

        .flowed {position: absolute;width:1120px;height: 390px;top:261px;left:104px;overflow:hidden;}
        .flowedItem {float: left;height:189px;width:167px;overflow:hidden;padding:3px 15px;}
        .normal {width:137px;height:179px; position: relative;}
        .text {width:137px;left:0px;position:absolute;height: 45px;top:133px; font-size: 12px; text-align: center}
        .text1 {font-size:18px;line-height: 22px; height: 22px; width: 135px;overflow: hidden;word-wrap: normal;white-space:nowrap;}
        .text2 {font-size:13px;line-height: 18px; height: 18px;width: 135px;overflow: hidden;word-wrap: normal;white-space:nowrap;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    var size = 12.0;
                    var page = Math.floor(focus / size) * size;
                    size = focusable[0].items.length - page >= 12 ? 12 : focusable[0].items.length - page;
                    var cur = focus % 6;
                    if( blocked == 0 && ( focus <= 5 && index == 11 ||
                                            cur == 0 && index == -1 ||
                                          ( cur == 5 || focus + 1 >= focusable[0].items.length ) && index == 1 ||
                                            cur > 1 && cur < 5 && focus + 6 >= focusable[0].items.length && index == -11 )  ||
                        blocked == 1 && ( index == -11 || focus == 0 && index == -1 || index == 1 && focus == 1 )
                    ) return;

                    if( blocked == 0 ) {
                        if( index == -1 ) focus -= 1;
                        else if( index == 1 ) focus += 1;
                        else if( index == 11) focus -= 6;
                        else if( index == -11 ) {
                            if( cur == 0 && focus + 6 >= page + size) { blocked = 1; focus = 0;}
                            else if( cur == 1 && focus + 6 >= page + size) { blocked = 1; focus = 1;}
                            else if( cur == 5 && focus + 6 >= page + size) { blocked = focusable.length - 1; focus = 0;}
                            else focus += 6;
                        }
                    } else if( blocked == 1 && ( index == 1 || index == -1 )){
                        focus += index;
                    }
                    else {
                        blocked = 0;
                    ...
                    if( vst != $("mask").style.visibility ){
                        $("mask").style.visibility = vst;
                        flowedShow(0);
                    }
                }
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                var item = items[i];
                var names = item.text.replace("（逐梦他乡重庆人HD）","").split('：');
                html += "<div class='flowedItem'><div class='normal' style='background: url(\"" + item.picture + "\")'><img src='" + ( i == focus && blocked == index ? 'images/focus-2015-07-14.png' : 'images/normal-2015-07-14.png' ) + "' style='left:0px;top:0px;'/>";
                html += '<div class="text" style="color:' + ( i == focus && blocked == index ? 'black' : 'white' ) + ';"><div class="text1">' + names[0] + "</div><div class='text2'>" + names[1] + '</div></div></div></div>';
            }
        ...
        function doEnter(){
            ...
            //首页，返回
            if( blocked == 1 ) {
                var page = Math.floor(focusable[0].focus / 12.0) * 12;
                if( page >= 12 &&  focusable[1].focus == 0 ) {
                    focusable[0].focus -= 6;
                } else if( page + 12 < focusable[0].items.length &&  focusable[1].focus == 1)  {
                    focusable[0].focus += 6;
                }
                flowedShow(0);
                return;
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-07-14.jpg') no-repeat;" onUnload="exit();"></body>
</html>