...
<%
    ...
    final String[] types = {"10000100000000090000000000107189","10000100000000090000000000107190"};
...
<head>
    <title>春节联欢晚会</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2017-01-25.png') no-repeat;background-position: 0px 0px;}
        .blocked0 {height:228px;width:309px;left:501px;top:281px;overflow: hidden;position:absolute;}
        .item0{color:#ec132e;height:38px;width:309px;float: left;font-size:20px;line-height:35px; overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;}

        .focusBg {width:1280px;height:720px;left:0px;top:0px; position:absolute;background:transparent url('images/focusBg-2017-01-25.png') no-repeat 0px 0px;}
        .blocked1 {height:306px;width:282px;left:513px;top:251px;overflow: hidden;position:absolute;}
        .item1{color:#feca72;height:51px;width:282px;float:left;font-size:20px;line-height:48px; overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;}

        .mask31 {width:166px;height:53px;left:587px;top:594px;background-position: 0px -100px;}

        .maskItem0 {width:351px;height:47px;left:468px;background-position: 0px -40px;}
        .mask00 {top:278px;}
        .mask01 {top:316px;}
        .mask02 {top:354px;}
        .mask03 {top:392px;}
        ...
        .maskItem1 {width:293px;height:33px;left:509px;background-position: 0px 0px;}
        .mask10 {top:262px;}
        .mask11 {top:313px;}
        .mask12 {top:364px;}
        .mask13 {top:415px;}
        ...
        .page0,.page1{width:100px;text-align: center;font-size: 20px;left:604px;height:20px;position:absolute;}
        .page0{color:#ec132e;top:506px;}
        .page1{color:#feca72;top:573px;}
        ...
        .pagez {width:70px;height:22px;left:516px;top:348px;font-size:22px;text-align:right;line-height:22px;position:absolute; color: white; }
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
                ...
                for(int i = 0; i < list.size() ; i++){
                    ...
                    for( int j = 0; j < vodList.size() ; j++ ){
                        ...
                        String str = "{ mid:'" + vod.getId() + "'," +
                              ...
                              "shortText:\"" + StringUtil.limitStringLength(vod.getName(), i == 0 ? 29 : 24 ) + "\"" + "," +
            ...
            %>
            { focus:0,items : [{name:'活动细则',style:'mask mask31'}]},
        ...
        var columns = 1; rows = 6;              //定义行列，　几行几列
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    var computeTotal = Math.ceil( (focusable[blocked].items.length  * 1.0 ) / pageCount );

                    if( blocked <= 1 && ( index == -1 && focus < pageCount || index == 1 && currentPage >= computeTotal || index == 11 && focus % pageCount == 0 ) ||
                        blocked == 2 && index != 11 ) return;
                    if( blocked == 2 ) { blocked = 0; focus = focusable[blocked].focus; }
                    else {
                        if( index == 11 ) { focus -= 1; }
                        else if( index == -11 ) {
                            if( focus % pageCount == pageCount - 1 || focus + 1 >= focusable[blocked].items.length ) {
                                if( blocked == 1 ) return;
                                blocked = 2; focus = 0;
                            } else {
                                focus += 1;
        ...
        var flowedShow = function(index){
            ...
            if( index == 0 ) {
                $("flowed").className = "blocked0";
                $("page").className = "page0";
                $("focusBg").style.visibility = 'hidden';
            } else {
                $("flowed").className = "blocked1";
                $("page").className = "page1";
                $("focusBg").className = "focusBg";
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                ...
                html  += ( i == focus && blocked == index && item.text != item.shortText  ) ? ( "<marquee style='width:" + (index == 0 ? 309 : 282 ) + "px;'>" + item.text + "</marquee>") : item.shortText;
                if( i == focus && blocked == index) {
                    $("mask").className = 'mask maskItem' + index + ' mask' + index + "" + (focus % pageCount);
                }
                html += "</div>";
            }
            var currentPage = Math.ceil( ( focus + 1.0 ) / pageCount );
            var computeTotal = Math.ceil( ( items.length  * 1.0 ) / pageCount );
            $("page").innerHTML = currentPage + "/" + computeTotal;
        ...
        function init(){
            ...
            $.buildUserInterface = function(option){
                ...
                html += "<div id='page' class='page'></div>";
            ...
            $.buildUserInterface({flowed:[0,1],popup:undefined,vote:undefined,video:undefined,position:undefined,external:{after:undefined,before:undefined}}); //'<div class="after"></div>'
            if( focusable.length == 2 ) {blocked = focusable.length - 1;}
            focused(<%= focused %>, true);
        }
        function goBack(keyBack){
            if( blocked == 1 ) {
                blocked = 0; focused(focusable[blocked].focus,true); return;
            }
        ...
        function doEnter(){
            ...
            //首页，返回
            if( blocked == 2 ) {
                blocked = 1; focused(focusable[blocked].focus, true); return;
            }
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-01-25.jpg') no-repeat;" onUnload="exit();"></body>
</html>