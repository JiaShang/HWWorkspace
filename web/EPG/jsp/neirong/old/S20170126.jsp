...
<head>
    <title>春节庙会</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2017-01-26.png') no-repeat;background-position: 0px 0px;}
        .mask1 {width:95px;height:46px;left:196px;top:277px;background-position: 0px 0px }
        .mask2 {width:107px;height:26px;left:191px;top:326px;background-position: 0px -50px }
        .mask3 {width:98px;height:45px;left:203px;top:471px;background-position: 0px -80px }
        .mask4 {width:107px;height:26px;left:471px;top:128px;background-position: 0px -130px }
        .mask5 {width:107px;height:26px;left:471px;top:153px;background-position: 0px -160px }
        .mask6 {width:107px;height:26px;left:471px;top:178px;background-position: 0px -190px }
        .mask7 {width:119px;height:47px;left:574px;top:552px;background-position: 0px -220px }
        .mask8 {width:161px;height:26px;left:575px;top:601px;background-position: 0px -270px }
        .mask9 {width:200px;height:26px;left:573px;top:628px;background-position: 0px -300px }
        .mask10 {width:107px;height:26px;left:576px;top:655px;background-position: 0px -330px }
        .mask11 {width:149px;height:45px;left:671px;top:181px;background-position: 0px -360px }
        .mask12 {width:99px;height:27px;left:871px;top:397px;background-position: 0px -410px }
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,items : [
                {name:'巨星云集',style:'mask mask1',playType:-3,action:[0,0,0,1,0,0,0,3],link:'/EPG/jsp/neirong/S20170122.jsp'},
                {name:'开年大戏',style:'mask mask2',playType:-3,action:[0,0,0,2,0,1,0,3],link:'/EPG/jsp/neirong/S20170123.jsp'},
                {name:'年夜饭',style:'mask mask3',playType:-3,action:[0,1,0,2,0,2,0,6],link:'/EPG/jsp/neirong/S20170121.jsp'},
                {name:'优酷专区',style:'mask mask4',playType:-3,action:[0,3,0,4,0,0,0,10],link:'http://192.168.17.74/youku/index.htm'},
                {name:'搜狐专区',style:'mask mask5',playType:-3,action:[0,3,0,5,0,0,0,10],link:'http://192.168.17.57:9888/sohuepg/index.html'},
                {name:'芒果专区',style:'mask mask6',playType:-3,action:[0,4,0,6,0,0,0,10],link:'http://192.168.17.155/nn_cms/web_template/index.html?from_pid=protal'},
                {name:'签到有礼',style:'mask mask7',playType:-3,action:[0,5,0,7,0,2,0,11],link:'http://192.168.33.76:8081/zpyl/20161222-zpyl.htm'},
                {name:'年画宝宝',style:'mask mask8',playType:-3,action:[0,6,0,8,0,2,0,11],link:'http://192.168.35.17:8086/nhbbzt/init?fromTitle=ztdl'},
                {name:'HIFI新年音乐',style:'mask mask9',playType:-3,action:[0,7,0,9,0,2,0,11],link:'http://192.168.34.81/HiFi_cq/wailiu/wl/HiFi_songList.html?albumId=wl_011&type=0&wlColor=ffffff_eec8a2'},
                {name:'I12送红包',style:'mask mask10',playType:-3,action:[0,8,0,9,0,2,0,11],link:'http://192.168.34.18:8081/activity/red_packet/index.do'},
                {name:'2017春晚',style:'mask mask11',playType:-3,action:[0,10,0,6,0,3,0,11],link:'/EPG/jsp/neirong/S20170125.jsp'},
                {name:'限时特惠',style:'mask mask12',playType:-3,action:[0,10,0,11,0,10,0,11],link:'http://192.168.33.75:80/TGO/themeActivity.htm?oprtCatNo=00000276'}
            ]},
        ...
        function visitedCount(focus, callback) {
            var cardId = CA.card.serialNumber;
            var voteName = '2017庙会专题';
            if( typeof focus != 'undefined' ) voteName = focusable[0].items[focus].name;
            var url = "http://192.168.49.56:8080/voteNew/external/clickCount.ipanel?icid=" + cardId + "&classifyID=391&content=" + encodeURIComponent(voteName);
            $.ajax(url,function(result){ if( typeof callback === 'function') callback(); });
        }
        function init(){
            ...
            switch(item.playType){
                ...
                case -3:
                    ...
                    if( ! item.link.startWith('http') ){
                        ...
                        var queryStr = "<%= StringUtils.isEmpty(request.getQueryString()) ? "" : request.getQueryString() %>";
                        if( ! queryStr.isEmpty() ) requestUrl += "?" + queryStr;
                        requestUrl += requestUrl.indexOf("?") >= 0 ? "&" : "?";
                        requestUrl += 'currFoucs=' + blocked + "," + focusable[blocked].focus;
                        url += encodeURIComponent(requestUrl);
                    }
                    visitedCount( focusable[blocked].focus, function () {
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-01-26.jpg') no-repeat;" onUnload="exit();"></body>
</html>