...
<%
    ...
    final String[] types = {"10000100000000090000000000106583","10000100000000090000000000106584"};
    final int[] position = {11,0,50,0};
    //TODO:每次需要修改
...
<head>
    <title>消暑夜市全攻略</title>
    <style>
        .mask {width:76px;height:64px;position:absolute;background:transparent url('images/mask-2016-08-12.png') no-repeat;background-position: 0px 0px;}
        .mask1 {left:221px;top:257px;background-position: 0px 0px;}
        .mask2 {left:458px;top:278px;background-position: -80px 0px;}
        .mask3 {left:622px;top:428px;background-position: -160px 0px;}
        .mask4 {left:761px;top:362px;background-position: -240px 0px;}
        .mask5 {left:906px;top:476px;background-position: -320px 0px;}
        .mask6 {left:1017px;top:325px;background-position: -400px 0px;}
        .mask7 {left:1086px;top:213px;background-position: -480px 0px;}

        .flowed1 {width:236px;height:44px;background-position:0px -70px}
        .flowed2 {width:289px;height:256px;background-position:0px -120px }

        .maskItem1 {width:214px;margin:0px 0px 0px 10px;overflow: hidden;height:44px;line-height:38px;font-size:22px; color:#ffea00;}
        .flowedContainer {width:200px;margin:60px 0px 0px 80px;}
        .flowedItem,.flowedMaskItem {width:200px;line-height: 36px;height:36px;color:white;font-size: 22px;}
        .flowedMaskItem {color:#ffea00;}
        .after {}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            <%
            value = "";
            final String[] actions = "{l:52,t:316}|{l:529,t:278}|{l:452,t:485}|{l:776,t:421}|{l:978,t:476}|{l:851,t:382}|{l:909,t:269}".split("\\|");
                ...
                for(int i = 0; i < list.size() ; i++){
                    ...
                    for( int j = 0; j < vodList.size() ; j++ ){
                        ...
                        String str = "{ mid:'" + vod.getId() + "'," +
                                   "text:\"" + vod.getName() + "\"," +
                                  (  i == 0 && j < 7 ? "posi:" + actions[j] + "," : "" ) +
                                  (  i == 0 && j < 7 ? "style:'mask mask" + (j + 1) + "'" + "," : "" ) +
                                 "length:" + StringUtil.length(vod.getName()) + "," +
                              "shortText:\"" + StringUtil.limitStringLength(vod.getName(), i == 0 ? 20 : 18) + "\"" + "," +
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && ( index == 11 && ( focus == 0 || focus == 1 || focus == 3 || focus == 6 ) || index == -1 && focus == 0 || index == 6 && index == 1 || index == -11 && focus == 2 ) || blocked == 1 && focus + 1 >= focusable[1].items.length && index == -11 ) return;
                    if( blocked == 0 ) {
                        if( focus == 0 && (index == 1 || index == -11 ) || focus == 1 && index == -1 ) {
                            blocked = 1; focus = focusable[blocked].focus;
                        } else if( index == -1 ) {
                                focus -= 1;
                        } else if( index == 1) {
                                focus += 1;
                        }  else if( index == 11 && focus <= 3 || index == -11 && focus >= 4  ) {
                            if( index == -11 && focus == 4 ) {
        ...
        var flowedShow = function(index){
            ...
            $("flowed").className = "mask " + ( blocked == 0 ? "flowed1" : "flowed2");
            var html = "";
            if( blocked == 0 ) {
                var item = items[focus];
                $("mask").className = item.style;
                $("flowed").style.left = item.posi.l + "px";
                $("flowed").style.top = item.posi.t + "px";
                html += "<div class='maskItem1'>"
                if (item.text != item.shortText ) {
                    html += "<marquee>";
                    html += item.text;
                    html += "</marquee>";
                } else
                ...
                $("flowed").style.left =  "322px";
                $("flowed").style.top = "358px";
                //每页显示数量
                pageCount = 5;
                flowCursorIndex = Math.floor(focus / pageCount) * pageCount;
                var html = '<div class="flowedContainer">';
                var length = items.length - flowCursorIndex >= pageCount ? pageCount : items.length - flowCursorIndex;
                for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                    var item = items[i];
                    if( i == focus ) {
                        html += "<div class='flowedMaskItem'>" + ( item.text != item.shortText ? ( '<marquee>' + item.text + '</marquee>') : item.text ) + "</div>";
                    } else {
                        html += "<div class='flowedItem'>" + item.shortText + "</div>";
                    }
                }
                html += "</div>";
            }
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-08-12.jpg') no-repeat;" onUnload="exit();"></body>
</html>