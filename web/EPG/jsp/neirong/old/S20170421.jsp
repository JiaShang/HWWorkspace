...
<%
    ...
    final String[] types = {"10000100000000090000000000107603","10000100000000090000000000107604"};
    final int[] position = {50,0,50,0};
    //TODO:每次需要修改
...
<head>
    <title>经典IP改编的影视剧</title>
    <style>
        .mask {width:163px;height:57px;top:258px;position:absolute;background:transparent url('images/mask-2017-04-21.png') no-repeat;background-position: 0px 0px;}
        .mask1{left:401px;background-position: 0px 0px;}
        .mask2{left:685px;background-position: 0px -60px;}

        .flowed {position:absolute;width:580px;height:250px;left:389px;top:349px;overflow: hidden;}
        .item {width:284px;height:60px;float:left;overflow: hidden;}
        .itemContainer,.itemContainer .marquee{width:266px;height:45px;line-height:40px;font-size:22px;background-color: transparent;}
        .itemContainer{color:white;overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;margin:0px 0px 0px 12px;}
        .focus {background-color:#E3D72E; color:black;}
        .after {}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,items : [
                {name:'电视剧',style:'mask mask1'},
                {name:'电影',style:'mask mask2'}
            ]},
            <%
            value = "";
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && !$.vote.tooltiped && blocked == focusable.length - 1 && focus == 1 ) focus = focus + 1;
                    ...
                    if( blocked == 0 && ( index == 11 || index == -1 && focus <= 0 || index == 1 && focus >= 1 ) ) return;
                    if( blocked == 0 ) {
                        ...
                        if( index == 11 ) {
                            ...
                            if( focus % 8 >= 6 || focus + 2 >= focusable[blocked].items.length )  return;
                            focus += 2;
                        } else if( index == -1) {
                            if( focus % 2 == 0 ) {
                                if ( focus < 7 ) return;
                            ...
                            if( focus % 2 == 1 ) {
                                if( Math.ceil( focus / 8.0 ) >= Math.ceil( focusable[blocked].items.length / 8.0 )) return;
                                focus += 7;
                            } else
                                focus += 1;
                        }
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                var item = items[i];
                html += "<div class='item'>";
                html += "<div class='itemContainer" + ( i == focus && blocked == index ? " focus" : "" )  + "'>";
                html += ( i == focus && blocked == index ? ( item.text == item.shortText ? item.text : ("<marquee class='marquee'>" + item.text + "</marquee>")) : item.shortText );
                html += "</div></div>";
        ...
        function init(){
            ...
            if ( blocked > 1 ) focusable[0].focus = blocked - 1;
            $.buildUserInterface({flowed:[1,2],vote:undefined,video:undefined,position:undefined,external:{after:undefined,before:undefined}}); //'<div class="after"></div>'
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-04-21.jpg') no-repeat;" onUnload="exit();"></body>
</html>