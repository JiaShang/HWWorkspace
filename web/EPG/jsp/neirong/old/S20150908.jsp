...
<%
    ...
    final String[] types = {"10000100000000090000000000105485"};
...
<head>
    <title>“韩剧专区今秋特别企划”第一弹</title>
    <style>
        .mask {width:144px;height:78px;position:absolute;background:transparent url('images/mask-2015-09-08.png') no-repeat fixed;background-position: 0px 0px;}

        .mask1 {left:589px;top:323px;background-position: 0px 0px;}
        .mask2 {left:586px;top:374px;background-position: 0px -80px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            ...
            %>
            { focus:0, items : [
                {name:'精彩回顾',style:'mask mask1'},
                {name:'我要抢票',playType:-2,style:'mask mask2'}
            ]},
        ...
        var columns = 1; rows = 2;              //定义行列，　几行几列
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 1 && ( focus == 0 && index == 11 || index == -1 || index == 1 ) ) return;
                    if( blocked == 1 ) {
                        if ( index == 11 )
                            focus -= 1;
                        else {
                            focus += 1;
                            if( focus >= 2 ) { blocked = focusable.length - 1; focus = 0; }
                        }
        ...
        function init(){
            ...
            $.vote = function (){
                ...
                if( blocked == focusable.length - 2 && focusable[focusable.length - 2].focus == focusable[focusable.length - 2].items.length - 1 ) { goBack(); return; }
                    ...
                    var url = "http://192.168.49.56:8080/voteNew/external/addVote.ipanel?icid=" + cardId + "&repeat=false&phone=" + $.vote.phone +"&classifyID=" + $.vote.current.id + "&content=" + encodeURIComponent('抢票');
            ...
            $.buildUserInterface({flowed:undefined,popup:[{blocked:1,item:0,target:0,style:'mask mask1'}],vote:[{blocked:1,item:1,id: 235}],video:undefined,position:undefined,gatewayCheck:undefined,external:{after:undefined,before:undefined}});
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-09-08.jpg') no-repeat;" onUnload="exit();"></body>
</html>