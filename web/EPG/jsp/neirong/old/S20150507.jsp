...
<%
    ...
    final String[] types = {"10000100000000090000000000105105"};
...
<head>
    <title>女司机是不是“马路杀手”？</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2015-05-07.png') no-repeat fixed;background-position: 0px 0px;}
        .maskItems{left:757px;height:27px;width:444px;}
        .mask1 {top:100px; background-position: -70px -15px;}
        .mask2 {top:150px; background-position: -70px -65px;}
        .mask3 {top:203px; background-position: -70px -118px;}
        .mask4 {top:256px; background-position: -70px -171px;}
        .mask5 {top:309px; background-position: -70px -224px;}
        .mask6 {top:360px; background-position: -70px -275px;}
        .mask7 {top:413px; background-position: -70px -328px;}
        .mask8 {top:465px; background-position: -70px -380px;}
        .mask9 {top:518px; background-position: -70px -433px;}
        .mask10 {top:570px; background-position: -70px -485px;}

        .voteItems {width:202px;height:37px;}
        .vote1 {left:60px;top:481px;background-position: -60px -545px;}
        .vote2 {left:274px;top:481px;background-position: -274px -545px;}
        .vote3 {left:492px;top:481px;background-position: -492px -545px;}
        .vote4 {left:61px;top:586px;background-position: -61px -586px;}
        .vote5 {left:275px;top:586px;background-position: -275px -586px;}
        .vote6 {left:492px;top:586px;background-position: -492px -586px;}
        .vote7 {left:61px;top:627px;background-position: -61px -627px;}
        .vote8 {left:275px;top:627px;background-position: -275px -627px;}
        .vote9 {left:492px;top:627px;background-position: -492px -627px;}
        .flowed {}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            ...
            %>
            { focus:0,voted:false, items : [
                {name:'A.绝对的“马路杀手”',playType:-2,style:'mask voteItems vote1'},
                {name:'B.男司机违章更多',playType:-2,style:'mask voteItems vote2'},
                {name:'C.文明出行 不分男女',playType:-2,style:'mask voteItems vote3'}
            ]},
            { focus:0,voted:false, items : [
                {name:'A.随意停车',playType:-2,style:'mask voteItems vote4'},
                {name:'B.转弯/变道不打灯',playType:-2,style:'mask voteItems vote5'},
                {name:'C.朝车窗外扔垃圾',playType:-2,style:'mask voteItems vote6'},
                {name:'D.逆行',playType:-2,style:'mask voteItems vote7'},
                {name:'E.会车不换近光',playType:-2,style:'mask voteItems vote8'},
                {name:'F.行人乱穿马路',playType:-2,style:'mask voteItems vote9'}
            ]},
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0  && ( index == 11 && focus < 1 || index == 1 ) ||
                        blocked == 1 && ( focus == 0 && index == -1 || index == 11 ) ||
                        blocked == 2 && ( focus % 3 == 0 && index == -1 || focus >=3 && index == -11 )
                    ) return;
                    if( blocked == focusable.length - 1) {
                        blocked = index == 11 ? 0 : 2; focus = index == 11 ? focusable[blocked].items.length - 1 : 2;
                    } else if( blocked == 0 ) {
                        if( index == -1 ) { blocked = focus <= 8 ? 1 : 2 ; focus = 2; }
                        ...
                        if( index == 1 ) {
                            if( focus % 3 == 2 ) {
                                if( blocked == 1 ) {
                                    blocked = 0; focus = focusable[blocked].items.length > 7 ? 7 : focusable[blocked].items.length - 1;
                                } else if( focus == focusable[blocked].items.length - 1 ) {
                                    blocked = focusable.length -1; focus = 0;
                                } else {
                                    blocked = 0; focus = focusable[blocked].items.length - 1;
                                }
                            } else focus += 1;
                        } else if( index == -1 ) {
                            focus -= 1;
                        }  else if( index == -11 ) {
                            if (blocked == 1) { blocked = 2; } else { focus += 3; }
                        } else if( index == 11 ) {
                            if (focus  > 2) { focus -= 3; } else { blocked = 1; }
                        }
                    }
                }
                ...
                if( blocked == focusable.length - 2 ) return;
        ...
        function init(){
            ...
            $.vote = function (){
                if( focusable[focusable[focusable.length - 2].backed].voted ) { if( !$.vote.tooltiped ) $.vote.tooltip($.vote.fail); else goBack();return; }
                ...
                if( blocked == focusable.length - 2 && focusable[focusable.length - 2].focus == focusable[focusable.length - 2].items.length - 1 ) { goBack(); return; }
                    ...
                    $.ajax(url,function(result){
                        if(result.recode != '002' || result.result == false ){
                            focusable[focusable[focusable.length - 2].backed].voted = result.result == false;
                            $.vote.tooltip($.vote.fail);
                        } else {
                            focusable[focusable[focusable.length - 2].backed].voted = true;
            ...
            $.buildUserInterface({flowed:undefined,popup:undefined,vote:[{blocked:1,item:undefined,id:210},{blocked:2,item:undefined,id:211}],video:undefined,position:undefined,external:{after:undefined,before:undefined}});
            if( focusable.length == 2 ) {blocked = focusable.length - 1;}

            focused(<%= focused %>, true);
        }
        function goBack(keyBack){
            if( blocked == focusable.length - 2) { if(focusable[focusable.length-2].focus == 0 && !focusable[focusable[focusable.length - 2].backed].voted)  {$.vote.delete("phoneNumber"); return;} else $.vote.hidden();}
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-05-07.jpg') no-repeat;" onUnload="exit();"></body>
</html>