...
<head>
    <title>吃遍重庆</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2015-09-30.png') no-repeat fixed;background-position: 0px 0px;}
        .item {width:252px;height:110px;background-position: 0px 0px;}
        .itemCol0{left:65px;}
        .itemCol1{left:364px;}
        .itemCol2{left:664px;}
        .itemCol3{left:964px;}
        ...
        .vote {width:34px;height:52px;background-position: -300px 0px;}
        .voteCol0{left:298px;}
        .voteCol1{left:597px;}
        .voteCol2{left:897px;}
        .voteCol3{left:1197px;}
        .voteRow0 {top:284px;}
        .voteRow1 {top:421px;}
        .voteRow2 {top:564px;}

        .mask1{width:375px;height:57px;top:211px;left:855px;background-position: -400px 0px;}
        .flowed {}
        ...
        .btnSure{position:absolute;width:117px;height:42px;left:492px;top:378px;background: transparent url("images/voteBg.png") no-repeat fixed;background-position: 0px -300px;}
        .btnCancel{position:absolute;width:116px;height:42px;left:704px;top:377px;background: transparent url("images/voteBg.png") no-repeat fixed;background-position: 0px -350px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            ...
            %>
            { focus:0, items : [
                {playType:-3,link:'/EPG/jsp/neirong/S20150930-1.jsp',style:'mask mask1'}
            ]},
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && ( focus <= 6 && focus % 2 == 1 && index == 11 ||
                                          focus >= 12 && focus % 2 == 0 && index == -11 ||
                            ( focus == 0 || focus == 1 || focus == 8 || focus == 9 || focus == 14 || focus == 15 ) && index == -1 ||
                            ( focus == 6 || focus == 7 || focus == 12 || focus == 13 ) && index == 1 ) ||
                        blocked == 1 && ( index == 1 || index == -1 || index == 11 ) ) return;
                     if( blocked == 0 ) {
                         if( index == 1 || index == -1 ) {
                             if( focus >= focusable[0].items.length - 2 && index == 1 ) { blocked = focusable.length - 1; focus = 0; }
                             else focus += index * 2;
                         } else if( index == 11 ){
                             if( focus >= 14 ) focus += focus % 2 == 0 ? 1 : -7;
                             else if( focus >= 8 ) focus += focus % 2 == 0 ? 1 : -9;
                             else if( focus == 7 ) {blocked = 1; focus = 0;}
                             else focus += 1;
                         } else if( index == -11 ) {
                             if( focus <= 7 ) {
                                 if( focus == 6 ) { blocked = focusable.length - 1; focus = 0;  }
                                 else focus += focus % 2 == 1 ? -1 : 9;
                             } else if( focus <= 13 ) focus += focus % 2 == 1 ? -1 : 7;
                             else focus -= 1;
                         }
                     } else if( blocked == 1 ) {
                         blocked = 0; focus = 7;
                     } else {
                         focus = index == -1 ? focusable[0].items.length - 2 : 6;
                         blocked = 0;
                     }
                }
            ...
            if(typeof item != 'undefined' && typeof item.style == 'string' )
                ...
                var tp = focus % 2 == 0 ? 'item' : 'vote';
                var col = 0, row = 0;
                var f = ( focus + ( focus % 2 == 0 ? 0 : -1 ) ) / 2;
                var row = f <= 3 ? 0 : f < 7 ? 1 : 2 ;
                var col = f <= 3 ? f % 4 : f < 7 ? ( f - 4 ) % 3  : ( f - 7 ) % 2 ;
                $('mask1').className = 'mask ' + tp + " " + tp + 'Row' + row + ' ' + tp + 'Col' + col ;
            }
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:undefined,popup:undefined,vote:[{blocked:0,item:undefined,id: 238}],video:undefined,position:undefined,gatewayCheck:undefined,external:{after:'<div id="mask1"></div><div style="width:1109px;height:304px;position: absolute;left: 69px;top:385px;background:transparent url(images/mask-2015-09-30.png) no-repeat fixed 0px -120px;"></div>',before:undefined}});
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-09-30.jpg') no-repeat;" onUnload="exit();"></body>
</html>