...
<%
    ...
    final String[] types = {"10000100000000090000000000107662","10000100000000090000000000107663"};
    final int[] position = {7,0,50,0};
    //TODO:每次需要修改
    int area = 1;
    final int popItemMaxCharLength = 20;

    String backUrl = "",focused = "";
    String fromEPG = "";
    String value = "";
...
<head>
    <title>妈妈我爱您</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2017-05-12.png') no-repeat;background-position: 0px 0px;}
        .title {width:276px;height:54px;left:89px;top:301px;}
        .title1 {background-position: 0px 0px;}
        .title2 {background-position: 0px -60px;}

        .flowed {position:absolute; width:600px;height:220px;left:116px;top:387px;}
        .itemContainer {width:293px;height:48px;float: left;overflow: hidden;}
        .itemContext,.itemContextFocus{width:253px;height:37px;}
        .itemContextFocus{background:transparent url('images/mask-2017-05-12.png') no-repeat 0px -120px;}
        .item,.itemFocus{width:233px; margin: 0px 10px 0px 10px; text-align: center;font-size: 22px;line-height: 30px;overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis; color:#d75e5e;}
        .itemFocus {color:#f9e9e1}
        .marquee { width:233px; font-size: 22px;line-height: 30px;}

        .arrow {width:24px;height:22px;left:420px;}
        .arrowUp {top:360px;background-position:0px -160px;}
        .arrowDown {top:581px;background-position:-50px -160px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && !$.vote.tooltiped && blocked == focusable.length - 1 && focus == 1 ) focus = focus + 1;
                    ...
                    if( blocked == 0 && ( index == 11 || index == -1 && focus <= 0 || index == 1 && focus >= 1) ||
                        ( blocked == 1 || blocked == 2 ) && ( index == -1 && focus % 2 == 0 || index == 1 && focus % 2 == 1 )) return;
                    if( index == 1 || index == -1 ) {
                        focus += index;
                    } else {
                        if ( index == 11 ) {
            ...
            $("title").className = focusable[0].focus == 0 ? "mask title title1" : "mask title title2";
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                ...
                if( i == focus && blocked == index ){
                    html += '<div class="itemContextFocus">';
                    html += '<div class="itemFocus">';
            ...
            if( items.length <= pageCount ){
                ...
                $("arrowUp").style.visibility = focusable[index].focus < pageCount ? 'hidden' : 'visible';
                $("arrowDown").style.visibility = Math.ceil ( (focusable[index].focus + 1.0) / pageCount ) * pageCount <= items.length ? 'visible' : 'hidden';
            }
        }
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:[1,2],vote:undefined,video:undefined,position:undefined,external:{after:'<div class="mask title <%= area == 2 ? "title2" : "title1"%>" id="title"></div><div id="arrowUp"></div><div id="arrowDown"></div>',before:undefined}}); //
            var f = '<%=focused%>'.split(',');
            blocked = Number(f[0]);
            for( var i = 1; i < f.length; i ++)
                focusable[i - 1].focus = Number( f[i]);
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-05-12.jpg') no-repeat;" onUnload="exit();"></body>
</html>