...
<%
    ...
    final String[] types = {"10000100000000090000000000105947"};
...
<head>
    ...
    <style>
        .mask {width:307px;height:54px;left:443px;position:absolute;background:transparent url('images/mask-2016-03-08.png') no-repeat fixed;background-position: 0px 0px;}
        .mask1{top:456px;}
        .mask2{top:496px;}
        .mask3{top:536px;}
        .mask4{top:576px;}
        .mask5{width:255px;height:37px;top:364px;left:905px;background-position: 0px -120px;}

        .maskRole{width:160px;height:45px;left:218px;top:533px;background-position: 0px -60px;}
        .focusBg{width:662px;height:246px;left:237px;top:369px;position:absolute;background:transparent url('images/focusBg-2016-03-08.png') no-repeat fixed 0px 0px;}
        .focusBg1{width:430px;height:80px;left:860px;top:462px;position:absolute;background:transparent url('images/focusBg-2016-03-08Mask.png') no-repeat fixed 0px 0px;}

        .mask11{width:410px;height:65px;left:860px;top:462px;position:absolute;background:transparent url('images/focusBg-2016-03-08Bg.png') no-repeat fixed 0px 0px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,items : [{name:'活动细则',style:'mask maskRole',playType:-3,link:'/EPG/jsp/neirong/S20160308Rule.jsp'}]},
            <%
            value = "";
            ...
            %>
            { focus:0,items : [
                {name:'链接',style:'mask mask5',playType:-3,link:'http://192.168.35.153:8080/programDetail.jsp?programId=387&videoId=0&lcn=shengli'},
                {name:'链接',style:'mask11',playType:-3,link:'http://192.168.98.65:10001/IPTVGameEPG/index.jsp?recoCode=LTR_10155'}
            ]},
        ...
        var columns = 1; rows = 4;              //定义行列，　几行几列
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && ( index == -1 || index == 11 || index == -11 ) ||
                        blocked == 1 && ( focus == 0 && index == 11 || focus == focusable[1].items.length - 1 && index == -11 ) ||
                        blocked == 2 && ( index == 1 || focus == 0 && index == 11) ) return;
                    if( blocked == 0 ) { blocked = 1; focus = focusable[blocked].focus; }
                    else if( blocked == 2 ) {
                        if( index == 11 ) { focus -= 1; }
                        else if( index == -11 ){
                            focus += 1;
                        ...
                        blocked = index == 11 ? 2 : 1; focus = focusable[blocked].focus;
                    }
                    else if( blocked == 1) {
                        if( index == -1 ){ blocked = 0; focus = 0;}
                        else if(index == 1){ blocked = 2; focus = 0;}
                        else focus += index > 1 ? -1 : 1;
                    }
                }
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:undefined,popup:undefined,vote:undefined,video:undefined,position:undefined,gatewayCheck:undefined,external:{after:'<div class="focusBg"></div>',before:'<div class="focusBg1"></div>'}});
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-03-08.jpg') no-repeat;" onUnload="exit();"></body>
</html>