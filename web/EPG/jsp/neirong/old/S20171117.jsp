...
//  模板       10000100000000090000000000108662
//  医疗小常识  10000100000000090000000000108663
//  我是大医生  10000100000000090000000000108664
    final String[] types = {"10000100000000090000000000108662","10000100000000090000000000108663","10000100000000090000000000108664"};
    if( !StringUtils.isEmpty(typeId))
        types[0] = typeId;

    final int[] position = {50,0,50,0,50,0};
    //TODO:每次需要修改
...
<head>
    ...
    <style>
        .mask {position: absolute;background: transparent url("images/mask-2017-11-17.png") no-repeat top left;}
        ...
        .mask11 {left:394px; top:242px; width:287px; height:156px;background-position: 0px 0px;}
        .mask12 {left:634px; top:242px; width:287px; height:156px;background-position: -300px 0px;}

        .flowed {width:1200px;left:89px;top:383px;height:285px;overflow:hidden;position: absolute;}

        .focusBg {width:1280px;height:720px;left:0px;top:0px;position: absolute; background:transparent url('images/focusBg-2017-11-17.png') no-repeat top left; overflow: hidden;}
        .title1 {left:475px;top:123px;width:353px;height:87px;background-position:0px -160px;}
        .title2 {left:475px;top:123px;width:353px;height:87px;background-position:0px -260px;}

        .focusBgFlowed{ width:300px;height:366px;left:495px;top:233px; position:absolute; overflow: hidden;}
        .flowItemContainer{width:300px;height:61px;overflow: hidden;float: left;overflow: hidden;}
        .flowItemIcon,.flowItemIconFocus { width:27px;height:61px;float: left;}
        .flowItemIconFocus{ background: transparent url('images/mask-2017-11-17.png') no-repeat -500px -279px;}
        .flowItem,.flowItemFocus {width:273px;height:61px;float:left;font-size: 22px; line-height: 56px; color:white;overflow:hidden;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;}
        .flowItemFocus {font-weight:bold;font-size: 24px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
                ...
                for(int i = 0; i < list.size() ; i++){
                    ...
                    for( int j = 0; j < vodList.size() ; j++ ){
                        ...
                        String str = "{ mid:'" + vod.getId() + "'," +
                              ...
                              "shortText:\"" + StringUtil.limitStringLength(vod.getName(), i == 0 ? 14 : 22 ) + "\"" + "," +
                                   (i == 0 ? "picture:'" + Utils.pictureUrl("images/default_preview_lb.png", vod.getPosters(), "1", request)+ "'" + ","  : "" )+
            ...
            %>
            { focus:0,items : [
                {name:'医疗小常识',style:'mask mask11'},
                {name:'我是大医生',style:'mask mask12'},
            ]},
        ...
        var columns = <%= ct %>; rows = 1;      //定义行列，　几行几列.
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && !$.vote.tooltiped && blocked == focusable.length - 1 && focus == 1 ) focus = focus + 1;
                    ...
                    if( blocked == 0 && ( index == -11  || index == -1 && focus <= 0 || index == 1 && focus + 1 >= focusable[blocked].items.length ) ||
                        (blocked == 1 || blocked == 2 ) && (index == 1 || index == -1 || index == 11 && focus <= 0 || index == -11 && focus + 1 >= focusable[blocked].items.length) ||
                        blocked == 3 && ( index == 11 || index == -1 && focus <= 0 || index == 1 && focus >= 1 ) ) return;
                    if( blocked == 0 || blocked == 3 ) {
                        if( index == 11 || index == -11 ) {
                            blocked = index == 11 ? 3 : 0; focus = focusable[blocked].focus;
                        } else {
                            focus += index;
                        }
                    } else {
                ...
                if( blocked == 1 || blocked == 2 ) {
                    $("title").className = "mask title" + blocked;
                    $('focusBg').style.visibility = 'visible';
                }
            }
            ...
            flowedShow(blocked == 1 || blocked == 2 ? blocked : 0);
        }
        var flowedShow = function(index){
            ...
            if( index === 0  ) {
                ...
                for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                    ...
                    html += '<div class="' + ( ( i == focus && blocked == index ) ? 'maskImage' : 'image' ) + '">';
                    html += '<img src="' + item.picture + '" style="width:178px;height:232px;"/>'
                    html += '</div>';
                    html += '<div class="text" ' + ( ( i == focus && blocked == index ) ? ' style="color:#<%=fc%>;top:6px;"' : '' ) + '>';
                    if( i == focus && blocked == index ){
                        html += item.text != item.shortText ? ( '<marquee class="maskMarquee">' + item.text + '</marquee>') : item.text;
                    } else {
                        html += item.shortText;
                    }
                    html +='</div></div></div>';
                ...
                for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                    var item = items[i];
                    html += '<div class="flowItemContainer">';
                    if( i == focus && blocked == index ){
                        html += '<div class="flowItemIconFocus"></div><div class="flowItemFocus">';
                        html += item.length >= 20 ? ( '<marquee style="line-height: 55px;">' + item.text + '</marquee>') : item.text;
                    } else {
                        html += '<div class="flowItemIcon"></div><div class="flowItem">';
                        html += item.shortText;
                    }
                    html +='</div></div>';
                }
                $("focusBgFlowed").innerHTML = html;
            }
        }
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:[0],vote:undefined,video:undefined,position:undefined,external:{after:'<div class="focusBg" id="focusBg" style="visibility: hidden;"><div id="title"></div><div id="focusBgFlowed" class="focusBgFlowed"></div></div>',before:undefined}}); //
            focused(<%= focused %>, true);
        }
        function goBack(keyBack){
            if( blocked == 1 || blocked == 2 ) {
                focusable[3].focus = blocked - 1 ;
                blocked = 3;
                $("focusBg").style.visibility = 'hidden';
        ...
        function doEnter(){
            ...
            if( blocked === 3 ) {
                blocked = focusable[3].focus + 1;
                $("title").className = "mask title" + blocked;
                $('focusBg').style.visibility = 'visible';
                flowedShow( blocked ); return;
            }
            //首页，返回
        ...
        function eventHandler(eventObj, __type){
            switch(eventObj.code){
                ...
                case "KEY_BACK":goBack(true); return false; break;
                case "KEY_EXIT":
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-11-17.jpg') no-repeat;" onUnload="exit();"></body>
</html>