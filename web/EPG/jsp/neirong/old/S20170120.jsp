...
<%
    ...
    final String[] types = {"10000100000000090000000000104935"};
    if( !StringUtils.isEmpty(typeId))
        types[0] = typeId;

    final int[] position = {20,0};
    //TODO:每次需要修改
    int focused = 0, area = 0;
    int popItemMaxCharLength = 24;
    ...
    String cl = null, bc = null,fc = null;
    ...
    try {
        ...
        w = StringUtils.isEmpty( request.getParameter("w") ) ? 237 : Integer.valueOf(request.getParameter("w"));
        h = StringUtils.isEmpty( request.getParameter("h") ) ? 73 : Integer.valueOf(request.getParameter("h"));
        ih = StringUtils.isEmpty( request.getParameter("ih") ) ? 40 : Integer.valueOf(request.getParameter("ih"));
        fs = StringUtils.isEmpty( request.getParameter("fs") ) ? 22 : Integer.valueOf(request.getParameter("fs"));
        mr = StringUtils.isEmpty( request.getParameter("mr") ) ? 8 : Integer.valueOf(request.getParameter("mr"));

        lt = StringUtils.isEmpty( request.getParameter("lt") ) ? 73 : Integer.valueOf(request.getParameter("lt"));
        tp = StringUtils.isEmpty( request.getParameter("tp") ) ? 353 : Integer.valueOf(request.getParameter("tp"));
...
<head>
    <title>一竖排的专题模板</title>
    <style>
        .mask {position:absolute;}
        .mask11 {width:181px;height:40px;left:1005px;top:647px;background: transparent url('images/mask-2017-01-20.png') no-repeat;background-position: 0px 0px;}

        .flowed {width:<%=w %>px;height:<%= h %>px;top:<%=tp%>px;left:<%=lt%>px;position:absolute;}
        .item,.focusItem {height:<%=ih + mr %>px; color:#<%=cl%>; overflow:hidden;text-align: center;}
        .focusItem{color:#<%=fc%>;background-color:#<%=bc%>;}
        .focusItem .marquee,.item .inner,.focusItem .inner{width:<%= w-20 %>px; height:<%= ih%>px;font-size:<%=fs %>px;line-height:<%= ih-6 %>px; overflow:hidden;}
        .item .inner,.focusItem .inner{margin:4px 0px 0px 10px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            ...
            %>
            { focus:0,items : [{name:'动画原声',style:'mask mask11',playType:-3,link:'http://192.168.34.81/HiFi_cq/wailiu/wl/HiFi_songList.html?albumId=wl_009&type=0&wlColor=ffffff_000000'}]},
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked == 0 && (
                         ...
                         index == 1 && ( <%=lt%> > 750 && columns == 1 ||  focus % columns == columns - 1 && columns != 1 ) || //按右光标键时
                     ...
                     if( blocked == 1) { blocked = 0;focus = focusable[blocked].focus; }
                     else {
                         if( index == 1 ) { if( columns == 1 ) { blocked = 1;focus = 0; } else focus += index; }
                         else if( index == -1 ) focus += index;
                         else if( index == 11 ) focus -= columns;
                         else if( index == -11 ) focus += columns;
                         if( focus >= focusable[blocked].items.length ) { return; }
                     }
                }
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                var item = items[i];
                html += '<div class="' + ( ( i == focus && blocked == index ) ? 'focusItem' : 'item' ) + '">';
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:[0],popup:undefined,vote:undefined,video:undefined,position:undefined,external:{after:undefined,before:undefined}});
            ...
            switch(item.playType){
                ...
                case -3:
                    ...
                    if( ! item.link.startWith('http') ){
                        ...
                        url += url.indexOf("?") > 0 ? '&' : '?';
                        url += 'backURL=';
                        var req = "<%= request.getRequestURL().toString() %>";
                        var query = "<%= StringUtils.isEmpty(request.getQueryString()) ? "" : request.getQueryString() %>";
                        if( ! query.isEmpty() ) req += "?" + query;
                        req += req.indexOf("?") >= 0 ? "&" : "?";
                        req += 'currFoucs=' + blocked + "," + focusable[blocked].focus;
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2017-01-20.jpg') no-repeat;" onUnload="exit();"></body>
</html>