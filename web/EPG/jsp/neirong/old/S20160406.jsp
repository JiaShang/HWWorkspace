...
<%
    ...
    final String[] types = {"10000100000000090000000000106043"};
...
<head>
    <title>祖孙乐趣味运动会</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2016-04-06.png') no-repeat fixed;background-position: 0px 0px;}

        .maskImgText {width:344px;height:48px;left:855px;top:51px; background-position: 0px -50px; }
        .maskLink {width:432px;height:266px;left:0px;top:487px; background-position: 0px -100px; }

        .flowedItem{width:370px;height:49px;left:724px;font-size:32px;color:#ECECFB;position: absolute;overflow: hidden;padding:0px 15px;line-height:48px;}
        .mask1{top:155px;}
        .mask2{top:215px;}
        .mask3{top:275px;}
        .mask4{top:335px;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,items : [{name:'链接',style:'mask maskLink',playType:-3,link:'http://192.168.35.129:4567/CyberCloud_Dispatcher.htm?CyberEntryID=1&CyberSiteCode=CS00001'}]},
            <%
            value = "";
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                 ...
                 if(blocked == 0 && ( index == 11 || index == -11 || index == -1 ) ||  blocked == 1 && ( index == 11 && focus < 1 || index == 1 || index == -11 && focus >= focusable[blocked].items.length > focusable[blocked].items.length ) ) return;
                 //TODO:当光标在首页、返回按钮上时，可同时按上，左键返回到列目中时，需要重写光标移动代码
                 if( blocked == focusable.length - 1 || blocked == 0) { blocked = index == -1 ? 0 : 1;focus = focusable[blocked].focus; }
                 else {
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < length; i++ ) {
                var item = items[i];
                $("flowed" + ( i + 1 )).innerText = item.text;
                if( i == focus && blocked == index )
                    $("flowed" + ( i + 1 )).className = "flowedItem mask" + ( i + 1) + " mask";
                else
                {
                    $("flowed" + ( i + 1 )).className = "flowedItem mask" + ( i + 1);
                    $("flowed" + ( i + 1 )).style.backgroundImage = "none";
                }
            }
        }
        function init(){
            ...
            $.buildUserInterface = function(option){
                ...
                pare.setMonth(3);
                pare.setDate(10);
                if( cur - pare.getTime() > 0 )
                    html += "<div class='mask maskImgText'></div>";

                html += "<div id='flowed1'></div><div id='flowed2'></div><div id='flowed3'></div><div id='flowed4'></div><div id='flowed5'></div><div id='mask'></div>";
            ...
            $.buildUserInterface({flowed:[1],popup:undefined,vote:undefined,video:undefined,position:undefined,gatewayCheck:undefined,external:{after:undefined,before:undefined}}); //'<div class="after"></div>'
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-04-06.jpg') no-repeat;" onUnload="exit();"></body>
</html>