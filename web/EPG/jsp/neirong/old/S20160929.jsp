...
<%
    ...
    final String[] types = {"10000100000000090000000000106724","10000100000000090000000000106730","10000100000000090000000000106727","10000100000000090000000000106729","10000100000000090000000000106725","10000100000000090000000000106726"};
    final int[] position = {50,0,50,0,50,0,50,0,50,0,50,0};
    //TODO:每次需要修改
...
<head>
    <title>打开火锅地图</title>
    <style>
        .mask,.arrow,.icon {position:absolute;background:transparent url('images/mask-2016-09-29.png') no-repeat;background-position: 0px 0px;}
        .mask {width:101px;height:79px;top:288px;}
        .mask1 {left:65px;}
        .mask2 {left:168px;}
        .mask3 {left:274px;}
        .mask4 {left:385px;}
        .mask5 {left:505px;}
        .mask6 {left:632px;}
        .arrow {width:46px;height:28px;left:360px;}
        .arrowUp {top:338px;background-position: 0px -100px;}
        .arrowDown {top:643px;background-position: -50px -100px;}
        .icon {width:45px;height:40px;float:left;overflow:hidden;position:static;}
        .iconFocus {background-position: 0px -150px;}
        .Empty {background-position: -50px -150px;}

        .flowed {position:absolute;height:280px;width:400px;top:370px;left:205px;overflow: hidden;}
        .itemContainer {width:400px;height:46px;color:#000000;line-height: 40px;font-size: 22px;}
        .item {width:350px;height:46px;float: left;text-align: left;overflow: hidden;}
        .after {}
    ...
    <script language="javascript" type="text/javascript">
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    if( blocked <= 0 && index == -1 || blocked + 1 >= 6 && index == 1 || focus == 0 && index == 11 || index == -11 && focus + 1 >= focusable[blocked].items.length ) return;
                    if( index == -1 || index == 1 ) {
                        blocked += index;
        ...
        var flowedShow = function(index){
            ...
            $("mask").className = "mask mask" + ( blocked + 1 );
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                ...
                if( i == focus ) {
                    html += '<div class="icon iconFocus"></div>';
                    html += '<div class="item" style="color:#C40909">' + ( item.text != item.shortText  ? ( '<marquee>' + item.text + '</marquee>' ) : item.shortText) + '</div>';
                } else {
            ...
            $("arrowUp").style.visibility = focus >= pageCount ? 'visible' : 'hidden';
            $("arrowDown").style.visibility = Math.ceil((focus + 1.0) / pageCount) < Math.ceil( items.length * 1.0 / pageCount ) ? 'visible' : 'hidden';
        }
        ...
        function init(){
            ...
            $.current = function (){
                ...
                return "/EPG/jsp/defaultHD/en/SaveCurrFocus.jsp?currFoucs=" + fstr.substr(0, fstr.length - 1 ) + "&url=";
            };
            ...
            $.buildUserInterface({flowed:[0,1,2,3,4,5],popup:undefined,vote:undefined,video:undefined,position:undefined,external:{after:undefined,before:'<div id="arrowUp"></div><div id="arrowDown"></div>'}});
            if( focux.isEmpty() ) {
                blocked = 0; focux = 0;
            } else {
                focux = focux.split(",");
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-09-29.jpg') no-repeat;" onUnload="exit();"></body>
</html>