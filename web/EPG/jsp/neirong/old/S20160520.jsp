...
<head>
    <title>决胜中高考2016</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2016-05-20.png') no-repeat;background-position: 0px 0px;}


        .flowed {width:384px;height:255px;left:721px;top:326px;position:absolute;}
        .itemContainer,.itemFocusContainer {width:384px;height:51px;overflow: hidden; background: transparent url('images/mask-2016-05-20.png') no-repeat 0px -68px;}
        .itemFocusContainer{background-position:0px -132px;}
        .item{width:340px;margin:0px 0px 0px 40px;color:#2380FF;font-size:22px;line-height: 40px;}
        ...
        .btnHome {width:93px;height:47px;left:1135px;top:35px;background-position: 0px 0px;}
    </style>
    <script language="javascript" type="text/javascript">
        ...
        var focusable = focusable ? focusable : [
            { focus:0,items : [
                {name:'名师押题，中考英语答题技巧，稳中求胜',length:<%=StringUtil.length("名师押题，中考英语答题技巧，稳中求胜")%>,shortText:'<%= StringUtil.limitStringLength("名师押题，中考英语答题技巧，稳中求胜",popItemMaxCharLength) %>',playType:-3,link:'http://192.168.9.174:80/dvb_cqocn_53/welcome/home.htm?welcome=zhongkaoyingyu'},
                {name:'只有准备好，才能打胜仗',length:<%=StringUtil.length("只有准备好，才能打胜仗")%>,shortText:'<%= StringUtil.limitStringLength("只有准备好，才能打胜仗",popItemMaxCharLength) %>',playType:-3,link:'http://192.168.9.174:80/dvb_cqocn_53/welcome/home.htm?welcome=gaokaoxiaotieshi'},
                {name:'告诉你如何将紧张的心态变成满分的动力',length:<%=StringUtil.length("告诉你如何将紧张的心态变成满分的动力")%>,shortText:'<%= StringUtil.limitStringLength("告诉你如何将紧张的心态变成满分的动力",popItemMaxCharLength) %>',playType:-3,link:'http://192.168.9.174:80/dvb_cqocn_53/welcome/home.htm?welcome=bujinzhangzhuanti'},
                {name:'高考历史最后突击，重点难点全都有',length:<%=StringUtil.length("高考历史最后突击，重点难点全都有")%>,shortText:'<%= StringUtil.limitStringLength("高考历史最后突击，重点难点全都有",popItemMaxCharLength) %>',playType:-3,link:'http://192.168.9.174:80/dvb_cqocn_53/welcome/home.htm?welcome=lishizhuanti'},
                {name:'名师归纳语数外必考要点',length:<%=StringUtil.length("名师归纳语数外必考要点")%>,shortText:'<%= StringUtil.limitStringLength("名师归纳语数外必考要点",popItemMaxCharLength) %>',playType:-3,link:'http://192.168.9.174:80/dvb_cqocn_53/welcome/home.htm?welcome=mingshiyati'},
                {name:'最具权威的化学知识要点归纳',length:<%=StringUtil.length("最具权威的化学知识要点归纳")%>,shortText:'<%= StringUtil.limitStringLength("最具权威的化学知识要点归纳",popItemMaxCharLength) %>',playType:-3,link:'http://192.168.9.174:80/dvb_cqocn_53/welcome/home.htm?welcome=huaxueshiyan'},
                {name:'高考生物必备，易错知识点考前秘籍',length:<%=StringUtil.length("高考生物必备，易错知识点考前秘籍")%>,shortText:'<%= StringUtil.limitStringLength("高考生物必备，易错知识点考前秘籍",popItemMaxCharLength) %>',playType:-3,link:'http://192.168.9.174:80/dvb_cqocn_53/welcome/home.htm?welcome=shengwuxibao'},
                {name:'临考必看，中高考语数外逐个突破',length:<%=StringUtil.length("临考必看，中高考语数外逐个突破")%>,shortText:'<%= StringUtil.limitStringLength("临考必看，中高考语数外逐个突破",popItemMaxCharLength) %>',playType:-3,link:'http://192.168.9.174:80/dvb_cqocn_53/welcome/home.htm?welcome=yatimijuan'},
                {name:'阅读-盛夏',length:<%=StringUtil.length("阅读-盛夏")%>,shortText:'<%= StringUtil.limitStringLength("阅读-盛夏",popItemMaxCharLength) %>',playType:-3,link:'http://192.168.42.52:8088/epg/newEast/biz_44668345/category/ccms_category_71321892/item/subject/42726.do'},
                {name:'2016高考抢分班',length:<%=StringUtil.length("2016高考抢分班")%>,shortText:'<%= StringUtil.limitStringLength("2016高考抢分班",popItemMaxCharLength) %>',playType:-3,link:'http://192.168.42.52:8088 /epg/newEast/biz_44668345/category/ccms_category_53626879/item/subject/32464.do'},
                {name:'中考化学必考点-关于“物质”',length:<%=StringUtil.length("中考化学必考点-关于“物质”")%>,shortText:'<%= StringUtil.limitStringLength("中考化学必考点-关于“物质”",popItemMaxCharLength) %>',playType:-3,link:'http://192.168.42.52:8088/epg/newEast/biz_50576451/category/ccms_category_58454031/item/subject/32561.do'},
                {name:'五大要点拯救高考英语',length:<%=StringUtil.length("五大要点拯救高考英语")%>,shortText:'<%= StringUtil.limitStringLength("五大要点拯救高考英语",popItemMaxCharLength) %>',playType:-3,link:'http://192.168.42.52:8088/epg/newEast/biz_96413110/category/ccms_category_85035510/item/subject/41282.do?pi=6&lfid=data5'},
                {name:'中考作文高分锦囊',length:<%=StringUtil.length("中考作文高分锦囊")%>,shortText:'<%= StringUtil.limitStringLength("中考作文高分锦囊",popItemMaxCharLength) %>',playType:-3,link:'http://192.168.42.52:8088/epg/newEast/biz_50576451/category/ccms_category_58454031/item/subject/42003.do'}
            ]},
        ...
        var arrows = [0,0];                     //可以移动到首页返回按钮的光标键「下，右」
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if(
                    ...
                    blocked == focusable.length - 1 && index != -11 ||
                    //如果焦点在投票上,按上下左右键时
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    focus = focus - 1;
                     ...
                     if( blocked == 0 && ( index == 1 || index == -1 || index == -11 && focus + 1 >= focusable[0].items.length )) return;
                     if( blocked == focusable.length - 1) { blocked = 0;focus = 0; }
                     else {
                         if( index == 11 || index == -11 ) {
                             focus += index > 0 ? -1 : 1;
                             if( focus < 0 ) { blocked = focusable.length - 1; focus = 0 ;}
                         }
                     }
                }
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                ...
                {
                    html += "itemFocusContainer'>";
                    html += "<div class='item'>" + ( item.length > <%= popItemMaxCharLength %> ? ("<marquee style='width:290px;'>" + item.name + "</marquee>") : item.name  ) + "</div>";
                    html += "</div>";
                } else {
                    html += "itemContainer'><div class='item'>" + item.shortText + "</div>";
                }
                html += "</div>";
            }
        ...
        function init(){
            ...
            $.buildUserInterface({flowed:[0],popup:undefined,vote:undefined,video:undefined,position:undefined,gatewayCheck:undefined,external:{after:undefined,before:undefined}}); //'<div class="after"></div>'
        ...
        function doEnter(){
            ...
            switch(item.playType){
                ...
                case -3:
                    ...
                    break;
                case 1:
                    top.window.location.href = $.current() + "/EPG/jsp/defaultHD/en/" + ( isKorean ? "hddb/hjzq/hj_tvD" : ( !$.isGoGatewayDetail && !iPanel.eventFrame.isEpg3 ? "high_TV_d" : 'hddb/vod/tv_')) + "etail.jsp?vodId=" + item.mid + "&typeId=" + typeId;
                    break;
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2016-05-20.jpg') no-repeat;" onUnload="exit();"></body>
</html>