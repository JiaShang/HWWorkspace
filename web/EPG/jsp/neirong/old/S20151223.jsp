...
<%
    ...
    final String[] types = {"10000100000000090000000000105780"};
...
<head>
    <title>2015感动重庆年度人物</title>
    <style>
        .mask {position:absolute;background:transparent url('images/mask-2015-12-23.png') no-repeat fixed;background-position: 0px 0px;}

        .pageUp{width:107px;height:25px;top:650px;left:62px;background-position: 0px 0px;}
        .pageDown{width:107px;height:25px;top:650px;left:173px;background-position: -111px 0px;}

        .flowed {position: absolute;width:1150px;height: 390px;top:254px;left:110px;overflow:hidden; }
        .flowedItem,.item {height:188px;width:220px;overflow:hidden;float: left;}
        .item {width:158px;}
        .normal {width:137px;height:179px; position: relative;background: transparent none no-repeat 0px 0px;}
        .text {width:148px;left:5px;position:absolute;height: 48px;top:140px; font-size: 18px; text-align: center;line-height: 22px; overflow: hidden;}
    ...
    <script language="javascript" type="text/javascript">
        ...
        function focused(index,initilized){
            ...
            if(typeof index == "number" && !initilized){
                ...
                if( index == 1 && ( !$.vote.tooltiped && blocked == focusable.length - 2 && focus == 1 || blocked == focusable.length - 1 && focus == 0 ))
                    ...
                    size = focusable[0].items.length - page >= 10 ? 10 : focusable[0].items.length - page;
                    var cur = focus % 5;
                    if( blocked == 0 && ( focus <= 4 && index == 11 ||
                                            cur == 0 && index == -1 ||
                                          ( cur == 4 || focus + 1 >= focusable[0].items.length ) && index == 1 ||
                                            cur > 1 && cur < 4 && focus + 5 >= focusable[0].items.length && index == -11 )  ||
                    ...
                    if( blocked == 0 ) {
                        ...
                        else if( index == 11) focus -= 5;
                        else if( index == -11 ) {
                            if( cur == 0 && focus + 5 >= page + size) { blocked = 1; focus = 0;}
                            else if( cur == 1 && focus + 5 >= page + size) { blocked = 1; focus = 1;}
                            else if( cur == 4 && focus + 5 >= page + size) { blocked = focusable.length - 1; focus = 0;}
                            else focus += 5;
                        }
        ...
        var flowedShow = function(index){
            ...
            for(var i = flowCursorIndex; i < flowCursorIndex + length; i += 1) {
                var item = items[i];
                html += "<div class='flowedItem'><div class='item'><div class='normal' style='background-image: url(" + item.picture + ")'><img src='" + ( i == focus && blocked == index ? 'images/focus-2015-12-23.png' : 'images/normal-2015-12-23.png' ) + "' style='left:0px;top:0px;'/>";
                html += '<div class="text" style="color:' + ( i == focus && blocked == index ? '#fffd3f' : 'white' ) + ';">' + item.text.replace("（重庆发现之2015感动重庆候选人故事展播HD）","") + '</div></div></div></div>';
            }
        ...
        function doEnter(){
            ...
            //首页，返回
            if( blocked == 1 ) {
                var page = Math.floor(focusable[0].focus / 10.0) * 10;
                if( page >= 10 &&  focusable[1].focus == 0 ) {
                    focusable[0].focus -= 5;
                } else if( page + 10 < focusable[0].items.length &&  focusable[1].focus == 1)  {
                    focusable[0].focus += 5;
                }
                flowedShow(0);
                return;
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2015-12-23.jpg') no-repeat;" onUnload="exit();"></body>
</html>