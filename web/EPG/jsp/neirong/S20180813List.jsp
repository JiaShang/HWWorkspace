...
<%
    ...
    String picture = column == null ? "images/bg-2018-08-13List.jpg" : inner.pictureUrl("images/bg-2018-08-13List.jpg",column.getPosters(),"7");
...
<head>
    ...
    <style>
        ...
        .item .container,.focusItem .container{width:<%=w %>px;height:<%=ih %>px;overflow:hidden;background:transparent url('images/mask-2018-08-13.png') no-repeat left top; background-position: -300px -100px;}
        .item,.focusItem{height:<%=ih + mr %>px; color:#<%=cl%>; overflow:hidden;}
        .item .container .inner,.focusItem .container .inner{width:<%= w-40 %>px; height:<%= ih%>px;font-size:<%=fs %>px;line-height:<%= ih-6 %>px; word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;overflow: hidden;}
        .focusItem .container{background-position: -20px -20px;color:#<%=fc%>;}
        .item .container .inner,.focusItem .container .inner{margin:4px 0px 0px 20px;text-align: <%= al%>;}
        ...
        .title {width:500px;height:53px;left:215px;top:228px;position:absolute;color:white;font-size:32px;line-height: 54px;text-align: left;}

        .page {width:22px;height:16px;left:1181px;position:absolute;color:#192454;font-size:16px;overflow: hidden;text-align: center;}
        .number {top:323px;}
        .count {top:346px;}
    </style>
...
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black<%= isEmpty(picture) ? "" : (" url(" + picture + ")")%> no-repeat;" onUnload="exit();">
    ...
    <div id='title' class="title"><%= column == null ? "" : column.getName() %></div>
