...
<head>
    ...
    <style>
        .mask {width:38px;height:38px;left:882px;position:absolute;background:transparent url('images/mask-2019-07-31.png') no-repeat;background-position: 0px 0px;}
        .after {width:1280px;height:720px;left:0px;top:0px;background:transparent none no-repeat;background-position: 0px 0px;}
        .mask11 {top:263px;}
        .mask12 {top:334px;}
        .mask13 {top:414px;}
        .mask14 {top:491px;}
        .mask15 {width:96px;height:40px;left:601px;top:565px;background-position: -1100px -200px;}
        ...
        .sure,.back{width:77px;height:55px;top:259px;}
        .sure{left:714px;background-position: -800px -400px;}
        .back{left:796px;background-position: -900px -400px;}

        .phoneNumber{ position: absolute;width: 218px;height: 22px;left: 390px;top: 275px;background-color: transparent;color: black;font-size: 26px;letter-spacing: 6px;}
    </style>
    <script language="javascript" type="text/javascript" src="player/common.js"></script>
</head>
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2019-07-31-Que.jpg') no-repeat;" onUnload="exit();">
    ...
    var initialize = {
        ...
        move        :   function(index){
            ...
            if( blocked == 0 ) {
                ...
                if( index == -1 && focusP <= 0 ||  index == 1 && focusP + 1 >= items.length || index == 11 && focus <= 0 || index == -11 && focus + 1 >= 5 ) return;
        ...
        show        :   function(){
            ...
            var html = "";
            if( blocked == 0 ) {
                background = "url('images/item-2019-07-31-" + String( focus + 1 ) +  ".png')";
                ...
                if( focus != 4 ) {
                    ...
                    html += '<div class="mask mask15"></div>';
                }
            } else {
                background = "url('images/item-2019-07-31-6.png')";
                $("after").style.backgroundImage = background;
        ...
        input : function(ch){
            var blocked = cursor.blocked;
            if( blocked != 1 || cursor.phoneNumber.length >= 11 ) return;
        ...
        goBack : function(){
            ...
            if( blocked == 1 && cursor.phoneNumber.length > 0) {
        ...
        select : function(){
            ...
            if( blocked == 0 ) {
                ...
                if ( date >= '2019-08-10 23:59:59') {
                    cursor.call("showTooltip",3);
                    return;
                }
                var items = cursor.focusable[0].items;
                var course =  0;
                for( var i = 0; i < items.length ; i ++) course += items[i].selected == items[i].answer ? 20 : 0;
                cursor.call("sendVote",{id:460,target:String(course),repeat:false,callback: function(result){
