...
<script language="javascript" type="text/javascript">
    ...
    var initialize = {
        ...
        move        :   function(index){
            ...
            if( blocked === 0 && ( index == -1 && focus % cursor.col === 0 || index === 11 && focus < cursor.col || index === -11 && Math.floor( focus * 1.0 / cursor.col ) * cursor.col + cursor.col >=  items.length) ||
                blocked > 0 && (index == 11 || index == -11 || index === 1 && focus + 1 >= items.length ))return;

            if( blocked === 0 ) {
                if( index === -1 || index === 1 ) {
                    if( index == 1 && focus % cursor.col == cursor.col - 1 ) {
                        cursor.call("select"); return;
                    } else  focus += index;
                } else {
        ...
        show        :   function(init){
            ...
            if( cursor.blocked === 0 || init) {
                ...
                for( var i = flowCursorIndex; i < flowCursorIndex + length; i += 1 ){
                    var item = items[i];
                    html += "<div id='area_" + ( i + 1) + "' class='areaItem' style='background-image:url("+ cursor.pictureUrl(item.posters,3, '', (i == focus ? 1 : 0 )) + ")'></div>";
                }
                $("area").innerHTML = html;
            }
