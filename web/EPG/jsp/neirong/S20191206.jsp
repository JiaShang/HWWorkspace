...
<head>
    ...
    <style>
        ...
        .title {position:absolute;width: 180px;height: 81px;top: 63px;  background: transparent url("shang/images/J20191204focus.png") no-repeat left top;}
        .title1 {left: 677px;}
        .title2 {left: 872px;}
        .title3 {left: 1068px;}
    </style>
...
<script language="javascript" type="text/javascript">
    ...
    cursor.initialize({
        ...
        nextVideo   :   function () {
            ...
            cursor.playIndex = playIndex = playIndex + 1 < cursor.focusable[cursor.blocked].items.length ? playIndex + 1 : 0;
            var item = cursor.focusable[cursor.blocked].items[playIndex];
            cursor.call("playMovie",item);
        },
        prepareVideo : function(){
            var playIndex = cursor.playIndex;
            if( cursor.focusable[cursor.blocked].items.length <= 0 )return;
            var item = cursor.focusable[cursor.blocked].items[playIndex];
            cursor.call("playMovie",item);
        },
        ...
        move : function(index){
            ...
            if( index == 1 && blocked >= 2  || index == -1 && blocked <= 0 || index === 11 && focus <= 0 || index === -11 && focus + 1 >= items.length ) return;
        ...
        show:function(){
            ...
            $("mask").className = "title title" + String( blocked + 1);
