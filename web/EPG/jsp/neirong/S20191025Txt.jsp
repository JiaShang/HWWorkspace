...
<%
    ...
    String titlePg  = inner.pictureUrl("", column.getPosters(), "4");
%>
<html>
<head>
    ...
    <style>
        ...
        .flowedBody {position:absolute;width:1048px;left:102px;top:234px;height:390px;overflow: hidden;}

        .flowedContainer,.flowedContainerFocus{width:1046px;height:69px;float:left;overflow: hidden;}
        .flowedContainerFocus {height:74px;word-break:keep-all;white-space:nowrap;text-overflow:ellipsis;overflow: hidden;position:absolute;left:102px;}
        ...
        .title{width:400px;height: 45px; position: absolute; left:100px;top:185px; background:transparent url('images/mask-2019-09-09.png') no-repeat;background-position: 0px 0px;}

        .flowedContainerFocus .item .marquee {width:985px;height:69px;line-height:60px;font-size: 24px;}

        .page {width:22px;height:22px;left:1180px;position:absolute;color:#15172E;font-size:20px;overflow: hidden;text-align: center;}
        .number {top:323px;}
        .count {top:346px;}
    </style>
    <script language="javascript" type="text/javascript" src="player/common.js"></script>
</head>
<body leftmargin="0" topmargin="0" style="overflow:hidden; background:black url('images/bg-2019-10-25-txt.jpg') no-repeat;" onUnload="exit();">
<div class="title" style="background:transparent <%= isEmpty(titlePg) ? "none" : "url('" + titlePg + "')" %> no-repeat"></div>
    ...
    var initialize = {
        ...
        lazyShow : function(){
            ...
            cursor.calcStringPixels(text, 24, function(width){
                if( width <= 985 ) return;
                $('mask').innerHTML  = '<div style="height:60px;background-color: #c3b272;"> <div class="item"><marquee class="marquee" scrollamount="8">' + text + "</marquee></div></div>";
            });
        },
        showItems   :   function(){
            var blocked = cursor.blocked;
            ...
            $("pageNum").style.left = $("pageCount").style.left = '1176px';
            ...
            $("mask").innerHTML = "<div style='height:60px;background-color: #c3b272;'><div class='item'>" + title + "</div></div>";
            $("mask").style.top = String(69 * current  + 229) + "px";
        }
    };
