...
<%
  // 转发路径
  String fowardpath = "go_geninfoPlay_mobile.jsp";
  String queryString=request.getQueryString();
  System.out.println("go_auth_pro queryString="+queryString);
  ...
  String spName = "";
  System.out.println("progId=="+progId+",,idType=="+idType);
  if ("FSN".equals(idType)){
	neVodId  = progId;
 	MetaData meta = new MetaData(request);
    Map vodMap = meta.getContentDetailInfoByForeignSN(progId, 0);
    if (null != vodMap)  {
		progId  = String.valueOf(vodMap.get("VODID")); 
		Map detailMap = meta.getVodDetailInfo(Integer.parseInt(progId));
		if (null != detailMap){
   ...
   System.out.println("neVodId=="+neVodId+",,progId="+progId+",idType="+idType);
  ...
  switch (resultCode)
  {
    ...
    case AUTH_FAIL_QUERYOK_IPTV:
      ...
      List monthList = (List)authInfo.get("MONTH_LIST");
	   System.out.println("auth monthList.size()="+monthList.size());
      //按次订购产品列表
