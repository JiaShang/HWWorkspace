...
<%!
 ...
 /**
  ...
  public VODMediaFile getVODMediaFile(int playType, Map mp, HttpServletRequest request)
  {
    ...
    if(null != mediaList && mediaList.size() > 0)
    {
      ...
      for(int i = 0; i < decodemodes.length; i ++)
      {
		...
		//System.out.println("=====================decodemodes=="+i+"==="+decodemodes[i]);	
  ...
  public Map checkAuth(Map mp,HttpServletRequest request)
  {
	...
	//System.out.println("go_auth_pro test extPar1 = "+ extPar1+", extPar2=="+extPar2);
...
<%
  ...
  //System.out.println("go authorization pro queryString==="+queryString);
  // 转发路径
  String fowardpath = "go_geninfoPlay_pro.jsp";
  ...
  if("FSN".equals(idType)){
    ...
    Map vodMap = md.getContentDetailInfoByForeignSN(progId, intContentType);
    if (null != vodMap)  { 
		if(intContentType == 0){
			progId  = String.valueOf(vodMap.get("VODID")); 
		}else{
			progId  = String.valueOf(vodMap.get("PROGID")); 
		}
		//System.out.println("idType="+idType+",progId="+progId);
		if(playtype.equals("11")){//兼容电视剧子集一键跳转;
			parentVodId = String.valueOf(vodMap.get("FATHERVODID"));
		}
		Map detailMap = md.getVodDetailInfo(Integer.parseInt(progId));
		if (null != detailMap && detailMap.size() != 0){
			spName = (String) detailMap.get("SPNAME");
			spName = spName.trim();
		}
	}
  }
  //System.out.println("neVodId=="+neVodId+",parentVodId="+parentVodId+",progId="+progId+",idType="+idType+",spName="+spName);
  ...
  */
  ...
  //System.out.println("go_auth neVodId=="+neVodId+",typeId="+typeId+",intPlayType="+intPlayType+",intParentVodId="+intParentVodId+",progId="+progId+",idType="+idType+",FSNTypeId="+FSNTypeId);
  ...
  switch (resultCode)
  {
    ...
    case AUTH_FAIL_QUERYOK_IPTV:
      ...
      if("Bytimes".equals(orderType))
      {
        if(null != timesList && timesList.size() > 0)
        {
          ...
          resultMp.put("Anci_flag","Anci_flag");
	  	  resultMp.put("price",getPrice(price));
        }
        else
        {
			int countNum = 0;
			...
			//System.out.println("Bytimes countNum=="+countNum);
		 	...
		 	if(countNum == 5){
				...
				for(int i=0;i<monthList.size();i++)
				{
					...
					//"100120".equals(prodCode) || "100000".equals(prodCode)
					if("100375".equals(prodCode) || "100473".equals(prodCode)){
						message = "请开通来点播产品，即可收视本节目。";
						tmpFlag = true;
      ...
      resultMp.put("message",message);
	  resultMp.put("MONTH_LIST",monthList);
	  resultMp.put("TIMES_LIST",timesList);
          break;
        ...
        case OPER_SUCESS:
          ...
          MetaData meta = new MetaData(request);
          // 获取VOD的详情信息
		  //System.out.println("intProgId="+intProgId);
          Map vodDetail = meta.getVodDetailInfo(intProgId);
          String price = (String)vodDetail.get("VODPRICE");
		  //System.out.println("geninfo getPrice(price)=="+getPrice(price));
			resultMp.put("price",getPrice(price));
			//System.out.println("Auth_removeBase_nor OPER_SUCESS price="+price+",getPrice(price)=="+getPrice(price));
           break;
  ...
  //System.out.println("go auth pro idType="+idType+",progId="+progId+",spName="+spName);
