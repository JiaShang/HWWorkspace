...
function getJishuFocus(){
	//$("jishuNum" + jishuListBox.focusPos).style.background = "#af2837";
...
function tip_fromBeginPlay() {
	...
	//$("data_ifm").src = baseurl+"/EPG/jsp/defaultHD/en/Authorization_ad.jsp?typeId=" + typeId + "&playType=11&parentVodId="+parentId
	//		+"&progId="+subList[jishuListBox.position].vodId + "&contentType=0&startTime=0&business=1";
	window.location.href = baseurl+"/EPG/jsp/neirong/player/playerAd.jsp?typeId=" + typeId + "&playType=11&parentVodId="+parentId
			+"&progId="+subList[jishuListBox.position].vodId + "&contentType=0&startTime=0&business=1";
...
function doBack() {
    <%
        String url = turnPage.go(-1);
        int indexBkUrl = url.startsWith("/EPG/jsp/defaultHD/en/playfilm.jsp") ? -4 : -1;
    %>
    window.location.href = "<%=turnPage.go( indexBkUrl )%>";
}
